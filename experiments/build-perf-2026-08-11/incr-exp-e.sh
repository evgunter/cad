...
phase() {
  local name="$1" incr="$2"
  # Env alone cannot unset a CONFIG-FILE rustc-wrapper; CARGO_BUILD_RUSTC_WRAPPER=""
  # is the override that actually disables it (verified).
  export CARGO_BUILD_RUSTC_WRAPPER=""
  export CARGO_INCREMENTAL="$incr"
  say ""
  say "=== config $name  (CARGO_INCREMENTAL=$incr, sccache OFF)"
...
say "config E: incremental ON, sccache OFF — the alternative to what shipped — is CARGO_INCREMENTAL=0 costing the edit loop?"
say "load at start: $(cut -d' ' -f1-3 /proc/loadavg)"

phase E_incr_no_sccache 1
