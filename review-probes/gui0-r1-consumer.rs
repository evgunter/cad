...
fn main() {
    ...
    println!("   nodes = {}, roots = {}, root = {root:?}", doc.order().len(), doc.roots().len());
    ...
    for exp in [3i32, 4, 5] {
        ...
        println!(
            ...
            s.faces,
            s.triangles,
            b.min(Axis::X), b.min(Axis::Y), b.min(Axis::Z),
            b.max(Axis::X), b.max(Axis::Y), b.max(Axis::Z),
        );
    }
    ...
    println!("   distinct undirected edges = {}, unbalanced = {unbalanced}", edges.len());
        ...
        match Camera::framing(&mesh.bounds(), aspect) {
            Ok(cam) => {
                ...
                println!(
                    ...
                    cam.distance(), cam.min_distance(), cam.max_distance()
                );
            }
    ...
    let ops = [
        CameraOp::Orbit { yaw: 0.3, pitch: 0.2 },
        CameraOp::Dolly { factor: 0.5 },
        CameraOp::Pan { right: 0.01, up: -0.005 },
    ];
    let walked = camera::fold(&cam, &ops).expect("fold");
    println!(
        "   after 3 ops: yaw {:.4} pitch {:.4} dist {:.5} target ({:.4},{:.4},{:.4})",
        walked.yaw(), walked.pitch(), walked.distance(),
        walked.target().x, walked.target().y, walked.target().z
    );
    // Two DIFFERENT refusals in one fold: which one comes back?
    let mixed = [
        CameraOp::Dolly { factor: -1.0 },
        CameraOp::Orbit { yaw: f64::NAN, pitch: 0.0 },
    ];
    println!("   fold(-dolly, NaN-orbit) = {:?}", camera::fold(&cam, &mixed));
    let mixed_rev = [
        CameraOp::Orbit { yaw: f64::NAN, pitch: 0.0 },
        CameraOp::Dolly { factor: -1.0 },
    ];
    println!("   fold(NaN-orbit, -dolly) = {:?}", camera::fold(&cam, &mixed_rev));

    println!("\n== 6. the input mapping, from outside ==");
    let map = InputMap::default();
    let vp = ViewportSize { width_px: 1280.0, height_px: 720.0 };
    let stream = [
        ViewportEvent::Drag { button: PointerButton::Primary, shift: false, delta_px: [40.0, 0.0] },
        ViewportEvent::Drag { button: PointerButton::Primary, shift: true, delta_px: [40.0, 0.0] },
        ViewportEvent::Drag { button: PointerButton::Middle, shift: false, delta_px: [40.0, 0.0] },
        ViewportEvent::Drag { button: PointerButton::Middle, shift: true, delta_px: [40.0, 0.0] },
        ViewportEvent::Drag { button: PointerButton::Secondary, shift: false, delta_px: [40.0, 0.0] },
    ...
    println!("   stream produced {} ops, final distance {:.5}", produced.len(), end.distance());
    ...
    for bad in [0.0, -1.0, f64::NAN, f64::INFINITY, f64::MIN_POSITIVE] {
        match DisplayTolerance::new(bad) {
            Ok(d) => {
                ...
                println!("   delta {bad:e} ACCEPTED by the door; scene_of -> {}",
                    match r { Ok(m) => format!("ok, {} tris", m.stats().triangles), Err(e) => format!("{e:?}") });
            }
    ...
    let empty = Aabb { min_x: 1.0, min_y: 1.0, min_z: 1.0, max_x: 0.0, max_y: 0.0, max_z: 0.0 };
    println!("   framing an inverted box: {:?}", Camera::framing(&empty, 1.0));
    println!("   framing with aspect 0:   {:?}", Camera::framing(&mesh.bounds(), 0.0));
    println!("   apply(Frame aspect NaN): {:?}", camera::apply(&cam, &CameraOp::Frame { bounds: mesh.bounds(), aspect: f64::NAN }));
    ...
    println!("   camera::apply: fn(&Camera, &CameraOp) -> Result<Camera, CameraOpError>  [no toolkit, no arena key]");

    println!("\n== 9. does Tol reach the scene as a parameter? ==");
    println!("   scene_of(&doc, delta, tol) — tol is a parameter; the only mint is the bin's pncad::tolerance::witness()");
    let _ = Tol::witness;
}
