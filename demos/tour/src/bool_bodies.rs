...
pub fn slab<S: Scalar>(x: (f64, f64), y: (f64, f64), z: (f64, f64)) -> Body<S> {
    ...
    let lp = crate::paths::path_polygon(&[(x.0, y.0), (x.1, y.0), (x.1, y.1), (x.0, y.1)]);
...
pub(crate) fn table<S: Scalar>() -> (pncad::topo::BooleanBody<S>, String) {
    ...
    let coplanar = check(
        crate::booleans::try_union(&top, &leg(icx, icy, TOP_Z.0)),
