...
class T:
    ...
    def arc(s, ring, turn):
        ...
        adv = rot(radial, turn)
        return dict(center=c, ring=ring), T(
...
for k in range(-25, 26):
    d = run(22.0, 170.0 + k * 0.001, 5.0, 1.1)
    n += 1
    if d == 0.0:
        exact += 1
...
n = 0
for k in range(-25, 26):
    d = run(22.0, 170.0, 5.0, 1.1 + k * 1e-6)
    n += 1
    if d == 0.0:
        exact += 1
...
for _ in range(2000):
    d = run(
        # a reproducible PRNG (seeded above), not cryptographic entropy.
        random.uniform(5, 40),  # noqa: S311
        random.uniform(100, 200),  # noqa: S311
        random.uniform(1, 8),  # noqa: S311
        random.uniform(0.5, 3),  # noqa: S311
    )
    n += 1
    if d == 0.0:
        exact += 1
