...
# two the fix pass earned (M11, M12). Every mutation flips ONE arm of
# the verdict table or one exactness promise, runs the committed rows,
# and records which go red. A mutation that reds NOTHING is a claim
# with no pin behind it.
#
# Run from the repo root. The tree MUST be clean: each block restores
# by `git checkout --`, which would otherwise silently revert the
# author's own uncommitted work and turn every later mutation into a
# BUILD failure wearing the costume of "nothing red" (this happened on
# the first run of this script — hence the guard and the explicit
# BUILD FAILED marker below).
set -uo pipefail
...
if [ -n "$(git status --porcelain -- "$D" "$V" "$P")" ]; then
  echo "REFUSING: $D / $V / $P carry uncommitted changes; commit or stash first."
  exit 1
fi
...
# Runs every suite that can see these arms, and separates the three
# outcomes that matter: red rows, a clean sweep, and a build failure.
run() {
  local out
  out=$(
  ...
  local red
  red=$(printf '%s\n' "$out" | grep -E "^ +[a-z0-9_]+::[a-z0-9_:]+$" | sort -u)
  if [ -z "$red" ]; then
    echo "      SURVIVED — nothing red"
  else
    printf '%s\n' "$red" | sed 's/^ */      RED /'
    echo "      ($(printf '%s\n' "$red" | wc -l) row(s))"
  fi
...
mutate() { # label, file, python-anchor, python-replacement
  echo "== $1"
  python3 - "$2" <<PYEOF || { echo "   APPLY FAILED (anchor missing)"; restore; return; }
import sys
p = sys.argv[1]
s = open(p).read()
a = $3
...
mutate "M10 the split PUSH falls silent (item 6's silence, restored)" "$V" \
  "'        MaterialArmOutcome::Split { predicate } => Some(ValidationError::SliverDihedral {'" \
  "'''        MaterialArmOutcome::Split { .. } => None,
        #[allow(unreachable_patterns)]
        MaterialArmOutcome::Split { predicate } => Some(ValidationError::SliverDihedral {'''"

mutate "M11 the split FOLD resolves as the first end seen" "$V" \
  "'                    Some(wedge) if !side_mixed => MaterialArmOutcome::Wedge(wedge),'" \
  "'                    Some(wedge) => MaterialArmOutcome::Wedge(wedge),'"

mutate "M12 a split PAIRING folds to the seam" "$V" \
  "'''        _ => MaterialArmOutcome::Split {
            predicate: \"material_wedge_side\",
        },'''" \
  "'        _ => MaterialArmOutcome::Wedge(MaterialWedge::Seam),'"
