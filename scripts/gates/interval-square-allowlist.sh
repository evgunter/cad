...
gate() {
  ...
  hits=$(gate_rust_code --skip-cfg-test --statements "${GATE_PRODUCTION_FILES[@]}" \
...
gate_selftest() {
  ...
  printf '%s selftest OK: passes a clean fixture, and prose, string literals and near-miss products; fires on each square spelling the matcher claims — bare identifier, field path, `self.` field, nested path, rustfmt-wrapped product, behind a block comment, and the parenthesized scaled square in both forms; and it stays RED, with a diagnosis, when `grep` itself cannot run\n' "$(gate_name)"
}
