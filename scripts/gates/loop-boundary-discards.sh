...
REGISTER=(
  ...
  # `split_edge`'s row carry, hulling the face's chart window over the
  # rows it already stores. Its arm is `clear_face_caches`'s above,
  # walking the same loops of the same face for the same reason; left
  # `unaudited` because that one is — inheriting a disposition that does
  # not exist is the register's own failure mode.
  "crates/topo/src/pcurves.rs|split_cache||1|unaudited"
