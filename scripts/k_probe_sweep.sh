...
# exactly this script with --gzip (raw rows are 162-203 MB, an undated
# reading of one cut that nothing re-takes — it is here to say why the
# durable record is gzipped, which stays true at any size in that
# neighbourhood; the gzipped
...
run_dump() {
  local eps=$1 label=$2 pkg=$3 module=$4 out=$5 feats log
  feats=$(feats_for "$module")
  log=$(mktemp)
  echo "=== k-probe sweep @ eps=$eps ($label) ==="
  CAD_TOLERANCE_EPS=$eps CAD_K_REPORT_OUT="$out" \
    cargo test -p "$pkg" --features "$feats" --test all -- \
...
for eps in 1e-6 1e-9 1e-12; do
  ...
  driver="$outdir/driver/$prefix$eps.csv"
  ...
  # THE E6 DRIVER'S OWN POPULATION (M10-3, E6's T6 obligation): the
  # margins the subdivision driver's certified leaves decide at their
  # own midpoints — the first sample of margins DRIVEN toward zero by
  # refinement rather than chosen by an author. It rides beside the
  ...
  rm "$corpus" "$demos"
  if [ "$gz" = "--gzip" ]; then
    gzip -9 -f "$merged" "$m2" "$driver"
    echo "wrote $merged.gz, $m2.gz and $driver.gz"
  else
    echo "wrote $merged, $m2 and $driver"
  fi
done
