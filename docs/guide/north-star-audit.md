...
| 27 | `teapot` | NO | G17 | STAYS NO, on SHELL and nothing else now — and the shell half of that is a JOB rather than a gap since LIB-G17: `Node.shell` binds `topo::shell_open` with the open faces as ordered names, so the vessel IS sayable (`crates/editor-core/tests/corpus/vessel.rs` is the spelling, `tests/test_shell.py` the executed cup); this row flips when the Python suite executes the scene, and the tour's conversion is filed on LIB's slate. Before LIB-G17: `shell`/`shell_open` shipped at #1048 and had no recipe node, so the vessel — the whole point of the scene — had no document, and that unit was held on #1202 behind a kernel birth-record ask (RECIPE-DOORS D5). The handle's blocker LIFTED at LIB-TUBE (it is `tube_along_arc`, which `Node.tube` now binds), so this row's G2 half is discharged; the lid (revolve + fillet) and the spout (revolve + transform) were always sayable. One body still has no document, so the scene is still NO |
| 28 | `die` | **YES** | — | — |
| 29 | `table` | **YES** | — | — |
| 30 | `silhouette` | **YES** | — | — |
| 31 | `silhouette3` | **YES** | — | — |
...
| 44 | `torusvessel` | NO | G17 | the same disposition as row 27 (a job since LIB-G17, not a gap: `Node.shell` is bound; the row flips when the Python suite executes the scene), on the scene that carries `shell`'s newest surface kind: `revolve` says the meridian, and then `topo::shell` — which has no `Node` variant — is the whole point. Nothing else about the body is unsayable: the meridian is one `Open`-lattice path whose only curved segment is an `arc_to` about a centre off the axis, which the PATHS lattice has bound since LIB-PYG1. So this row moves the instant G17 does, and it is the row that says what G17 costs now: the torus arm (#1494) widened what `shell` REACHES without widening what a document can ask for |
...
| G17 | **Shell has no recipe node** — CLOSED at LIB-G17 as a gap; its three rows stay NO as JOBS | 3 | register B; `docs/KERNEL-VERBS.md`'s shell row (**SHIPPED at #1048**, the Q8 substrate arc) | **LIB-G17 landed `Node::Shell` and `Node.shell`** (`{ target, thickness, open }`, the open faces as ORDERED names — the first named carries the rim — an empty list the sealed hollow; `crates/editor-core/tests/corpus/{cup,vessel}.rs` and `tests/test_shell.py` execute it). What keeps rows 27, 44 and 45 at NO is this page's own discipline — a YES is a scene the Python suite executes — and no Python row authors those scenes yet; the tour's conversion is on LIB's slate. The text below is the gap as it stood before the node, kept because it says what the rows cost. The same shape as G16, one verb over. `topo::shell` and `topo::shell_open` shipped, and `Node` has no shell variant either — so the vessel that is `shell`'s DESIGNATED demo (Ev, 2026-08-09: *"a vessel is a shelled revolve"*) is the one part of the teapot with no document — and, since the torus-walled vessel arrived, the whole of two more scenes (rows 44 and 45), which are nothing BUT a revolve and a shell. That is what makes this gap's cost legible now: #1494 widened the set of walls `shell` can hollow without widening what a document can ask it for, so the verb's reach and its authorability moved apart rather than together. `torusvesselcup` also names a SECOND unbound door as its secondary — `Body::merge_coplanar_faces`, a repair rather than a construction — which is recorded on that row instead of here, because no scene is blocked by it alone. The teapot scene's other three bodies are not what blocks IT: the lid is a revolve plus a `Node.fillet`, the spout a revolve plus a `Node.transform`, both sayable today; the handle is `tube_along_arc`, so it waits on G2. Klein is `shell`'s other consumer and does NOT appear here, which is the distinction worth keeping: that scene pays for the verb's absence by authoring every wall as its own two offsets by hand, and the result is a body Python can build (row 15). Paying a cost is not the same as being blocked |
