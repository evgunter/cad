...
**Result: 36 of the 45 tour stops are authorable through Python
today** — 32 outright, and 4 more only with a caveat the row states:
three re-author by hand what the scene says structurally, and one says
it through a different node than the scene's. 9 are blocked by a
missing door.
...
The three gaps partition the 9 NO rows exactly, counted off the table
above: **G2** takes 5 (rows 13, 19–22), **G17** 3 (rows 27, 44, 45) and
**G19** 1 (row 38) — 5 + 3 + 1 = 9. G17's count rose from 1 when the
torus-walled vessel arrived: the same unbound verb, two more scenes
asking for it. **G16** takes 0: LIB-G16 closed
it, and rows 2, 11 and 12 went with it. G2's own count fell from 8 to
