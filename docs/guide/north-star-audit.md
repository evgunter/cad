...
**Result: 37 of the 45 tour stops are authorable through Python
today** — 34 outright, and 3 more only with a caveat the row states:
each re-authors by hand what the scene says structurally. 8 are
...
| 27 | `teapot` | NO | G17 | STAYS NO, on SHELL and nothing else now — and the shell half of that is a JOB rather than a gap since LIB-G17: `Node.shell` binds `topo::shell_open` with the open faces as ordered names, so the vessel IS sayable (`crates/editor-core/tests/corpus/vessel.rs` is the spelling, `tests/test_shell.py` the executed cup); this row flips when the Python suite executes the scene, and the tour's conversion is filed on LIB's slate. The handle's blocker LIFTED at LIB-TUBE (it is `tube_along_arc`, which `Node.tube` now binds), so this row's G2 half is discharged; the lid (revolve + fillet) and the spout (revolve + transform) were always sayable. One body still has no document, so the scene is still NO |
| 28 | `die` | **YES** | — | — |
| 29 | `table` | **YES** | — | — |
| 30 | `silhouette` | **YES** | — | — |
| 31 | `silhouette3` | **YES** | — | — |
...
| 44 | `torusvessel` | NO | G17 | The body is a revolve and a shell, and both are bound: `Node.revolve` says the meridian (one `Open`-lattice path whose only curved segment is an `arc_to` about a centre off the axis, bound since LIB-PYG1) and `Node.shell` hollows it, the mouth named as its two half-faces (`crates/editor-core/tests/corpus/vessel.rs` is the spelling). What keeps the row at NO is this page's discipline — a YES is a scene the Python suite executes — and no Python row authors it yet; it flips with the tour's conversion (`work/lib/teapot-scene-through-node-shell.md`). The torus arm (#1494) widened what `shell` reaches, and the document can now ask for it |
| 45 | `torusvesselcup` | NO | G17 | the same body one verb later. Since LIB-G17 the shell half is row 27's job rather than a gap (`Node.shell` binds `shell_open`, an empty designation being the sealed hollow); what is still unbound is the SECOND door: `Body::merge_coplanar_faces`, which this scene calls as its second step. The merge is a REPAIR rather than a construction, so binding it is a separate question from G17's — recorded here as the row's named secondary rather than as a gap of its own, because no scene is blocked by it alone: row 44 next door is the same vessel without the merge and is blocked by G17 on its own |
...
| G17 | **Shell recipe node** — CLOSED at LIB-G17 | 3 | register B; `docs/KERNEL-VERBS.md`'s shell row (**SHIPPED at #1048**, the Q8 substrate arc) | `Node::Shell { target, thickness, open }` and `Node.shell` landed (RECIPE-DOORS D5): the faces to open are ORDERED names — the first designated face of a chart carries its rim — and an empty list is the sealed hollow. `crates/editor-core/tests/corpus/{cup,vessel}.rs` and `tests/test_shell.py` execute it: the cup's closed forms exactly, the vessel's two-faced mouth as one rim, rebinds and rebuilds keeping the names. Rows 27, 44 and 45 stay NO as JOBS, not gaps: this page's discipline is that a YES is a scene the Python suite executes, and the three scenes' Python rows are the tour's conversion (`work/lib/teapot-scene-through-node-shell.md`); row 45 additionally names `Body::merge_coplanar_faces`, still unbound. What the gap cost while it stood: the verb's reach and its authorability moved apart (#1494 widened the walls `shell` can hollow without widening what a document could ask for), and the teapot's mouth was found by a numeric plane scan because nothing could name it |
...
The two remaining gaps partition the 8 NO rows exactly, counted off
the table above: **G2** takes 5 (rows 13, 19–22) and **G17** 3 (rows
27, 44, 45) — 5 + 3 = 8. G17's count rose from 1 when the
torus-walled vessel arrived: the same unbound verb, two more scenes
asking for it. **G16** takes 0: LIB-G16 closed
...
reading.
Authorable = 34 outright + 3 YES\* = 37, and 37 + 8 = 45.
