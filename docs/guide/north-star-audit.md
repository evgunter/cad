...
half and on `evaluate(doc)`'s missing resolver. A gap can close
...
- `ContentPin` and `DocRef` — which VERSION, and the (part, version)
  ...
  version is. What is NOT bound is the half that USES a reference —
  no `InstantiatePart` node, and `evaluate(doc)` takes no resolver:
  see G18.
...
| 46 | `bench` | NO | G18 | the whole assembly vocabulary is unbound, and structurally FIRST `evaluate(doc)` takes no resolver, so an `InstantiatePart` node cannot evaluate from Python at all. G15 was this row's named SECONDARY blocker and is CLOSED (LIB-G15): the two bench documents are now writable into a workspace side by side and readable back through a pinned `DocRef`, and what they still cannot do is instantiate one another |
...
One further gap is no row's PRIMARY blocker and still matters to the
library:
