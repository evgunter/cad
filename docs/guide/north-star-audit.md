...
The standing goal, recorded by Ev on 2026-08-09 and written into
...
**Result: 44 of the 50 tour stops are authorable through Python
today** — 36 outright, and 8 more only with a caveat the row states:
...
**LIB-G16** bound the chamfer node, closing G16 (`spacer`,
`diechamfer` and its bumped twin); **LIB-TUBE** bound the tube PAIR
— `Node.tube` and `Node.hollow_tube`, two kinds over the kernel's
two public doors — closing G2's tube half (`tube_along_arc`,
`hollowelbow`, `hollowtorus`) and lifting the tube blocker off
`lily` and `teapot`, which stay NO on sweep/placement and shell
respectively.
...
- `SketchPlane` — the sketch plane as a VALUE: the named cyclic
  ...
  `from_frame(origin, u, v)`. The binding adds no predicate of its
  own: rigidity is the kernel mint's, so `from_frame` orthonormalizes
  the pair it is given (keeping `u`, yielding `v`) and raises
  `FrameError` when the two span no plane.
...
- `Node.boolean(op, a, b, declare=…)` — union, intersect, subtract.
  ...
  flush pairs as typed `FlushFinding` values — every carrier the
  `Rest` ladder verifies, plane through torus, since SEAT-FW closed
  G19 — `Node.declare(findings)`
...
- `Workspace(path)` — a DIRECTORY of documents, scanned by each
  ...
  map; `create`, `resave`, `save_at` and `save_as_new_document` are
  the write doors (there is no general mutation API, which is the Rust
  surface's posture kept), and the last two are the two acts a save is
  — a save at a path keeps the identity and refuses typed rather than
  letting a copy claim it twice, a save AS A NEW DOCUMENT forks;
...
- `Body.tessellate(chordal)` and the `Mesh` it answers — the ladder's
  ...
  `Mesh.boundaries` answers the per-edge boundary polylines in the
  same index alphabet, so the model's edges are drawable too. What
  does not cross are the arena keys: a patch's face and a boundary's
  edge are keys, so both are addressed by index, and `NodePick`'s
  `patch_names` / `boundary_names` are the inversion.
...
the LIB residual register (`work/lib/log.md`, "LIB residual register",
...
missing door — so the ids partition the 7 NO rows exactly; secondary
...
| 13 | `lily` (15 bodies) | NO | G2 | STAYS NO, and the reason narrowed rather than vanished at LIB-TUBE: the tube half of its blocker is gone (`Node.tube` exists), but the scene also needs SWEEP — `wire_sweep` refuses unconditionally, banked on U4/LQ3 — and PLACEMENT (G7). A scene is YES when every body in it is sayable, and one door of three arriving does not make fifteen bodies buildable |
...
| 23 | `tube_along_arc` | **YES** | — | **LIB-TUBE.** `Node.tube(spine, u_ref, major_radius, window, minor_radius)` binds this door: the spine is a `Node.datum_axis` consumed whole (origin = centre, direction = axis, `Node.revolve`'s own precedent), the reference direction crosses as components, and the window is a `TubeWindow` value with two spellings. The scene's claim is the STORAGE contract, so the flip is metered at the stored bits, not the volume: `lib_tube_node.rs` reads `Surface::Torus`'s `minor_radius` off the built body and compares `to_bits()` against the authored number. **CORRECTION to this row's own prose:** it used to say "one node kind, not two", which was a sequencing note written at VERBS-TUBEWALL, never ratified design. The #1205 ruling (Ev, 2026-08-29) went the other way — two node kinds — and RECIPE-DOORS D4 was revised to match. Both are ordinary additive vocabulary now: #1553 demolished the schema version this row once had to be sequenced against |
...
| 25 | `hollowelbow` | **YES** | — | **LIB-TUBE.** `Node.hollow_tube(...)` with a `TubeWindow.arc` window, and the wall is a REQUIRED argument — hollowness is spelled by which door you call. The row flips only as far as its own claim, which is the storage contract and not the shape: `hollow_tube_elbow` (registered corpus) is metered at the STORED BITS — two outer half-walls holding the authored `minor_radius` verbatim, two inner ones holding `minor_radius - wall` as one IEEE subtraction — beside the annular Pappus forms. The volumes alone would not settle it (they agree to 1e-12 either way), which is exactly why this row's subject was never the volume |
| 26 | `hollowtorus` | **YES** | — | **LIB-TUBE.** The same door at the full period: `Node.hollow_tube` with `TubeWindow.full()`, which builds a torus shell whose cavity is inserted through the shared void door. `lib_tube_node.rs` meters `V = 2π²R(rₒ² − rᵢ²)` and the two-shell topology, and pins the same stored-bits contract as row 25. The distinction from `hollowring` (row 24) SURVIVES the flip and is worth keeping: that row is this census through the PROFILE door — a holed profile revolved, radii reconstructed — and this one is through the parameter door, radii stored. Both are YES now; they are still not the same claim |
| 27 | `teapot` | **YES** | — | **LIB-TEAPOT.** The whole scene is ONE recipe document now, and `TestTeapot` in `tests/test_north_star.py` executes it: the pot is a `Node.revolve` hollowed twice by `Node.shell` — an EMPTY open list for the sealed body, the mouth's two `Band`/`BandPi` half-discs BY NAME for the cup — the lid is a revolve whose three latitude rims roll through `Node.fillet` on their `BandRim` names, the spout is a CANAL — seven annular sections on `Node.datum_frame`s that are a circular arc's own tangent frames, skinned by `Node.loft` and placed by `Node.transform` — the handle is `Node.tube` on a `Node.datum_axis`, and the two joins the operand gate refuses are `Node.boolean` nodes that fail at `evaluate` under the same payloads the kernel raised, at the gate's TWO different rungs (a germ pair for the handle, a rung-3 edge carrier for the canal). **The numeric plane scan is retired**: the mouth used to be found by scanning the built body for planar faces at the mouth's station because nothing could name it, and the tour's `plane_chart_at` and `rim_at` are deleted with it. Every body is held to the scene's own closed forms, restated in Python from the same dyadic constants — the canal's being the STRAIGHTENED FRUSTUM, since a bend about a spine through the sections' centres is volume-neutral, and asserted as a BRACKET against the kernel's own certified enclosure because these rational walls are quadrature faces. The Python row runs at the default eps, where that door opens; the Rust scene runs the same body at 1e-12 too, where it does not — the reporting target is under this body's quadrature floor, so there is no volume NUMBER to bracket against. Tier 3 certifies it there all the same, on the SIGN of that same enclosure, and the tour's ribbon prints the bracket instead of a number. The lid's three rims roll in ONE `Node.fillet` request, as `fillet_edges` takes them: the flange's rim and the dome's foot are the two ends of one meridian segment, so both bands slit and cross that segment's seam, and `RoleSeg::BandSlit` and `RoleSeg::BandCross` tell the two apart by the band that made each |
| 28 | `die` | **YES** | — | — |
| 29 | `table` | **YES** | — | — |
| 30 | `silhouette` | **YES** | — | — |
| 31 | `silhouette3` | **YES** | — | — |
...
| 38 | `twopeg` | **YES** | — | ONE cell since montage-v3, carrying the mated body and the apart framing both. The APART half was always authorable — two PARTS that are ordinary transverse booleans plus a rigid lift. The MATE is authorable now: its three declared contacts are one planar `Rest` and two CYLINDRICAL ones, and the flush detector reports all three, because its reach is the `Rest` ladder's reach rather than a plane-only subset of it. `TestTwopeg` authors it end to end — 25 findings, `declare_all`, and the union GLUES at the scene's own exactly-additive oracle (2·6·4·1 = 48) — and a sibling row declares only the walls the plane rung reaches and gets the curved-face refusal back, so the cylindrical declarations are measured to be what unlocks the arm |
...
| 43 | `bench` | **YES** | — | ONE cell since montage-v3, assembled AND flat-packed, and the flat-pack now says its posts through the scene's own node: `Node.pattern`, since LIB-BENCH-PATTERN. What that unit MEASURED — the question the star stood for, which is what a PLURAL value does at the doors a single body walks through — is that the flat-pack answers the same either way: the gather takes the `Instances` value (`crates/editor-core/src/product.rs`'s arm for it), the instance-qualified names and their per-placement cap frames read the same, and the A5 gate passes OUTRIGHT with nothing minted over `PATTERN_COUNT x post + shelf` of material. `TestBenchLayout` runs every one of those expectations against BOTH spellings, so the equality is a reading rather than a sentence. The two part at exactly one door and the pattern is the better side of it: a mate head stands on a pattern's COPY, where the fused spelling leaves no instance to be a member of and refuses — measured, and the layout's own expectations are unaffected because it declares nothing. Nothing is hand-authored. On the assembled half, the assembly series is complete, and this row is what it was for. LIB-G15 put the two part documents in a store; LIB-G18a gave evaluation a resolver; LIB-G18b bound the AUTHORING half — `Node.instantiate_part`, `Node.mate`, `DocEdit.set_placement` / `set_roots`, `solve_document`, `product`, `assemble`. `TestBenchStand` in `crates/pncad-py/tests/test_assembly_author.py` writes the whole scene from nothing and is held to the scene's own expectations: one placement cluster of three instances with the earliest as gauge, ONLY that gauge carrying an authored frame, the far post's solved translation to 1e-12, the identity rotation two aligned frame-coincidence mates compose to, `2 x post + shelf` of material, two minted declarations, and an A5 gate that CERTIFIES. No name is hand-composed — a mate reference is what `Evaluation.select` answers when queried on an instantiate node, which is the flow this row was really waiting on |
...
| 48 | `impeller6` | YES\* | G8 | every door the scene needs is bound and named: `DocParam.count` declares the parameter, `Doc.parse_expr` reads the step string VERBATIM (`360 deg / scalar(blades)` — the grammar's one Count→Scalar promotion lives in the parser, so no binding has to carry it), `PatternKind.circular(axis, step)` takes a `Node.datum_axis`, and `Node.placed_union` says the whole blade family as ONE node whose value is an ordinary body a `Node.boolean` consumes. So the scene's own claim — that the COUNT slot and the STEP slot read the SAME parameter, and one `set_doc_param_value` moves both while the blades still close the circle — is sayable through the surface, relation and all. The star is rows 40–42's and exactly theirs: no Python row executes the placed-union-into-a-base fusion yet |
| 49 | `impeller8` | YES\* | G8 | same |
| 50 | `impeller12` | YES\* | G8 | same |
...
the *point of the scene* does not. `heatsink`'s residue — which
`impeller` now shares — is narrower than it was: the fin (or blade)
FAMILY is one `Node.placed_union` node whose count rides a document
parameter (LIB-PYPU), and the last step, fusing that family into the
base, is not a missing kernel door either (G8 retired that reading).
What is left is a JOB: no Python row executes the fusion yet. (`diecomposed` was this mark's other case — hand-narrowing
...
doc = Doc()
sketch = doc.insert(
    Node.polygon([(Expr.length_in(a, m), Expr.length_in(b, m)) for a, b in T], plane=doc.sketch_frame(plane=t_plane))
)
...
bindings-parity residual in `work/lib/log.md` → "LIB residual
...
|---|---|---|---|---|
| G2 | **Sweep** (loft closed, tube closed) | 5 | register B ("the big three") | Loft left this gap when LIB-PYG23A bound `Node.loft`. What remains is not an unbound door but two BANKED ones. **Sweep**: `wire_sweep` refuses unconditionally (`SWEEP_FRONTIER`, `editor-core/src/eval/wire.rs`) — the path-composition lane banked past M6 by the PR 10 MAJ ruling, so binding it would flip no row and un-banking is kernel-side. **Tube: CLOSED at LIB-TUBE.** `Node::Tube` and `Node::HollowTube` landed under the #1205 split ruling — two node kinds over the kernel's two public doors, the wall REQUIRED on the hollow kind — and `Node.tube`/`Node.hollow_tube` bind them. Rows 23, 25 and 26 flip; the schema-sequencing coordination item this cell used to name is discharged with them — and, since #1553, there is no version to sequence against at all. The tube/sweep/3-D-path tail is a design conversation: U4's measured spec, and **LQ3, ratified 2026-08-10 (#362, LIBRARY-DESIGN §L7)** — which names the discharge site rather than building it. A `geom-curves` chain→curve composition door is what would narrow `wire_sweep`'s refusal from everything to genuinely-unjoinable chains, and that un-banking is kernel-side work needing the kernel program's concurrence. Ratified direction, landed door not yet: rows 19–22 stay NO until U4's units land — UNMOVED by LIB-TUBE, which touched no `wire_sweep` and no path composition. **The count, re-derived at LIB-TUBE**: VERBS-TUBEWALL had widened this gap from six rows to eight by putting `hollowelbow` and `hollowtorus` behind the same missing node. Three of those eight now flip outright (23, 25, 26) and two more lose their tube half while staying NO on other blockers (13 on sweep + placement, 27 on shell), so the gap partitions FIVE rows: 13, 19, 20, 21, 22. What made the hollow pair the sharpest statement of what the node had to carry is now what its tests measure: the wall is one number the parameter door STORES (both inner half-walls hold `minor_radius - wall`, one IEEE subtraction of the caller's own two numbers), and a revolve of an annulus — which Python could always say, and `hollowring` is the row where it says it — reconstructs that radius instead. Same shape, different claim; both rows are YES now and they are still different claims |
...
| G17 | **Shell recipe node** — CLOSED at LIB-G17 | 1 | register B; `docs/KERNEL-VERBS.md`'s shell row (**SHIPPED at #1048**, the Q8 substrate arc) | `Node::Shell { target, thickness, open }` and `Node.shell` landed (RECIPE-DOORS D5): the faces to open are ORDERED names — the first designated face of a chart carries its rim — and an empty list is the sealed hollow. `crates/editor-core/tests/corpus/{cup,vessel}.rs` and `tests/test_shell.py` execute it: the cup's closed forms exactly, the vessel's two-faced mouth as one rim, rebinds and rebuilds keeping the names. Rows 27, 44 and 45 stayed NO as JOBS rather than gaps — this page's discipline is that a YES is a scene the Python suite executes — and **LIB-TEAPOT did the job for two of the three**: `TestTeapot` authors the whole teapot as one document (both hollows, the lid's named rims, the placed spout, the tube handle and the two refusing joins) and `TestTorusvessel` authors the torus-walled vessel's sealed hollow, each against the scene's own closed forms. Row 45 is what is left, and it is left on the SECOND door rather than on this one: `Body::merge_coplanar_faces`, still unbound. What the gap cost while it stood: the verb's reach and its authorability moved apart (#1494 widened the walls `shell` can hollow without widening what a document could ask for), and the teapot's mouth was found by a numeric plane scan because nothing could name it |
| G19 | **Declared contact beyond the plane** | 0 (**CLOSED**, SEAT-FW) | closed against `docs/SELECT-DESIGN.md` §3 (the detect/declare protocol) and VERB-SEAT-DESIGN §1 S3 (`crates/verbs/README.md`); the kernel side is M9-3's declared cylindrical `Rest`, which `twopeg` renders | The gap was never the DECLARE arm and never the verifier — both were carrier-agnostic already: the document layer's `resolve_declarations` pushes a `FacePairDeclaration` whatever the surface kind, and `topo`'s `carrier_pair_relation` is *"the one carrier-pair door"*, with plane, sphere, cylinder and torus rungs. What was missing was the DETECTOR's curved arm, because both seats' `find_flush_candidates` ran a rung that called the planar projection of that door and answered `None` for any curved carrier — so the two CYLINDRICAL `Rest`s `twopeg`'s mate declares had no Python path, a `FlushFinding` being the sole input to the declare arm and unbuildable by hand. SEAT-FW pointed the rung at `carrier_pair_relation` itself, so detection and declarability now coincide. `TestTwopeg` executes the closure: **25** findings on the scene's two parts (19 `SameOpposite` — the mating plane and eighteen peg-against-bore pairs — and six `SameOriented` walls), `declare_all`, and a union that GLUES at 48; a pair whose ONLY coincidence is cylindrical reports its wall pairs too; declaring just the `SameOriented` walls still refuses in the curved-face arm, which is the differential that says the curved declarations do the unlocking; and `pncad.FlushFinding()` still raises `TypeError`, so the no-fusion boundary is where it was |
| G8 | **Multi-solid boolean operand** (replication + structural params CLOSED) | degrades 6 | register B | **Re-diagnosed at LIB-PYPU, from executed evidence.** The row used to read "no pattern node and no `SetStructuralParam` edit". Two of those three halves are now bound: `Node.placed_union`/`placed_union_at` say a whole placed family in ONE node whose value is an ordinary `body` (`TestHeatsinkFins`, `tests/test_placed_union.py`), and `DocEdit.bind_count_param` binds its Count slot to a document parameter, so `heatsink`'s 5 → 7 → 9 is ONE `set_doc_param` edit each against the corpus's own dyadic pins. What is left is the residual the old entry hid behind the binding question: the heat sink's shape is that family UNIONED INTO A BASE, and **that fusion is no longer blocked**: `PlacedUnion` lowers through `graft_disjoint_all_onto_keyed`, so its value is ONE solid of N shells rather than the N-solid body `setopfinish` refuses, and an ordinary `Boolean` consumes it (`topo/src/instance.rs`'s door docs say so; `corpus/die_tool.rs` feeds a group straight into a Subtract, and `demos/tour/src/heatsink.rs` now unions one into a base). The reasoning this row used to carry — that `combine`'s single-solid operand gate meant "the fusion needs a kernel door that does not exist" — described the PRE-#571 lowering and is retired. What holds the three rows at YES\* now is narrower and is a job rather than a gap: **no Python row executes the fusion yet.** `Node.boolean` and `Node.placed_union` are both bound (`pncad.pyi`), so the path is `placed_union` → `boolean(Union, base, group)`; this page's own discipline is that a YES is executed by the Python suite, so the rows flip when `TestHeatsink` authors that and asserts the solid's volume, not before. `Node::Pattern` **is bound now** (LIB-B-PART), and the reason it was not is the thing that changed rather than this row: its value is a plural `Instances` payload no boolean can consume, which was an argument for leaving it out only while nothing consumed one at all. `Node.part` is that consumer — it projects ONE body out of a plural value, so a pattern's family reaches every single-body seat one copy at a time — and binding the projection without its only source would have shipped a door with an unreachable half. The row's own arithmetic is untouched: no scene flips, because the fusion below is still what the heat sink needs and a `Part` chain is not it (`test_a_plural_payload_cannot_feed_a_boolean` now executes all three states — the group a boolean takes, the pattern it refuses, and the Part of that pattern it takes). LIB-G18b added a fourth degraded row (43) from the OTHER side — the flat-pack layout authored through `placed_union` rather than through the scene's own `Pattern` — and **LIB-BENCH-PATTERN has since taken it back off**, by authoring the flat-pack with `Node.pattern` and MEASURING what the plural value does at the doors: the gather takes it, the names and per-placement frames read the same, and the A5 gate passes outright, so nothing there was ever waiting on this gap. It also measured the price the substitution had been paying, which is not a boolean's: a mate head stands on a pattern's copy and refuses on a `placed_union`'s. What still degrades the rows is that fusion alone, and the count rose from three to SIX when `impeller` arrived (rows 48–50) — not because the gap widened but because a second scene asks the same thing of it, and asks it of a CIRCULAR family whose count and step read one parameter. One Python row would take all six: the path is the same `placed_union` → `boolean(Union, base, group)` either way. This row's own arithmetic is otherwise untouched (`test_a_plural_payload_cannot_feed_a_boolean` executes the split's refusal and asserts the group's singular `body` as the contrast). One further residue, measured here and NOT a row-blocker, and now CLOSED (LIB-G18a): `evaluate(doc, prior=previous)` is the memo, and `Evaluation.reused` / `Evaluation.recomputed` — bound already, and until there was a prior to pass they could only ever read (0, n) — make the scene's *memoized recompute* observable. The two counters sum to the nodes that RAN OR WERE REUSED — the live node count when nothing refused, and short by exactly the poisonings when something did — so they are evidence rather than a hint. The row is unchanged otherwise: the fusion is still a kernel door |
| G14 | **Split naming: chord multiplicity + tied upstreams** | 0 (**CLOSED**, LIB-G14) | closed by LIB-G14 against `crates/editor-core/src/names/README.md`'s split-naming-walls section (RATIFIED, #512); the `NamingError`-`Display` gap it hid behind is #380, closed separately | The row's original diagnosis — "a plane crossing boolean-minted faces" — was WRONG, and the survey (`cad-work/g14-survey.md`) is how that surfaced: a split ACROSS boolean-minted faces named fine all along. TWO disjoint M4-era deferrals refused, and the one this scene hit involves no boolean at all — a section line that re-enters ONE operand face (an inner loop, or any non-convex face) would mint `SectionEdge{side, face}` twice; a plain L-shaped single-loop extrude reproduced it. The second wall refused a split whenever ANY operand-table entry was tied, even for pass-throughs nowhere near the tie. Both were logged as one M4 PR-3 sentence and read as one refusal because `NamingError` had no `Display`. Fixed as ratified: the chords become an N2 TIE (A2), and tied upstreams PROPAGATE as tied (B1), matching `name_pattern`/`name_in_part`/`graft_names` |

The two remaining gaps partition the 6 NO rows exactly, counted off
the table above: **G2** takes 5 (rows 13, 19–22) and **G17** 1 (row
45) — 5 + 1 = 6. G17's count rose from 1 to 3 when the torus-walled
vessel arrived and fell back to 1 at LIB-TEAPOT, which converted the
teapot scene and wrote the two Python rows the flips were waiting on;
what is left under G17 is one row whose blocker is a different door. **G16** takes 0: LIB-G16 closed
it, and rows 2, 11 and 12 went with it. **G19** takes 0 since the
flush detector's rungs became the verifier's: row 38 flipped with it.
G2's own count fell from 8 to
5 at LIB-TUBE: rows 23, 25 and 26 flipped, and rows 13 and 27 kept
their NO on blockers that were never the tube (sweep + placement;
shell).
G8 blocks none and still degrades six, which is what makes six of
the eight starred rows YES\* rather than YES (40, 41, 42 and 48, 49,
50 — the other two, 46 and 47, carry `—` and are starred for their own
reasons) — they carry G8 in the gap
column as the cause of the star, not as a blocker: LIB-PYPU narrowed
what G8 IS without moving a mark, because the half it closed was never
the half the mark depended on. Its count fell from four to three at
LIB-BENCH-PATTERN, which took the fourth row (43) off the list by
measuring rather than by narrowing the gap: the flat-pack is authored
THROUGH the scene's own node now, and what the star had stood for is a
reading. It rose to six with `impeller`, which is a scene arriving
rather than the gap moving.
Authorable = 36 outright + 8 YES\* = 44, and 44 + 6 = 50.
