...
| 26 | `hollowtorus` | **YES** | — | **LIB-TUBE.** The same door at the full period: `Node.hollow_tube` with `TubeWindow.full()`, which builds a torus shell whose cavity is inserted through the shared void door. `lib_tube_node.rs` meters `V = 2π²R(rₒ² − rᵢ²)` and the two-shell topology, and pins the same stored-bits contract as row 26. The distinction from `hollowring` (row 24) SURVIVES the flip and is worth keeping: that row is this census through the PROFILE door — a holed profile revolved, radii reconstructed — and this one is through the parameter door, radii stored. Both are YES now; they are still not the same claim |
