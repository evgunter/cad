...
$ scripts/render-hosted.sh --from-ci                # + montage images (CI already rendered them; see demos/README.md)
```
...
| `projectbox` | `projectbox.rs` | A 15-op boolean chain with a per-op dyadic oracle | Honest note: square-only; `gate_planar` refuses curved operands |
