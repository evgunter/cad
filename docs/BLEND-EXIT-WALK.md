...
- **Residue re-homed before this exit, by file**: by the cut
  (row 6) and by this walk's PR: `S90-impl` and the `CornerConfig`
  row to CARVE. Nothing is left in `work/blend/` but the program files
  and this walk.
