...
| census.rs (CONTACT-1) | census_touch_side / census_touch_dihedral / census_touch_span | the touch analysis's cone signs: a dot of two unit directions (a boundary ray or face normal against a candidate plane's normal; an edge's next in-face direction against the previous face's normal), and the norm of two unit directions' cross product, each levered at the metering arm — the shortest edge of either cone at the touch (`Margin::levered`, the `sector_shape` convention) | m | OK (new in CONTACT-1) |
