...
| census.rs (CONTACT-1) | census_touch_side / census_touch_dihedral / census_touch_span / census_touch_piece | the touch analysis's cone signs, each a dot, cross-product norm or sine of unit directions levered by `Margin::levered` at the arm of what it measures: a boundary ray at its OWN edge's length (a sector bisector at its sector's arm, the `sector_shape` arm) — so a ray reads on a plane only when its far point does; a face normal at its piece's arm (the shorter of its two bounds); a dihedral at the far bound's own arm; a candidate built from two rays at the shorter of theirs; and whether a candidate is a half-space's face plane at the LONGEST arm of the pair | m | OK (new in CONTACT-1; per-ray levers since its dual-review fix pass) |
