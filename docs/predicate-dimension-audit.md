...
| census.rs (CONTACT-1) | census_touch_side / census_touch_dihedral / census_touch_span | dimensionless readings of unit directions (a ray's side of a candidate plane, a face normal's alignment, a dihedral, a tilt), each levered by `Margin::levered` under the ONE rule stated at `census.rs`'s `touch_lever`: at the lever of the face the reading stands for — the distance from the touch point to that face's farthest boundary vertex | m | OK (new in CONTACT-1) |
