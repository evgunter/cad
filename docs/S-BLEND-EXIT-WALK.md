...
| 10 | "issue 987 (ruled-spine carve) is double-gated … It schedules only after a design conversation AND a named consumer; neither exists today" | CARRIED (unchanged) | Neither exists at exit either. The gate held: no unit touched it, and the OQ6 run-out taxonomy stays reserved for Evan per A3-3's parked pair. |
...
- **Block accounting — one question for Evan**: BLEND-B1 and B2
...
- **Dispatch discipline, calibrated**: the program's one
  ...
  #1323 (Evan's pointer), and the needle rule propagated into
...
- **Open with Evan at exit** (none block ratification; all have
