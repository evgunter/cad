# M8 log — kernel residuals

Narrative record; the plan is docs/M8-PLAN.md (ratified structure:
PR #300 comment 5234714306). Convention as ever: seam entries at
pipeline seams, unit entries at merges, the tail is the live state.

## Opening state (2026-08-09, at ratification)

M8 opened with two units already in flight (dispatched under the
pre-renumbering demo-hardening directive, recorded as M8-1/M8-2 at
their merges): **M8-1** (#301, Newell chart-frame re-anchor) in its
fix pass after R1 APPROVE-WITH-FIXES (2 MAJ: in-CI anchor coverage;
doc-claim narrowing); **M8-2** (rational span-meter) implementing on
lane span-meter. M8-3/M8-4 queue next (specs from the #288-era
substrate). A/B block M7-12 consumed; the next block opens as M8-13
(running index preserved). Dual-review shared counter: last fixed
ordinal 14 (#301's R1, single).

## M8-1 MERGED (#301, 2026-08-10)

The boundary-anchored Newell chart frame is live: both Adafruit wild
files tessellate (204 / 12 triangles, volumes to 0.015%/0.000% of
exact), the wild montage holds 8 cells, import censuses unmoved. The
review earned both MAJORs honestly (the position-noise ~ν² class is
now a PINNED typed posture with the doc claim narrowed, and the
anchor judgment has an in-suite falsifier that reds under the
