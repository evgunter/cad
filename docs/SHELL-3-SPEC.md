...
**Status: DRAFT (SHELL orchestrator, 2026-09-04; re-read against main
2026-09-08)** — ratified at dispatch. Executes Ev's ruling B on `[ev]`
#1737 (`work/shell/shell-curved-clearance-consumer.md`). M10-7 (#1725)
has merged, so its `Sym<T>` seam is on main; what dispatch still waits
for is PROPS' sign-hull unit (branch `props/sign-hull`, in flight),
which retires the planar re-chart (`in_plane_axis` / `chart_frame` and
their two call sites, ~130 lines of `clearance.rs`) — a move of the
file and an edit inside it must not run concurrently, and the move
lands second (§6). Binds the implementer of unit `SHELL-3`; deleted at
merge per `docs/DOC-LEDGER.md`. Read
...
- **M10**: the file is M10's; ask its orchestrator to co-review
  (the joint unit). The `Sym<T>` seam of #1725 is on main.
- **PROPS**: its sign-hull unit (`props/sign-hull`) retires the
  planar re-chart — `in_plane_axis`, `chart_frame`, the
  `chart_axis` fields of `Window` and `GeometryWitness`, and the
  witness serialization's `chart=` columns. This unit dispatches
  after that merges and moves the file AS IT THEN IS; if the order
  inverts, the lane merges main before its first commit and reports
  the re-chart's state in the PR body rather than carrying either
  version by hand.
