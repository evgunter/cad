...
E2 amended per Evan — mandatory truncation replaced by **tail-mass
accounting**. Uncommented E-decisions stand, pending his careful pass.
...
delegation, W1–W9, the F1 lattice, recorded-ε/SetTolerance. This doc
pins what the MVP *is* — where distributions live, what a
sensitivity means, what the driver certifies, what is loudly
...
- **Analyzed box vs. support vs. measure**: the box the kernel
  ...
  *measure* never enters kernel evaluation — it prices leaves and
  tail at reporting time (E1/E6).
...
instantiation carries probability — there is no `Distribution`
scalar.
...
scalar channel does all the propagation; only pricing lives above.

- Why measures cannot ride the scalar channel: dependency makes
  distribution arithmetic **wrong, not loose**. Interval under
  dependency is sound-but-loose (`x−x` has width but contains truth
  — ⊇-monotone conservatism composes); pushing marginals through
  operators *forgets correlation*, with no conservative direction
  to hide in ("wider" is undefined without choosing an order).
...
- **The analyzed box is the analysis's knob, not the distribution's
  ...
  default mass, a recorded policy dial like K). The choice never
  affects a certificate's truth — it only moves mass between the
  analyzed and tail columns.
- **Tail-mass accounting**: mass outside the analyzed box is an
  ...
  single honesty gate (E10). Truncated support is optional sugar
  with `tail_mass ≡ 0`, a convenience, not a requirement.
...
only (F2 verbatim; sinks have none). Persisted as ordinary nodes;
...
- Rationale: measurements must be persisted, stable-named,
  ...
  dangle; Measure nodes fail loudly through N5 diagnosis.
- "Measurements only vs. any expression" dissolves: the sublanguage
  is total and finite by charter — any expression is Measure-
  wrappable, every Measure node dual/interval-evaluable. Same thing.
...
O(n·build) with trivial parallelism; both are pure-performance
additives later (E11.4).
...
- **`worst_case` is the headline and the only gating number**: the
  ...
  and silently wrong under curvature; the linearization survives
  only as the per-contributor *table*, labeled advisory.
...
- **`coverage`** keeps the report honest: "99.2% certified; 0.3%
  refused (reasons); 0.5% tail (unanalyzed)". `worst_case` is
  certified over the analyzed box; the tail term says exactly what
...
service on the `Real`-generic editor-core evaluation service
(banked principle; no parallel path).
...
- Every predicate definite → leaf **certified**; lineage-scoped key
  ...
  build it brackets; Measure enclosures are containment-true over
  the whole leaf.
...
(the display-layer argument in D4).
...
semantic unit stays the leaf — leaf results are self-contained, so
connectivity proofs would be machinery for zero certificate content.
...
- **When a leaf straddles a wall**, the certificate refuses and the
  ...
  - `Bifurcation(WitnessBifurcation)` — the box reaches across a
    ...
    branch. (`solver_branch_margin` samples from driver runs feed
...
- Rationale: refusing on tangent poison would let `abs` at a kink
  ...
  inverting the ratified hierarchy. A straddle hull containing zero
  IS information ("possibly non-monotone"), consumed as exactly
...
(D9 makes the key the correctness proof, same as everywhere), and
...
Assertion should gate `build()` itself — v1 says no, assertions
...
1. **No Monte Carlo lane.** Pure replay keeps MC additive forever;
   ...
   certificates, and shipping it first would set "probably fine"
   as the epistemic register. If ever, a labeled advisory lane
   beside the certified one.
...
3. **No distributions on structural (Count) parameters** — typed
   ...
   exploration, not tolerance analysis; structural variation stays
...
σ = 0.02 mm, unbounded); `Measure::Distance(hole1_wall, hole2_wall)`
(the web) and `Assertion { web ≥ 0.5 mm }` in the document. The
analysis takes the default ±3σ quantile box for d₁, d₂ — tail mass
1 − 0.9973² ≈ 0.54%, carried additively from here on.
...
- Driver (E6): the analyzed 3-box certifies in four leaves after
  ...
  Accounting: 99.06% certified, 0.4% refused, 0.54% tail.
...
- **SetTolerance × distributions**: bands are Lengths,
  ε-independent — but sliver-refusal mass depends on ε, so ε edits
