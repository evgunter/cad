...
## Sweep 6 — 2026-09-03: seventeen settled issues leave `work/issues/`

Sweep SHA: `f955ddc75cda454a268f9214d2a753ae1a9bbd0f` — the same tip
sweep 5 took, so every path below is recoverable at
`git show f955ddc75cda454a268f9214d2a753ae1a9bbd0f:work/issues/<NAME>.md`.

Sweep 5's rule — `work/` tracks work still to be done, not work that has
been done — applied to the unowned issues themselves. Each of the
seventeen below was re-checked against the tree rather than against its
own body: the claim was read at its cited `file:line`, and where the
issue named a reproduction or a row, that command was run. The
verification is the PR's record; what is kept here is which issue went
and what settled it.

Five were already `status: closed` on migration (spent bookkeeping);
the other twelve were `open` and are settled by landed work.

| issue | # | what settled it |
| --- | --- | --- |
| `asm-orchestrator-resume-handoff` | 430 | Spent handoff; superseded on-thread by #611, residue now S-MATE's slate |
| `blend5-r1-probes-owes-ledger-line` | 1327 | `reader_census` green — the file no longer reads Rust source |
| `display-budget-include-str-names-v15` | 1272 | `include_str!` names the unsuffixed `gallery_ring.pncad`; suite 3/3 |
| `emit-fillet-drops-upstream-n2-tie` | 708 | `names/defer.rs` — `up()` returns `Upstream { name, tied }`; `emit_chamfer.rs:42` records no remaining site |
| `interior-diagonal-intersection-pcurve-lane` | 498 | P-2 mints `Pcurve::General` for the interior column; the negative control is now a positive row |
| `interval-eps-1e-12-stale-arc-constant` | 1338 | Constant re-measured to `1.136…e-12` in both copies; the three rows pass at ε=1e-12 |
| `m8-4-interior-column-row-vacuous` | 1167 | Row flipped by P-2; ε-conditional attachment and posture tolerance both removed at the source |
| `m9-3-join-lane-resume-handoff` | 607 | Spent handoff; the join lane landed, and every pointer it named is gone |
| `no-editor-core-annulus-fillet-naming-row` | 1294 | `editor-core/tests/blend5_rim_support.rs` is the row, and says so in its header |
| `pcurve-claim-ordinal-201` | 1140 | Row landed at `MODEL-AB-LOG.md:1828`, contamination note carried |
| `pcurve-claim-ordinal-202` | 1209 | Row landed at `MODEL-AB-LOG.md:2134`, closing block PCURVE-1 |
| `pick-target-mesh-pairing-unverifiable` | 1098 | The cache holds `NodePick`s; the witness row records the revisit and the outcome |
| `profile-templates-spell-no-revolvable-silhouette` | 1457 | `ProfileShape::Path` landed with a `path` form (`41a752b`) |
| `reader-census-blind-in-dot-path-clone` | 1234 | Dot-skip scoped to the root-relative path (`e66db55`); verified in a dot-component checkout |
| `reader-census-selftest-vacuous-in-worktree` | 1274 | Same fix, same commit; the row is a set equality, so an empty walk reds |
| `sampled-eps-gates-hide-band-sensitive-reds` | 1356 | "ε posture (issue-1356)" is a standing spec item across MATE/MESH/BOOL; the four instance repairs landed |
| `two-orchestrators-dead-at-usage-walls` | 600 | Spent; both walls cleared on the 7-day rollover |

**One re-home, not a deletion.** `unit-vector-invariants-carried-as-prose`
(#457) moved to `work/code-quality/`. It is a prose-held-invariant
finding of the register's own class (`S93`'s precedent) that no row
cites; its migration note reasoned only over the territory-owning
programs, and code-quality claims no paths.

**Three in-tree comments corrected**, each left asserting a gap that a
retired issue tracked: `sweep/tests/blend_tworims.rs` (#1294's drive
exists now), `viewer/tests/story_authoring.rs` and
`viewer/tests/story_parametric.rs` (#1457's `ProfileShape` gap is
closed; both scenes predate the `Path` arm rather than lacking one).
