...
the rows marked ADVERSARIAL — D2, **D21** and **D30** still live, with D1
...
`link_half_edges`' precondition rather than restate it — and **D30** and
**D31** by **#744**, a second span search for knot lists the validated type
cannot represent, and a union-and-refine routine duplicated across two crates
on top of the query that unit gave a home. (D30/D31 took the next numbers the
orchestrator assigned, not the next gap: **D22**, **D23** and **D26–D29** were
assigned to concurrent lanes and land with them.) Every one of them was
...
**placed D21** and **D25**, and **D8 as #744**, which also edited `mesh/` and
`step-export/` — the `mesh/` half is **under Track C's C5**, whose two C¹
carrier gates are now four lines each — and **placed D30** and **D31**.
**Nothing is in flight.**
**D17, D19, D20, D21, D25, D30 and D31 are edge-free and unstarted** — though
...
collides with no other row here. **D30** is `geom-brep/props/quad.rs` alone and
**D31** is `sweep/skin.rs` + `geom/curves/fit.rs`, neither of which any other
live row names. **D18 landed as #736**, taking W2c with it.
...
fillet-helper row**. **D2 is now the single widest unblocked row**, and nothing
outside Track D gates it.
