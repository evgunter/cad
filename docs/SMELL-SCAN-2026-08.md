...
wrong**. `choose_op` is 2.7% of the lane (451 ms of a 16.9 s deterministic
replay of 9324 generator steps); the whole of it cannot hold a 46% regression.
The cost is in what a split *does* — certification, the inverse, the oracle's
deep compare, and every later step running against a body splits made bigger.
Two of D14's premises also failed on contact: only 2.0% of steps reach the
shrink phase at all (cases are 1–47 steps from an empty body against
`GROW_CAP = 28`), so skipping zero-weight rows — the fix as specified —
measured 452 ms against 451 ms, nothing. #722 landed lazy enumeration anyway
(27% off `choose_op`, generated sequence byte-identical over 305 replayed
decision streams) and recorded that the suite median did not move.
...
enumeration as **D14**, both **retired by #722** (the convention is now checked
by a walk of the mutation surface; the enumeration is lazy, and the
measurement that closed it refuted the row's own account of where D5's +46%
went — see S15). Its
...
| **U6** — S15's prose-held invariants | **D5**, landed as **#713**; of the three rows it could not close, **#708** (tie propagation) stands as an issue and **D13** (the pcurve convention) and **D14** (the fuzz lane's eager enumeration) were retired by **#722** |
...
PR.** D8, D10, D11, D12, D15 and D16 are edge-free and unstarted (D1
landed as #710, D3 as #704, D4 as #707, D5 as #713, D6 as #706, D9 as #712,
and D13/D14 as #722 — #707 was D13's gate). D8 edits `sweep/src/skin.rs`, so it
