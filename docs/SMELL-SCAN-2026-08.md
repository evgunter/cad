...
**No row number is reserved; D54 is the highest one placed.** Placements:
...
prose-held at 102 sites the week the mechanism was ratified. Every one of them
...
PR's own review; **D54** came out of **#767** (D28) — that unit closed the arms
whose kernel payload had a `Display` and could not close the five whose payload
is an editor-core type that has none. Row numbers are assigned centrally because several lanes mint rows in
...
| **E-d** | **D33** | `docs/predicate-dimension-audit.md` | style; **closes on a coverage number plus a scope verdict** | Wave 1, edge-free. Do not widen the audit to the workspace before deciding whether its two-crate bound is deliberate. |
| ~~**E-e**~~ | ~~**D28**~~, and issue ~~**#693**~~ | ~~`editor-core/src/eval/`~~ | ~~style~~ | **Landed as #767.** Twelve payload-discarding arms in one `Display`, not the row's eight, plus a thirteenth in `program.rs`; `TransformError` had no `Display` to forward and `topo` mints one. The C-f note above was **wrong**: #731 does touch `eval/mod.rs` and `eval/anchor.rs` — disjoint by item, not by file. Five arms whose payload is an editor-core type with no `Display` are placed as **D54**. #693 discharged. |
...
| **D28** | **FIXED by #767.** The re-derived census is **twelve** payload-discarding arms in that one `Display`, not the row's eight: the eight op arms plus `Profile`, `ProfileReplay`, `Band` and `Escalated`, all of which forward now, and two more of the same class elsewhere in the crate, found by a shape sweep rather than a symbol one: `program.rs`'s `ProgramRefusal::Validate`, and `edit.rs`'s `EditError::MetaUnversioned`, whose dropped `MetaVersionError` made the message **wrong** rather than merely thin — two of that payload's three arms are not a missing `"v"` field, and one of them is exercised by a test that only matched the variant. **Two payload types had no `Display` at all** — `topo::transform::TransformError` and `MetaVersionError`, both minted here; `TransformError` — the eighth op arm had nothing to forward, so `topo` mints one, and the type is public through `pncad::prelude`. Five further arms hold a payload and still render prose, each because the payload is editor-core's own type with no `Display` (`EvalError`, `crate::resolve::ResolveError` twice, `WitnessBifurcation`) or because the arm owns a recourse menu the payload does not carry (`UndeclaredContact`, F6, pinned by test in both languages) — placed as **D54**. The row's *"the payload is already gone by the time the tag map runs"* is **wrong and was already retracted by D37**: `node_error_tag` takes `&NodeErrorKind` intact and the render is a separate call at `py/value.rs`, so the loss was the **message**, never the tag. Guarded by `a_kernel_payload_arm_forwards_the_payloads_own_message` (representative, not exhaustive — nothing makes it exhaustive) and by one assertion on the live fillet refusal in `m5_pr12_fillet_node.rs`. **Issue #693 discharged in the same PR.** | #740 (D2), via its FFI-boundary check | `editor-core/src/eval/mod.rs` | style | nothing |
...
| **D54** | **D28's class, one step in: five refusal arms in `editor-core` still drop a payload, and the blocker is three missing `Display` impls on the crate's OWN types.** #767 forwarded every arm whose payload had a `Display`; these five had none to forward. `NodeErrorKind::Expr` and `ProgramRefusal::Resolve` hold `expr::EvalError`; `NodeErrorKind::DeclareResolve` and `::FilletSelectionResolve` hold `crate::resolve::ResolveError`; `NodeErrorKind::WitnessBifurcation` holds `witness::WitnessBifurcation`. Each renders a class and drops the instance — *which* of N5's three rungs refused and about what name, *what* the expression evaluator objected to. Not a design question: `NamingError` was the identical shape and #380 minted its `Display`, after which `eval/mod.rs`'s `Naming` arm forwards it and `NAMING-DESIGN.md` records that its absence was what made Python see one opaque refusal. Two qualifications a taker owes. **`WitnessBifurcation` is never constructed before the M6 solver**, so its arm is a future loss, not a live one, and leaving it may be the right answer — say which. **`UndeclaredContact`'s dropped `diag` is deliberately out of scope**: that arm owns a two-armed recourse menu (F6) whose prose is pinned by test in both languages, and the menu is what the caller needs, not the diagnostic. | #767 (D28) | `editor-core/src/{expr.rs,resolve/mod.rs,witness.rs}`, then the two `Display`s in `eval/mod.rs` and `program.rs` | style | **C-f (#731)** owns `resolve/mod.rs` — sequence after it |
