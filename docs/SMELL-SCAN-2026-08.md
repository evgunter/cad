...
**No row number is reserved, and the highest number placed is NO LONGER THE ALLOCATOR.** Numbers are handed out in **per-track blocks** — Track E holds `D81`–`D100`, Track F `D61`–`D70` — because *"the next unassigned number"* was never a global fact: four orchestrators read `main`, each correctly derived the next free number, and each derived the same one. A sequence with one allocator per branch is not a sequence, and nothing was stale — four correct derivations of one quantity disagreed because the quantity was not global. **This is D23's shape with the roles reversed**: there, a set was read off names instead of contents; here, a maximum was computed off one branch instead of the register. Take your block's next number from your orchestrator, never the next gap visible on `main`. Placements:
