...
and `DESIGN.md` records the general rule — *ask whether the type can stop
representing the state before asking the taxonomy to grow*. The pin behind the
...
deleted, and `DESIGN.md`'s D2 addendum records the general rule instead of a new
row — *ask whether the type can stop representing the state before asking the
taxonomy to grow*. That is an edit to a ratified contract and **#768 does not
merge on its own authority**. D29 is `Op { site, source: EulerOpError }` and
