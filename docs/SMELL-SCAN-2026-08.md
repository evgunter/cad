...
C7, C9–C11, C13, C14 and C16 are all edge-free and takeable today — with the caveat that **C13 and C14 want a plan signed off by Evan before implementation**, being cross-crate public-API changes with a design element rather than cleanups. **The numbering has two holes and one displacement, all of them live rather than errors:** **C12** arrives with #731, **C15** is C-p's per-face slack gate hole (#738, issue #746), and **C16** is the row C-R20 assigned as C11 before #714's fix pass landed a C11 of its own — `main`'s C11 stands and C16 is confirmed. Row numbers on this track are assigned by the orchestrator and **a number is not owned until it is on `main`**, which is why the displacement is recorded here rather than resolved by renumbering the table.
