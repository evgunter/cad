...
| ~~**E-k**~~ | ~~**D35**~~ | ~~`docs/DESIGN.md`'s D2 addendum, and whatever the answer names~~ | ~~style; **closes on a decision plus its reason**~~ | **DONE — #NNN. The answer is (d), no gate**, and the reason is in the D2 addendum where a reader of row 4 will meet it. Population re-derived at `25175838`: **103** kernel call sites, of which **76 are one state** whose row 0 is already answered *no*. **Three** defective messages, all fixed here. **Two things this lane found rather than decided:** #777 (row 0) had never reached `main` — it merged into `smelle/d27-d29` **19 seconds after** #768 merged that branch, so the retarget never fired and `main` carried D21's *"1 row 0"* against a table with no row 0 in it; recovered here byte-for-byte. And the residue is **row-0 work, not message work** — 13 candidate sites, enumerated in the PR body, **awaiting a row number from the orchestrator** (a verdict is not a placement, and this lane does not mint one). |
