...
| **C20** | **The demo sweeps are the only `sweep_body` bodies left with no orientation coverage — and they are the ones furthest from anything pinned.** #779's class sweep (pattern `sweep_body::<` / `loft_body::<` / `sweep_body(` / `loft_body(` over `crates/`, `demos/`, `tools/`; 66 hits; blind spot: a caller reached through a macro-generated path or a re-export under another name, or a body hand-assembled from euler ops) found that **every turning-path `sweep_body` caller in the tree builds one of four fixtures**, and three are now pinned: the **square elbow** — six callers (`sweep/tests/m5_s11_concave_sense.rs`, `m7_skin_integral.rs` ×2, `mesh/tests/probe_review.rs`, `mesh/tests/m7_nurbs_trimmed.rs`, `step-export/tests/common/mod.rs`), all the same body *constant for constant*, so #636's one row covers all six; the **rational circle-section elbow** (`m7_skin_integral.rs:378`) and the **helices at ½, 1 and 2 turns** (`m8_14_long_turn_sweep.rs`), both closed by #779. The fourth is `demos/tour/src/{lily,skinned}.rs` — **`lily.rs`'s arched leaf spine, `skinned.rs`'s S path and its twisted cubic**, three bodies that build and that nothing anywhere asserts face outward. (**`klein.rs`'s U-turn is NOT one of them**, and a taker should not go looking for a body there: that call is a documented refusal wall, `LoftError::ReversedStacking`, because the loft's stacking trilean compares only the LAST placement against the FIRST section's plane normal. The extreme case is closed by refusal rather than by a bit.) **This inverts the usual coverage-gap argument.** A gap normally sits at the easy end, on a case nobody bothered to pin. This one sits at the hard end: every pinned fixture has **constant curvature and a monotone frame roll** — a quarter-turn planar arc, or a helix — and a **centrally symmetric** section, a centred square or a circle. All three demo bodies break one of those. `skinned.rs`'s S path is two OPPOSED arcs, so its curvature reverses sign and the minimal-rotation frame's turn is not monotone through the inflection; its twisted cubic `(At, Bt², Ct³)` varies both curvature and torsion, where a helix holds both fixed; and `lily.rs`'s kite section is **not centrally symmetric** (`keel ≠ ridge`), which no pinned fixture is. So the class is closed where the argument is easiest to make and open where the bit is most likely to be wrong. **Disposition, so it is not re-decided**: #779 declined it on scope — the lane's file set was `sweep/tests/`, these are `demos/`, and a demo is evidence rather than a pin, so the fix is a **coverage row** (probe the demo-built bodies' wall orientation from a test) and NOT an edit to the demo scenes, which must keep reading as a user would write them. `common::orient::LevelIndex` is the oracle it needs and already exists in `sweep/tests/common/`; the taker's one real question is where a probe over demo-built bodies lives, given that `demos/` is not a test crate. **NOT part of this row, and still unassigned**: the corpus probes no CAP face's orientation at all — `along_v()` stops at `v ∈ [0.05, 0.95]` and the probe walks `side_faces` only. That is #636's own open question to its reviewer, inherited verbatim by #779 and held out loud in its §6 rather than parked here. | **C-R19 tier one — a lane takes it.** No design fork: the oracle exists, the bodies exist, and what is missing is rows. The one judgement in it is where they live, which is a placement call, not a design element. Not gated on anything. |
