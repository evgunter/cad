...
"outside v1's admitted `Rest`/`Tangent`". The two instances outside
`editor-core` went to **#711** (§D row C8, Track E's E-m) rather than staying
inside a finding marked FIXED, and are **FIXED by #784** — where the
recognizer half turned out to be a *false* unreachability claim rather than a
true one, and the spec half had been answered by a document deletion nobody
had connected to it.
...
| ~~**C8**~~ | **FIXED by #784** — issue #711, taken by Track E as **E-m** and closed there. | **Both items were false as filed, in opposite directions.** The recognizer arm is not unreachable and the preference order is not unfalsifiable: the certificate is a ratio, and one patch read at two budgets exercises each arm (rows **P9**, **P10**). The spec sentence has no document — `docs/ASM-R2A-SPEC.md` was deleted in `12732c5c` and is recorded in `docs/DOC-LEDGER.md`, whose landed rule is stronger than either candidate ruling the row offered. |
...
| ~~**E-m**~~ | ~~**#711**~~ | `step-import/src/recognize.rs` | style | **DONE — #784.** Neither an encoding change nor the tighter certificate: the *claim* was false. Stubbing `try_cylinder`'s promoting arm to `Ok(None)` and inverting the preference order both survived the whole file before this unit; **P9** and **P10** turn each red, off one exact full-period cylinder read at two budgets. The second item's document does not exist — see the struck **C8** row. Residues: **D86** (the unit's CI red — `scripts/`, so Track F's, and BLOCKING), **D87** and **D93**. |
...
| **D87** | **A deleted document can void a live row's deliverable, and the ledger's own coverage sentence does not reach that case.** `docs/DOC-LEDGER.md:59` closes its inbound-references note with *"No file was deleted that a **live** pointer depends on for its content."* Read as written it is about **content**-dependence, and for that it holds: the citations it has in mind are *"see also"* pointers in append-only logs, and the ledger's recovery recipe resolves every one of them. What it does not reach is a live pointer that depends on a deleted file as a **target**. The measured instance is this register's own **C8**: a scheduled row, live at the moment of Sweep 1 and for every day since, one of whose two candidate deliverables was *"a dated clarifier in the spec"* — an edit to line 21 of `ASM-R2A-SPEC.md`, which that sweep deleted. C8's *content* was self-contained (it quotes the sentence), so nothing about the row became unreadable; **half its option set became unperformable**, and no reader of either document noticed for as long as the row went unstaffed. The row closes on **one sentence, chosen**: either the coverage claim states the content/target distinction explicitly, or the sweep procedure gains a check that no scheduled work item names a path it is about to delete. The second is the stronger answer and is not free — it needs a pass over the live registers, which is why this is a choice and not a patch. Note the asymmetry that makes it worth writing down: a deletion sweep can **see** its own inbound citations by grepping for the filename, and C8 cites `docs/ASM-R2A-SPEC.md:21` verbatim — so this instance was greppable from inside the sweep, and the grep was for the wrong property. | #784 (E-m / #711), from re-deriving the row's own second item and finding its target absent | `docs/DOC-LEDGER.md` | style, and it **closes on a chosen sentence** — *"the claim is right as scoped, and here is the scope stated"* is a passing answer | nothing |
| **D93** | **Two more S24-shape unreachability claims in `step-import`, neither checked by anything and neither re-derived.** `crates/step-import/src/entities.rs:2403` — *"this check cannot fire on a rotation by construction"* — and `:1389` — *"ONE-bound face is not reachable — the list has one entry."* Both are the shape S24 names: a boundary asserted in prose, in a place the compiler and the suite are both silent about. **They are reported as UNVERIFIED and the row must re-derive them, not clarify them.** The reason to expect that is #784's item (1), one file over in the same crate: the claim there — *"the promoting arm is unreachable"*, *"unfalsifiable by execution"*, *"no authorable patch double-certifies"* — was **false in all three parts**, and it had been believed through four documents because its argument silently fixed one side of a ratio. A prose unreachability claim in this crate has a measured history of being an assertion rather than a fact, so *"restate it more clearly"* is the one outcome this row should not produce. Each closes either as an executed pin (the arm fires under some input, here is the row) or as a stated condition that makes it unreachable (and what would make it reachable), which is the standard #784 set. | #784 (E-m / #711)'s sweep of the crate for the S24 shape; both hits reported rather than fixed, per that lane's scope | `crates/step-import/src/entities.rs` | style | nothing — but **read D86 first**: until it lands, a change closure of `step-import` alone is red on four `test (interval, …)` legs before anything compiles, for a reason in `scripts/`, not in the crate |
