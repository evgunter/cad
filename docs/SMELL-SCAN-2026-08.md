...
`link_half_edges`' precondition rather than restate it. Every one of them was
placed by a lane on its way out rather than left in a PR body — D15's as
**D17** (the 14 `probe` test suites no CI lane type-checks), D16's as **D18**
(`link_half_edges`' two unproven callers, which were W2c's last two sites),
...
D7's `PairSolve` row's as **D24**, and D18's as **D21** and **D25**. Row
...
not a placement** (§D's fourth ordering rule) is why all seven are rows here
rather than sentences in seven PR bodies.
...
**D17, D20, D21, D25, D32, D33 and D34 are edge-free and unstarted** — though D21
...
lane and can run at any time; **D19 landed as #747**, and it named no enforcement shape: the roster is a
...
collides with no other row here. **D18 landed as #736**, taking W2c with it. **D32, D33 and D34 are #747's own
...
question and now has D19's criterion to answer it with, and D34 is
`gate-roster.sh`'s scope — 23 scripts invoked by `ci.yml` against the 13 its
directory holds. None is gated on anything and none gates anything.
