...
| ~~**E-h**~~ | ~~**D21**~~ | ~~`topo/src/{split,attach,movefac,revert}.rs`, `splitting/finish.rs`, `boolean/combine.rs`~~ | ~~**ADVERSARIAL**~~ | **Landed as #773.** 17 sites, all row 4, under a stated reading — the row's 14 plus `movefac.rs`'s accumulator lookup and **a seventh file the floor never named**, `merge_faces.rs`. Every arm proved live by poisoning its key. One site could not meet #720's standard and stays row 1, placed as **D61**. |
...
| **D21** | **FIXED by #773.** The re-derived census outside W2c's three modules is **17**, and the reading is stated because three of them exist: *a lookup whose `None` is silently discarded at a write in a mutation phase*, over `crates/topo/src` production code (the `#[cfg(test)] mod` declarations in `lib.rs` are the src/test cut, not an in-file marker), all spellings. That is the row's own 14 plus `movefac.rs:143`'s accumulator lookup and **two sites in a seventh file the floor never named** — `merge_faces.rs`, where `normalize_merged_roles` spells the discard `else { return Ok(()) }` under a comment already reading *unreachable*. The sweep pattern is *every `.get*(`/`get_*_mut(` call bound by `if let Some` / `while let Some` / a `&&`-continued let-chain / `let … else`, classified by what its `None` arm does*, plus a second pass for `.unwrap_or_default()` / `.unwrap_or()` / `.map_or` / `.is_some_and` over the same doors. **What it cannot match**: a lookup behind a helper that returns `Option` under a name containing no `get` (`mate`, `loop_cycle`, `half_edge_end` were added by hand, anything else was not); a discard split across a `let` binding and a later `match`; `Index`-style panicking access, which is a different defect; and anything outside `crates/topo/src`. **Three judgement calls, re-made rather than inherited.** (a) `validate.rs:3006`'s exclusion **stands but not for the row's reason** — at `#736`'s head that line is inside `shell_of`, a closure that *propagates* `None` out with `?` and whose caller's `continue` is documented as deferring to passes 1 and 7; and the site the row's "local accumulator" phrase actually fits, now `validate.rs:3039`, is an entry-or-insert whose `None` arm inserts. Neither is a discard. (b) The accumulator exclusion is **narrowed**: excluded where the `None` is genuinely possible (`combine.rs`'s sparse provenance maps, `finish.rs:559`'s `Above`/`Below` filter), included where the map is filled in the same call and the `None` cannot occur (`movefac.rs:143`). (c) The read-side residue the 11-reading leaves uncounted is **not zero and not separable**: `split.rs`'s `get_vertex(v)` read feeds the let-chain write two lines down, so converting only the write would have *relocated* the discard into the short-circuit. Both are one lookup now. **Per-site discharge**: `split_edge`'s `edge` and `v` are plan-phase-proven and `w` is minted in-call; `attach.rs`'s two are plan-phase-proven; `movefac.rs`'s `shell`/`solid` are plan-phase-proven, its `face` and `label` come from the labelling walk that resolved every member it inserted; `revert.rs`'s three iterate `self` and write into `self.clone()`; `combine.rs`'s two and `finish.rs`'s one are keyed by the map that minted them in-call; `merge_faces.rs`'s two are proven by `merge_group`'s curved-survivor gate, which returns `StaleKey` on the same key. **No site rests on tier-1 validity, and every arm was proved live**: each of the 17 keys was poisoned in turn and the suite watched for its own message — all 17 fired and none was masked into being dead code, and a probe pass first confirmed all 17 sites execute (648–16 951 times each) on the green suite. **One site could not meet the standard and was deliberately left row 1**: `merge_faces.rs:766` reads its face key out of a loop's back-pointer, which no check in the call proves, and silently defaults the ring list — placed as **D61**. **The per-arena token was considered and rejected**: what made `Live` pay was one helper with 44 call sites where the proof rides the *signature*; here every write is an inline field assignment across seven arenas with no shared consumer, so a token would have nowhere to be consumed and would have to become an infallible `get_*_mut` door instead — a different mechanism, and one none of these sites needs, since each already holds its proof in the same call. New instrument: `topo/src/review_d21_probes.rs`, five rows pinning that the doors refuse a stale key **typed** and leave the body deep-equal — `attach.rs` has **no test module of its own and no stale-key row anywhere in the workspace** — its existing error-path coverage (`crates/topo/tests/`, `crates/sweep/tests/`) is on the *later* plan-phase gates (`DescriptionNotAdjacent`, `Certification`), all of it on `set_edge_curve`, none on `set_face_surface` — and `movefac.rs`'s only stale-key row plants a shell the component walk never reaches. | raised by D18 (#736), whose sweep crossed the census boundary; the count and the three missed spellings are #736's style review | `topo/src/{split,attach,movefac,revert}.rs`, `topo/src/splitting/finish.rs`, `topo/src/boolean/combine.rs` | **ADVERSARIAL** for the same reason D18 was — each conversion turns a garbage-out into a panic, and the sites that look easiest are the ones a glance is most likely to be wrong about | **D25**, landed as **#755** — sequence after it on file overlap in `split_edge` |
...
| **D61** | **A discard that cannot meet #720's standard, and therefore wants a typed error rather than an `unreachable!`.** `merge_faces.rs:766` re-homes a dying face's rings with `self.get_face(other).map(…rings…).unwrap_or_default()` — inside `absorb`'s mutation loop, so a `None` silently drops every ring of the absorbed face and the merge returns `Ok` on a body that has lost them. `other` comes from `edge_faces`, which returns a **loop's `face` back-pointer** without resolving it in the face arena; nothing in the call proves it, and the only thing that would is tier-1 validity — which is exactly what #720's standard forbids leaning on. So this is not a conversion: it is D9 row 1, and the shape is already in the same function (`MergeCoplanarError::Op { error: EulerOpError::StaleKey { … } }` at the curved-survivor gate). **The behaviour change is the deliverable and it is not free** — today a tier-1-invalid body silently loses rings; afterwards the unlicensed path refuses (safely: `merge_coplanar_faces` stages on `self.clone()` and commits only at the end, so the source is untouched) and the declared-licensed path turns it into a `SkippedMerge` string. Decide whether that second half is the right disposition, or whether the declared path wants the refusal to propagate. **`.unwrap_or_default()` over an arena lookup is its own spelling of the idiom** and D21's sweep found exactly one instance of it in `crates/topo/src` production code — worth a second look outside that crate. | D21 (#773), which converted the 17 sites that **do** meet the standard and deliberately left this one | `topo/src/merge_faces.rs` | style — the conversion question is settled, the open one is which of the two paths refuses | nothing |
