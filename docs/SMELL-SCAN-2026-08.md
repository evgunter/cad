...
C7, C9–C11, C13, C14 and C16 are all edge-free and takeable today — with the caveat that **C13 and C14 want a plan signed off by Evan before implementation**, being cross-crate public-API changes with a design element rather than cleanups. **The numbering has two holes and one displacement, all of them live rather than errors:** **C12** arrives with #731, **C15** is reserved for C-p's per-face slack gate hole, and **C16** is the row C-R20 assigned as C11 before #714's fix pass landed a C11 of its own. Row numbers on this track are assigned by the orchestrator, not taken from the next visible gap — which is why the displacement is recorded here rather than resolved by renumbering the table.
...
**D20, D21, D22, D23, D25, D26, D28, D30, D31, D32, D33, D34 and D35 are
edge-free and unstarted** — though
