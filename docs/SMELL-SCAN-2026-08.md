...
**FIXED by #643 and #NNN.** The lead was filed *unsure*; it is confirmed by
...
1. **The class was wider than three. All eight crossings are now
   ...
   any. **#NNN pushed one through all five.** Every one of them
...
3. **Three disciplines for that one hazard, none of them linted**:
   ...
   spelling (`RingInterval::from_certified`, #NNN), but the clamping
   half does not.
...
| **W2b** | **S1 / S2** — `RingInterval`, and whether `Interval` becomes always-on. The inherited five-crossing sub-unit is **DISCHARGED by #NNN** and no longer part of this row. | **D1**, **W1c** | Blast radius is **535 refs in 15 files**, not the ~600 sites this report first claimed; five files carry 60%. Build cost measured at ~zero. The decoration seam is **partly** cleared: #643 split `Bounds` from `CertifiedEnclosure` and converted the three crossings S41 named, so `Bounds::lo`/`hi` is now reliably a *bracket* at all 535 refs. **#NNN closed the remaining five**, so `Bounds::lo`/`hi` is a bracket at every one of the 535 refs and no crossing in the workspace reaches the ring through it. It also retired this row's sizing estimate: the conversion needed **no** caller widening at all — zero ripple, `--workspace --all-targets --features interval` unchanged — because each crossing's lane is already statically split at a concrete scalar. |
