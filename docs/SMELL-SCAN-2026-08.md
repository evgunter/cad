...
| ~~**E-f**~~ | ~~**D25**~~ | ~~`topo/src/euler.rs` and every `link_half_edges` caller~~ | ~~**ADVERSARIAL**~~ | **Landed as #755.** 44 call sites, all type-checked; the seven liveness blocks are one constructor. **E-h inherits nothing from it** — D21 has no half-edge splice sites; see that row. |
...
| **E-h** | **D21** | `topo/src/{split,attach,movefac,revert}.rs`, `splitting/finish.rs`, `boolean/combine.rs` | **ADVERSARIAL** | After **E-f**, which landed as **#755** — re-derive the census against its head. The deliverable is a sweep with a stated blind spot, not a count — three spellings escaped the last one. |
...
```
E-f (D25, Live key)  ──✗  E-h (D21) — the edge is CUT: #755 landed and D21 has
                          no half-edge splice sites, so it inherits nothing
E-g (D27, D29)       ──┐
...
D21 was to sequence after **D25** for a structural discharge of its half-edge
sites; #755 landed and it has none, so that edge is cut and D21 is edge-free
too. **D27** and **D29** sequence against
