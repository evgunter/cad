...
**The placement audit is FIXED by #NNN (D26): the four rows that had no row
anywhere are now placed as D36–D39, and one of them is partly refuted.** Each
was re-derived against the tree rather than transcribed, and each disposition
is recorded on its own row in the table below. What #NNN closed is the
...
- **`StepImportError` is no longer *"out-of-fence for a Display impl"***
  ...
  clause was two live sites by the time it was audited, and **#NNN fixed both**
...
- **`tags.rs`'s founding premise had expired.** Its module doc justified the
  ...
  and `py/doc.rs:22` says so in the same crate. **#NNN corrected it.**
...
**No row number is reserved; D39 is the highest one placed.** Placements:
...
visible — D26 in turn placed **D36–D39** by **#NNN**, one per member, after
refuting one of the four as stated. Row numbers are assigned centrally because several lanes mint rows in
...
The first wave was seven lanes that share no file (**E-c has landed**, as
#NNN). The gates are not dependency gates — they are file-overlap and
...
**D36–D39 are placed and unstaffed.** They are E-c's discharge of **D26** —
one row per member of S19's four unplaced rows, with `pncad-py/tags.rs`
refuted as stated and re-derived to its residue. Placing them is what closed
D26; **staffing them is the orchestrator's**, and until it happens they are
exactly the *"a deferral must land in a register that executes"* condition
(§C3) — a register entry, not a lane. Two of the four share one mechanism
(**D37**(a) and **D39**: a `PathError` discriminant that lives in `profile`),
so a single lane taking both is cheaper than two.
