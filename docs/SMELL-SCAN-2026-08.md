...
| **E-k** | **D35** | `docs/DESIGN.md`'s D2 addendum, and whatever the answer names | style; **closes on a decision plus its reason** — *"no gate"* is a passing answer | After ~~**E-g**~~ and **E-h**. **E-g (#768) is in and moved the population by one**: `sweep`'s `unreachable!` total is unchanged at 23 tokens, one site moved from `surgery.rs` to the new `admit.rs`, one prose mention went with `corner_convexity`, and one new site is `battery.rs`'s chain walk. D27 minted **no** row-4 site for its own three helpers — a fact carried in a value leaves no branch to convert — which is itself an input to this decision. |
