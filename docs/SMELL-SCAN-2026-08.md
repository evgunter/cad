...
C7 and C9–C14 are all edge-free and takeable today — with the caveat that **C13 and C14 want a plan signed off by Evan before implementation**, being cross-crate public-API changes with a design element rather than cleanups. (**C12 arrives with #731**; row numbers on this track are assigned by the orchestrator, not taken from the next visible gap.)
