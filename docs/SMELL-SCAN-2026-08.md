...
| **D25** | **FIXED by #755.** `Live` is a half-edge key that a lookup has returned, minted only by `Live::certify` — one constructor, private field, own module — with `link_half_edges(a: Live, b: Live)`. The seven per-caller liveness blocks are gone; the proof now arrives through `Body::certify_half_edge` (the plan-phase door, typed `StaleKey`), `mint_halves` (which returns its two halves certified) or `Body::loop_cycle_live` (the walk, which resolved every member it returns). All **44** call sites are type-checked. The `&mut self` question the row flagged is answered in the PR body and in `topo/src/live.rs`'s module docs: the token is a statement about the moment it was made, the residue is *do not remove half-edges between certifying and splicing*, and the arenas being slotmaps means a token that outlived its key fails the lookup and announces rather than corrupting. | raised by #736's style review, via the seven-block enumeration and the licensed inference in the helper's own replacement comment | `topo/src/euler.rs` and every `link_half_edges` caller | **ADVERSARIAL** — it changes a signature threaded through every Euler operator's mutation phase, and a `Live` that can outlive the mutation that kills its key would be worse than the prose it replaces | nothing (but D21 should follow it) |
