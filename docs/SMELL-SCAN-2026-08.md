...
| **D21** | **W2c's census stopped at three modules, and the discard idiom has more sites in `crates/topo` that nothing has ever counted.** The D2 addendum's *"silent discard is never an answer"* was executed across `euler.rs` / `euler_ring.rs` / `euler_kill.rs` (58 sites, #720 + #736) because those are the modules S12's census walked. A grep of the same idiom over the rest of `crates/topo/src` returns **at most 11**: three in `split_edge` itself (`get_half_edge_mut(hm)`, `get_edge_mut(edge)`, `get_vertex_mut(w)` — all three inside the mutation phase whose own comment says *"infallible from here on"*), two in `attach.rs`, three in `movefac.rs`, three in `revert.rs`. **11 is a ceiling, not a census.** It is a grep total that no one has re-derived per site, and it is already known to be loose: `movefac.rs`'s `component.get(face)` is a lookup into a local `SecondaryMap<FaceKey, usize>` rather than into the arena, so it is probably an ordinary `Option` match and not idiom 4 at all — the same correction that took #720's count from ~60 to 58. **Re-derive before working from it**; a number a taker has to redo should not look like one they can inherit. **The three in `split_edge` are the cheap ones and the ones with a proof already written**: `hm` and `edge` are resolved by that operator's plan phase and `w` is minted three lines above, and #736 left them only because its row was scoped to W2c's census. **The part a future taker should not inherit from this row**: at a glance every one of them looks trivially provable — `attach.rs`'s two and `movefac.rs`'s `shell`/`solid` are resolved by their own plan phases and nothing kills them; `movefac.rs`'s `face` comes from a list whose members the labelling loop above `contains_key`s; `revert.rs`'s three iterate the very arena its `out` is a `clone()` of, so every key is live by construction. **That glance is exactly the posture that produced D18**, where the qualifier *"cannot fail on the operator paths"* was not narrow but wrong, so the row is a per-site discharge and not a rubber stamp. **One instrument does not carry over.** **`attach.rs` guards with a `debug_assert`ed full `validate(self)` rather than `assert_euler_postcondition`** (`movefac.rs` uses the postcondition; the two are not the same instrument), so what a mis-conversion trips there is not what it trips in the Euler modules — which matters, because `debug_postcondition_fires_on_corrupt_input` is the row that separates a postcondition panic from a mis-converted `unreachable!` and it covers neither of these files. **What this row is not**: a licence to convert on the module's reputation. #720's standard is per-site — minted in the same call or proven live by a check in the same call, never by tier-1 validity — and it was falsified across roughly half of #720's own sites, so a site that cannot meet it wants a plan-phase check first (as D18's two did) or a typed error, not an `unreachable!`. | raised by D18 (#736), whose sweep crossed the census boundary | `topo/src/{split,attach,movefac,revert}.rs` | **ADVERSARIAL** for the same reason D18 was — each conversion turns a garbage-out into a panic, and the sites that look easiest are the ones a glance is most likely to be wrong about | nothing |
...
discard idiom's uncounted sites in `crates/topo` outside W2c's three-module
