...
**No row number is reserved; D51 is the highest one placed.** Placements:
...
prose-held at 102 sites the week the mechanism was ratified — and **D46** and
**D51** by **#761** (D33): the 23 funnel-reaching names in the audit's own two
crates that its 143 rows reach under no reading, and a `DESIGN.md` sentence
describing that audit's open findings as they stood two retirements ago. Every one of them
...
| ~~**E-d**~~ | ~~**D33**~~ | `docs/predicate-dimension-audit.md` | style; **closed on a coverage number plus a scope verdict** | **DONE — #761.** The bound is deliberate and now stated; 143 rows reach 223 of the two crates' 246 funnel names, and the 23 that no reading covers are enumerated in the file and placed as **D46**. |
...
| **D33** | **FIXED by #761.** The audit's two-crate bound is **deliberate, and it binds the TABLE, not the document** — the dimensional argument is made where the comparand is built, and the out-of-ledger crates (`profile`, `sweep`, `editor-core`'s eval/naming, `geom-curves`) argue their doors at their own sites, which the clause-(i) note already said and the funnel-bypass paragraph already called *"scoped on purpose"*. The *dispositions* were never so bounded (F12, F13, F14, F15 are `editor-core`), so the bound is on the **inventory**, not on the file. Both facts, and the fact that the workspace roster is `docs/K-REPORT.md`'s restatement rather than this table, now stand at the top of the audit. **Coverage, measured against `a0a6e1a5`**: shipped `geom-brep` + `topo` hold **313 funnel call sites** under nine spellings, 12 of them forwarding hops inside wrappers, so **299 decision sites carrying 246 distinct predicate names** — 210 written as a literal at a spelled site, **36 carried** by a module-private `const`, a struct field or a local table. The **143 rows reach 223 of the 246** (124 verbatim, 99 through a family cell or an abbreviated slash-list), leaving **23 recorded nowhere in the document under any reading**, now enumerated there with their homes. **23 is a floor, not an estimate**: the 99 family matches were read as generously as the glosses allow, so a stricter reading grows the 23 and nothing can shrink it — those names carry no row, no family cell, no slash-list, no gloss and no prose mention. The corpus half adds nothing the code half misses once the carriers are read, and has its own hole: **80** of the 246 never appear in the committed M7 baseline, which also still carries six spellings #652 retired. **Two corrections to this row as it was written**: the table has **143** rows, not 145 — 145 counts the two `| site |` header lines — and **all seven** of #747's orphans are outside the two crates, not five, because `demo_flush_orient` and `demo_flush_parallel` are `demos/tour` as well. Auditing the 23 is **D46**. | D19 (#747), which cross-checked its orphans against this document and found the bound | `docs/predicate-dimension-audit.md` | style, and it closed on a coverage number plus a scope verdict | nothing (D19 landed the criterion it uses) |
