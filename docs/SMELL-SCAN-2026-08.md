...
| **H2** | **S39 stale claims** — twelve rows (two added by #647's style review: `DESIGN.md:1362`'s false `topo::boolean` path, and `predicate-dimension-audit.md`'s stale line anchors), each classified **benign rot** vs **lost invariant** *before* its sentence is touched. `enters.rs:14` is the (ii) candidate: the outward-normal property was devolved onto every caller with no type enforcing it. | M |
