...
the shape the census's has** — that is **D65**.
...
- **#753 is ten files, not the two named above** — it also holds
  ...
  *"neither of E-a's two files"* and concludes it is free; that conclusion is
  drawn from this table's Scope column instead of from the PR, and D65's fix
...
| **F8** | **D65 and D66**, placed by Track E's lane E-b out of D23. **This row was written against D23's FIRST form, from the open PR, and both halves moved under review — the numbers changed and one of the two findings was withdrawn.** **D65** (was D44): the *executed* probe-suite set has no floor. The executed set is the modules `k_probe_sweep.sh`'s two `run_dump` filters name; `editor-core/tests/m5_pr5_corpus_probe.rs` is named by none and has never run in CI. Every document's *"thirteen of the fourteen"* came from reading `-p editor-core` out of the invocation instead of the filter out of the command. **The totals this row quoted (*"2 of 16"*, *"14 type-checked and never run"*) are withdrawn as prose counts of a set the census gate derives every merge** — see D65. And the obvious floor is unsound: `run_dump` passes `--ignored`, that file's test is not `#[ignore]`d, so filter-reachability would score it covered while it executes nothing. **D66** (replaces D45, which is withdrawn): `scripts/` and `.github/` carry locally-derivable enumerations D23's corpus excluded — `ci.yml:62`'s *"12 aggregators"* and `check-test-aggregation.sh:13-16`'s target counts. **D45's own thesis collapsed under review**: two of its four legs (`PERF-SCAN`'s *"367 `tests/*.rs` files"* and *"all 14 crates with tests"*) re-derive **exactly** at the scan base it said they failed at, so *"three of six"* is two of five, and what is left is annotated in place at §S36 and §S11 rather than carried as a row. | D65, D66 | `scripts/k_probe_sweep.sh`, `scripts/gates/probe-suite-census.sh`, `.github/workflows/ci.yml`, `scripts/check-test-aggregation.sh` — **all four inside E-a's scope or beside it; sequence beside D22 and D34** | **ACCEPTED** | style |
