...
| **D34** | **`gate-roster.sh` enforces wiring for one DIRECTORY while `ci.yml` invokes 23 scripts, and the claim a reader takes from it is wider than its scope.** This is D17's own defect one level up — a mechanism whose scope is narrower than the sentence it prints. The roster reports *"ci.yml wires a self-tested step for all 12 gates in `scripts/gates/`"* (13 after #739). True of the directory it scans; silent about everything else the workflow runs. **Enumerating from `ci.yml` rather than from `ls`** — `grep -vE '^\s*#' .github/workflows/ci.yml | grep -oE 'scripts/[A-Za-z0-9_/.-]+\.(sh|py)'` — **gives 23 scripts against the 13 gates its directory holds (14 `.sh` files, one of them the shared `lib.sh`).** The ten outside: `doc-gate.sh`, `check-test-aggregation.sh`, `check-interval-cfg-additive.py`, `rundump-guard-selftest.sh`, `interval-only-selection.py`, `check_admesh.sh`, `check_step.sh`, `ci-filter.py`, `k_probe_sweep.sh`, `tess_budget_sweep.sh`. **`doc-gate.sh` is the one that makes it sharp**: it exists precisely because `cargo check` and `cargo clippy -- -D warnings` are **silent** about rustdoc lints (its own header, #465), and #740's lane reddened `rustdoc (gate)` on a fix-pass push for exactly that — an intra-doc link to a renamed item, invisible to both. A guard for a hole nothing else covers, itself outside the guard-wiring check. **The scope problem is not only WHICH scripts but HOW they are invoked, and that half is worse.** A `scripts/gates/*.sh` glob runs each file once, bare. `ci.yml` invokes `probe-suite-census.sh` **three ways** — bare, `--selftest`, and `--crates` — and the `--crates` call is in a **different job** (`k-lint`) from the other two (`discipline`). Every roster gate is invoked at least twice (`--selftest` then the real pass), so a glob runs half of each; and `scripts/check-interval-cfg-additive.py` is a `.py` gate with the same two-call contract, invisible to a `*.sh` glob entirely. A scope verdict that names files and not invocations will still under-run the surface. **And the class reproduced itself in the PR that found it**: `scripts/rundump-guard-selftest.sh`, a guard *this session added*, landed outside the roster **by default** — nobody chose that, the directory did. **Write the row over the SCOPE QUESTION, because that is what has to be decided and the answer is not obviously "the directory".** #739 made the roster cover one more script by moving it **into** `scripts/gates/`, which works — and **nothing makes that the default for the next one**. Some of the ten plausibly should *not* be roster-enforced: `ci-filter.py` is the change filter itself (the thing that decides whether jobs run at all), and `check_admesh.sh`, `check_step.sh` and `interval-only-selection.py` consume CI-built artifacts rather than scanning sources, so the `--selftest`-then-real-pass contract does not fit them unchanged. **A row that says "wire all ten" would be wrong.** Decide whether the roster's subject is the **directory** or the **workflow**, say which of the ten each answer implies, and — whichever is chosen — make the printed sentence describe the scope it actually has, since that sentence is what a reader cites. Two facts make it checkable today: the **23-vs-13** count, and `rundump-guard-selftest.sh`'s placement. | D19 (#747), which enumerated the gate surface from `ci.yml` while re-verifying #739's merge | `scripts/gates/gate-roster.sh`, and `.github/workflows/ci.yml` if the answer moves any script | style, and it **closes on a scope verdict** — not on wiring ten scripts | nothing |
