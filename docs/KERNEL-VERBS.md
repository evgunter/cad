...
| **curved boolean breadth** | cyl×sphere, sphere×sphere, cone/torus operands | the banked germ-chord lanes (DESIGN frontier (d)); the SSI lift removed the storage half | each lane is its own unit; the teapot's spout∪body would ride revolve-surface × NURBS classes — far. **The operand gate is PAIR-scoped** (VERBS-GATE): a kind with no wired arm disqualifies an operation only where its BOX may meet a face of the other operand, so a cone or torus face that clears the other body no longer gates anything, and the refusal names the germ PAIR and both faces (`CurvedPairUnsupported`) with the box conservatism stated — overlap is a *may*, not a *does*. **No germ class was added**, so what moved is honesty and reach, not breadth. Measured on the register's own consumers: the Klein bottle's pieces still cannot be joined, but the refusal is now the flare's CONE against a plane of the loop — NOT the coincident annular mate the model cares about — and its self-intersection still cannot be trimmed, the same pair under `Subtract` (`klein::wall_probes` walls 3, 4). The lily's tepal seam, asked as sphere×sphere, answers `(Cone, Sphere)`: the lantern's conical pucker reaches the carving ball first, so that wall waits on the cone lane as well as on sphere×sphere (`lily::wall_probes` wall 7). **The lily's FLOWER WELD is no longer an SSI ask** (VERBS-LILYWELD PR-1, #1059): the flower/arch junction is authored circle-coincident — the lantern's neck cone is cut at the arch tube's own radius, so its rim IS that tube's terminal meridian circle, and `lily::weld_circle` computes both off the two STORED carriers and asserts them equal on every run (the lantern axis is the stem's own tangent, so the meridian circle at that station is the one circle a coaxial cone can be cut to). The union still refuses `CurvedPairUnsupported { op: None, (Cone, Torus) }` at the operand gate, because that gate reads KINDS and never loci — the coincidence is invisible to it. **That reading was itself wrong, and measuring it is what refuted it** (VERBS-LILYWELD PR-2, 2026-08-27): widen the gate in a scratch tree and the next refusal is `NonMaximalFaces` on the lantern itself — its two AXIS-TOUCHING planar caps, the F7 defect `merge_coplanar_faces` will not repair — and one door after that the curved PIERCE arm (wall 12's door). The `carrier_eq` rung is never reached and has no consumer: PR-1's own abutment made the weld's declared contact plane×plane, and a cone×torus Rest declaration would be `Contradicted`, correctly. So **wall 2's binding blocker is #1031**, not #968's shape; the gate-admission question is DEFERRED until #1031 lands and the sequence is re-measured, because no declaration covers the cone×torus pair for an admission rule to consult. Pinned live by `lily::review_probes::the_declared_weld_refuses_exactly_as_the_undeclared_one_does` and `the_lanterns_two_axis_touching_caps_are_the_f7_defect`. **#1031's POLE HALF has since landed** and the chain is one door shorter: measured, an axis-touching planar cap and the curved wall beside it are the SAME structure (two faces on one surface key, two meridian edges, a valence-2 pole), so `gate_maximal_faces`' planar rule was over-reaching onto revolve poles — `boolean::reduce::pole_split_cap` now exempts exactly that shape, and there was never a one-face form to demand because tier 2 REQUIRES the valence-2 pole. **lily wall 7 moved** as a result (`NonMaximalFaces` → `CurvedPierceUnsupported`); wall 2 did not, its blocker chain being gate → (F7, now open) → the curved pierce arm. #1031 stays open for its OTHER half — two genuinely coplanar faces at an ordinary edge, measured on the teapot cup's meridian plane with valence-4 endpoints, whose merge door refuses too (`MergedFaceRoleAmbiguous { face: 4v1 }`), so that half is a kernel question and not an authoring one. This is the FOURTH instance of *stated blocker ≠ binding constraint* (wall 7/#1031, wall 2/#1059, wall 8 in the M9 exit walk) and the first caught in a SPEC's own text rather than a probe comment — the measure-before-building rule is what caught it |
...
Consumers waiting on rows above: the full
...
`CurvedPairUnsupported`, so the teapot is FOUR solids (walls 3 and 4);
...
`sweep_body` will not round (`ReversedStacking`) and no
...
- **`shell` survives exactly ONE junction shape, and it is not about
  ...
  variable. A TANGENT junction refuses too, at a second door
