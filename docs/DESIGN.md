...
| `docs/AXIS-DECLARATION-DESIGN.md` | **OPEN QUESTION**, not ratified; asked by WIRE | Axis-flavoured declarations (coaxial, structural-parallel) have no identity channel: `ParamSource` carries stored scalar fields only. Recommends declared intent invalidated structurally by placement-chain comparison, so no numerical check decides whether a rotation happened |
