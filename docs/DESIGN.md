...
| `tools/README.md` | RECOMMENDATION — awaiting Ev | The instrument crates' shared rule, clauses `CC1`–`CC5`: where a cross-column check belongs when the admissions table is per column, and in which voice it speaks. `tess-lint` and `k-lint` both police a CSV column by column, and a property spanning two columns has no entry to live in; the class had eight instances in the tree and no statement. Recommends that the check goes at the reading boundary and only there (`CC1`), that a producer-side entailment is **not** a disposition (`CC4` — this page once stated its opposite, and the row proving it wrong is now a test), and that the deciding question is `Report`'s own — would the gate otherwise assert something false about a MEASUREMENT (`CC5`). **The second design page outside `crates/<crate>/README.md`**, and the first governing two sibling crates rather than one directory of scripts |
