...
| Serialization | `serde` + `serde_json` | MIT/Apache | **Adopted at M4 PR 6 (#112)** for persistence schema v1; the `float_roundtrip` feature is LOAD-BEARING (last-ulp parse drift caught day one); kernel crates stay serde-free (layering enforced by `scripts/gates/kernel-serde-free.sh`, a manifest scan — serde reaches a crate only through its manifest; `profile` is additionally sealed from inside by `profile/tests/seal.rs`). A kernel TYPE that must persist has its bytes described ABOVE the boundary, by a `#[serde(with)]` module in `editor-core` |
