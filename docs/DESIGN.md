...
| `crates/viewer/README.md` (from *The three layers (G1)*) | Ratified; GUI v1 shipped | GUI architecture G1–G5, GQ1–GQ7: the three-layer split, egui as toolkit, what v1 ships. **The clause-id sections only** — the page's *Module boundaries* half is implementation record the program maintains itself, and that page's *How to read this page* states the split |
