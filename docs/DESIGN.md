...
| `docs/RECIPE-DOORS-DESIGN.md` | RATIFIED (2026-08-29, in-chat: D2–D5; D1 reclassified as orchestrator sequencing) | Recipe doors for the shipped surgery verbs — chamfer, tube, shell. D2/D3: `Node::Chamfer` is `Node::Fillet`'s twin and reuses the fillet ROLE vocabulary (the minting node is the discrimination); the emitter pays #708's tie-deferral debt at the same time. D4, REVISED by the #1205 ruling (Evan, 2026-08-29): the tube vocabulary SPLITS — `Node::Tube` and `Node::HollowTube`, wall REQUIRED on the hollow kind, `Option` nowhere in the recipe vocabulary, one schema bump for both; the kernel's two public doors are read as its vocabulary and the shared private `build` as implementation. D5: shell WAITS on a kernel `ShellNaming` birth record. D2/D3 implemented at LIB-G16 (schema v16, #918); revised D4 implemented at LIB-TUBE (schema v17) |
