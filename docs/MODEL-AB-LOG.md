...
LIB-TUBE DISPATCHED (2026-08-29, block LIB-12 slot 4 — the block's
LAST slot; arm read back from the redacted record via git history,
NOT restated; the standing LIB-12 contamination flag rides the dual).
Spec: docs/LIB-TUBE-SPEC.md under RECIPE-DOORS D4 AS REVISED by the
#1205 split ruling. Pre-draw fields at spec: **M-L / STRUCTURAL**.
**SCHEMA CLAIM (the prose tripwire per the seam discipline): this
unit claims the next SCHEMA_VERSION after main's constant at its
branch point — v17 as of this entry (main was at v16, LIB-G16's
chamfer bump, read by eye at branch and at every re-merge); meaning:
"Node::Tube and Node::HollowTube, the tube recipe doors". ONE number
covers BOTH node kinds, per the revised D4 — a second number would
claim the second kind is a change a reader could meet without the
first, which is false.** Lane lib-tube, branch lib/tube. Ordinal
claimed at review dispatch. Row at merge.
