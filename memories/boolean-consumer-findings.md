...
1. **Cookie-cutter seams are SILENTLY WRONG** (fail-loud violated). When the
...
3. **Touching/coplanar unions refuse** (expected PR 6 territory, typed):
   ...
   knife through a two-shell Voided body → same RingHoming refusal.
...
When the PR 5 fix pass lands, re-check the demo call sites listed in the
ev/demo-boolean-tour report: `die_blocked()` asserts the pocket subtract is
STILL wrong (it fails loudly the day the fix lands — promote the die then),
and the table/openbox notes narrate defects that should then disappear.
