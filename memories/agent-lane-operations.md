...
may only add. **The `workflow_dispatch` inputs are now the ONLY spelling
that narrows**: a trailer may name only the whole-dimension value
(`lane=both` / `eps=all` / `klint=all`) and reds on anything else, so
`CI-Config: lane=interval` buys a RED, not less gate. A run gates all six
lane/eps points and all five unifications by default. (2026-09-04.)
**The run record is the instrument; the workflow source is not.**
