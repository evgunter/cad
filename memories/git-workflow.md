...
(`gh pr merge --merge`, never `--squash`/`--rebase`). Assumed exception
(unconfirmed): PRs that ratify open design questions in DESIGN.md are
design conversations per docs/M0-PLAN.md — wait for Evan's sign-off before
