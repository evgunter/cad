...
/// `seam_arrival_check`, its declared twin, has no such arm: with the
/// arm below ε the levered turn AND the levered side both read Zero, so
/// a leg arriving from ANY direction satisfies the declaration. The
/// deviation is caught downstream at the gate, so it is loud rather than
/// silent — but the declared check itself accepts unchecked.
#[test]
fn r2_a_degenerate_closing_leg_passes_the_declared_check_unchecked() {
    let t = Tol::witness();
    let (eps, _) = band();
    let stub = 1e-4 * eps;
    ...
    println!("R2: degenerate declared close -> {:?}", declared.is_ok());
    let undeclared = build(false);
    println!("R2: degenerate undeclared close -> {undeclared:?}");
    assert!(
        declared.is_ok(),
        "the declared check accepts a sub-eps arm whatever the arriving direction: {declared:?}"
    );
    let verdict = Profile::new(SketchPlane::xy(), vec![declared.unwrap().loop_]).validate(t);
    println!("R2: degenerate declared close, at the gate -> {verdict:?}");
    assert!(verdict.is_err(), "the gate is what catches it");
}
...
fn r2_a_straight_arrival_onto_an_arc_first_side_authors_but_does_not_validate() {
    ...
    // AFTER: the declared spelling closes, with NO tangent joint.
    let closed = ring(true).expect("the declared straight arrival closes");
    println!(
        "R2: arc-first-side, declared -> closed, tangent_joints {:?}",
        closed.loop_.tangent_joints()
    );
    assert!(closed.loop_.tangent_joints().is_empty());

    // And the gate refuses the loop the lattice just minted.
    let verdict = Profile::new(SketchPlane::xy(), vec![closed.loop_]).validate(t);
    println!("R2: arc-first-side, at the gate -> {verdict:?}");
