...
/// **RE-AIMED ON ADOPTION (the fix landed).** As delivered this row
/// replayed the census's needle list VERBATIM into a local copy and
/// asserted that four live spellings slipped past it — the qualified
/// `<ProfileLoop<f64> as RawLoop<f64>>::polygon` the tree uses in ~50
/// places, the turbofish, the function reference, and a type alias.
/// The fix (R1-m2/R2-MINOR-1) deletes generic argument lists before
/// matching and matches PATHS rather than calls, so three of the four
/// are caught.
///
/// Two things changed here and both matter. The row now calls the
/// census's OWN `is_writer` instead of a copy of it, so it cannot go on
/// passing against a matcher that has moved; and the fourth line — the
/// type alias — is asserted to be MISSED, on purpose, because that is
/// a stated blind spot of a text census (following an alias is name
/// resolution) and the instrument for it is the compiler, in CI's
/// wasm32 non-dev check. A row that pretended otherwise would be the
/// original finding wearing the fix's clothes.
#[test]
fn r2_the_door_census_reads_the_qualified_spelling_and_says_what_it_cannot() {
    let caught = [
    ...
    for line in caught {
        assert!(
            crate::raw_door_census::is_writer(line),
            "the census still cannot see this spelling: {line}"
        );
    }

    // The declared blind spot, held as one: an alias names neither type
    // on the calling line, and nothing textual can follow it.
    assert!(
        !crate::raw_door_census::is_writer("    let t: Tbl = Tbl::new(Vec::new());"),
        "the census claims to catch an aliased call — either it grew a \
         resolver or this row is lying about what it cannot do"
    );

    // And the normalizer does not eat comparisons or arrows.
    assert!(!crate::raw_door_census::is_writer(
        "    if a < b && c > d { }"
    ));
    assert_eq!(
        crate::raw_door_census::without_generics("fn f(x:u8)->Vec<u8>{}"),
        "fn f(x:u8)->Vec{}"
    );
}
...
/// **RE-AIMED ON ADOPTION (the fix landed).** As delivered this row
/// reported a gap: `lift.rs`'s header says every value-equal census row
/// is pinned against its own tight ceiling, two of the three had one
/// (`bracket` < 1e-12, the carrier phase < 1e-15) and `rounded_rect` —
/// the row this unit's widened lift added — had none, so what actually
/// held it was `VALUE_EQUAL_ABS` = 1e-12, three hundred times the
/// residue measured. R2-MINOR-8 gave it one.
///
/// The row keeps its measurement and adds the thing that was missing:
/// it reads `lift_census.rs` and asserts a ceiling for this row is
/// actually written there. Measuring a figure that no assertion holds
/// was the finding; a probe that only re-measured it would let the
/// ceiling be deleted again in silence.
#[test]
fn r2_the_new_value_row_has_a_ceiling_of_its_own() {
        ...
        LiftOutcome::Lifted {
            ...
            assert!(
                ...
                "a residue this small is a different row than the one \
                 measured; re-derive the ceiling rather than widening \
                 this: {worst_abs:e}"
            );
        }
