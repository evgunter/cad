...
mod dominance {
    ...
    pub(crate) fn signed_swept(from: f64, to: f64, turn: f64) -> f64 {
        let s = ((to - from) * turn).rem_euclid(TAU);
        s - TAU * (s / TAU + 0.5).floor()
    }

    /// `+1` or `−1` with equal probability.
    pub(crate) fn sign(rng: &mut test_utils::fuzz::Rng) -> f64 {
    ...
    pub(crate) fn tally(surv: &[Survivor], rho1: f64, rho2: f64, stats: &mut (u64, u64, u64)) {
        if surv.len() != 2 {
            return;
        }
        stats.0 += 1;
