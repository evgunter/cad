...
//! takes plain `f64` literals, embeds them with [`Real::from_f64`] —
//! an *exact* embedding for every implementor, so nothing here can
//! change a number — and calls exactly one kernel constructor. There
//! is no arithmetic, no defaulting, and no panicking: [`validated`]
//! returns the kernel's own `Result` with the kernel's own error.
//!
//! Before this seam existed, the demo corpus carried six
//! near-identical `p2` helpers and four `validated` wrappers, one per
//! scene, plus 95 hand-written `S::from_f64` calls. That is the whole
//! reason the module exists.
//!
//! The generic parameter stays: these are the *entry* points, so the
//! same scene source still instantiates at `f64` for a normal build
//! and at a telemetry or interval scalar for a certified one. What
//! goes away is the conversion noise at every literal.
...
/// scalar operands such as `Extrusion::Distance(real(0.75))`.
#[inline]
