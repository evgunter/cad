...
/// and its fix, on one row.** As written for the review the cone arm
/// had no guard of its own: the sphere arm filtered on
/// `props_circle_axis_class` before measuring a pole against a circle,
/// while the cone arm read EVERY `Line` carrier as a generator, so a
/// line lying on no cone at all answered `NotOneChartBranch` — a
/// sentence about an apex it never approaches — where the SHAPE door
/// answers `props_meridian_generator`. What kept it honest was
/// entirely the layering claim (the shape door ran first), which a
/// public door may not rest on.
///
/// **Re-aimed by the union fix pass**: the arm now filters lines on
/// `props_meridian_apex`, the parse's own incidence margin and the
/// cone's mirror of the sphere's filter, so the assertion below is
/// inverted from what this probe recorded. The construction is the
/// probe's, unchanged — a line nowhere near the cone whose foot-point
/// parameter falls inside its stored span — and it is the row that
/// would redden if the guard were removed again.
#[test]
fn r1_the_cone_arm_does_not_read_a_stray_line_as_a_generator() {
    ...
    assert_eq!(
        branch,
        Ok(()),
        "the branch door must SKIP a line that is no generator of this cone, not answer \
         about an apex it never approaches; got {branch:?}"
    );
}
