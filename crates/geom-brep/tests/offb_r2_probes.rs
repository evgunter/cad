...
/// **P6b — the same lever, permissive direction.** AMENDED at
/// adoption in the same way as P6: the assertion (one fixed
/// geometry, one verdict, whatever `|d|` is) is unchanged and now
/// holds by construction, since the predicate no longer sees `d`.
///
/// A planar patch
...
/// this row went red and drove the fix. `offset_normal_floor` now
/// classifies `Margin::over_lever(floor, max chart speed)` and takes
/// no `d` at all; the row is unchanged in what it asserts, and now
/// passes for the right reason. Its original text is kept below
/// because it is the derivation of the defect.
///
/// `offset_normal_floor` classified `Margin::levered(sine_floor,
/// |d|)`. The geometry it is
