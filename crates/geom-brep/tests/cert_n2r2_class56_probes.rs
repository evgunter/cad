...
fn plane() -> Surface<f64> {
    Surface::Plane { origin: Point3::new(0.0, 0.0, 0.0), normal: Vec3::new(0.0, 1.0, 0.0), u_ref: Vec3::new(1.0, 0.0, 0.0) }
}
fn band() -> Band {
...
fn poison_one(i: usize, p: Point3<f64>) -> Point3<f64> {
    if i == 3 { Point3::new(f64::NAN, p.y, p.z) } else { p }
}
...
fn n2r2_class5_plane_nurbs_limbs_on_masquerade() {
    ...
    for (name, f) in [("x-poison", poison_x as fn(usize, Point3<f64>) -> Point3<f64>), ("z-poison", poison_z), ("one-point", poison_one)] {
        let w = wall(f);
...
fn n2r2_class6_certify_lanes_on_masquerade() {
    for (name, f) in [("x-poison", poison_x as fn(usize, Point3<f64>) -> Point3<f64>), ("z-poison", poison_z), ("one-point", poison_one)] {
        ...
        let carrier = Curve3::Nurbs(std::sync::Arc::new(segment(Point3::new(1.0, 0.0, 0.0), Point3::new(1.0, 0.0, 1.0))));
        let witness = carrier.eval(0.5);
        let ends = (carrier.eval(0.0), carrier.eval(1.0));
        let spec = EdgeCurveSpec { description: EdgeDescriptionSpec::Intersection { s1, s2, witness }, carrier: carrier.clone(), param_start: 0.0, param_end: 1.0 };
        let r = EdgeCurve::certify_nurbs_lane(spec, ends.0, ends.1, |k| arena.get(k).cloned(), band());
        eprintln!("[class 6 plane×NURBS {name}] certify_nurbs_lane -> {}", match &r { Ok(e) => format!("Ok(cert {:?})", e.certificate()), Err(e) => format!("Err({e:?})") });
        // The chart (iso) lane: the carrier is the wall's u=0 boundary row.
        let spec = EdgeCurveSpec { description: EdgeDescriptionSpec::chart(s2), carrier: carrier.clone(), param_start: 0.0, param_end: 1.0 };
        let r = EdgeCurve::certify(spec, ends.0, ends.1, |k| arena.get(k).cloned(), band());
        eprintln!("[class 6 chart/iso {name}] certify -> {}", match &r { Ok(e) => format!("Ok(cert {:?})", e.certificate()), Err(e) => format!("Err({e:?})") });
        let spec = EdgeCurveSpec { description: EdgeDescriptionSpec::chart(s2), carrier, param_start: 0.0, param_end: 1.0 };
        let r = EdgeCurve::certify_nurbs_lane(spec, ends.0, ends.1, |k| arena.get(k).cloned(), band());
        eprintln!("[class 6 chart/iso via nurbs lane {name}] -> {}", match &r { Ok(e) => format!("Ok(cert {:?})", e.certificate()), Err(e) => format!("Err({e:?})") });
    }
}
