//! **The ruled band's carve** — the open band on CURVED supports, cut
//! off at transverse caps.
//!
...
//!
//! Carve ([`carve`]): per end, split each rim edge at its foot
//! (`surgery::seam_split_param`, the ladder's one home for a split
//! parameter) and `mef` the cut-off arc across the cap between the two
...
/// the verdict.
pub(super) struct RuledPlan<'a, T: Real> {
...
impl<'a, T: Decide + Bounds> RuledPlan<'a, T> {
    ...
    pub(super) fn link(&self) -> AdmittedOpen<'a, T> {
        self.link
    }
}
...
/// with the plan.
pub(super) fn carve<T: Decide + Bounds>(
    body: &mut Body<T>,
    plan: &RuledPlan<'_, T>,
    rec: &mut BlendNaming,
    tol: Tol,
