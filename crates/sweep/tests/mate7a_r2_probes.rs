...
/// `escalate` 1e-8). The rim is the same rim to within the band, but
/// the routing goes silent and the bare class refusal comes back.
#[test]
fn r2_an_in_band_rim_identification_is_silently_no_rim() {
    ...
    assert!(
        matches!(e_exact, BooleanError::RimArmUnbuilt { .. }),
        "the unit's own row: the exact kiss routes"
    );
    ...
    assert!(
        ...
        "R2: an in-band rim identification comes back as the bare class refusal, not an \
         escalation: {e_pert:?}"
    );
}
...
/// **A DEFINITE mismatch and an INDETERMINATE one are the same
/// answer.** The row above shows the in-band case; this one shows the
/// definitely-different case reaching the identical refusal, so the
/// author cannot tell "no rim" from "I could not decide whether there
/// is a rim".
#[test]
fn r2_definitely_no_rim_and_cannot_tell_are_indistinguishable() {
    let a = full_torus(RING);
