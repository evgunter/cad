...
/// **The fold's vertex PAIRING carries no information.** The slab
/// margin is a SUM of differences over a full zip of two equal-length
/// lists, so it equals the difference of the two sums: rotating the
/// top section's traversal order cannot change it. The row states that
/// as a property of the door — the same two stacks, one with the top
/// square authored from a different corner, decide identically — so
/// the "mean top-vertex displacement" the docs describe is in fact the
/// displacement of the vertex CENTROID and nothing about the
