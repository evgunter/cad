...
fn face_loops(body: &Body<f64>, f: FaceKey) -> Vec<LoopKey> {
    ...
    core::iter::once(d.outer).chain(d.rings.iter().copied()).collect()
}
...
fn audit(what: &str, source: &Body<f64>, chart: &[FaceKey], shelled: &Shelled<f64>) {
    ...
    assert_eq!(topo::validate_geometric(body, Tol::witness()), Ok(()), "{what}: tier 3");
    ...
    assert_eq!(outer_src, expect_outer, "{what}: outer's source column is the undesignated operand faces in arena order");
    for &(r, s) in &rec.outer {
        assert_eq!(r, s, "{what}: a survivor keeps its key");
        assert!(body.get_face(r).is_some(), "{what}: outer {r:?} does not resolve");
    }
    ...
    assert_eq!(rec.inner.iter().map(|&(_, s)| s).collect::<Vec<_>>(), src_faces, "{what}: inner");
    assert_eq!(rec.inner_edges.iter().map(|&(_, s)| s).collect::<Vec<_>>(), src_edges, "{what}: inner_edges");
    assert_eq!(rec.inner_vertices.iter().map(|&(_, s)| s).collect::<Vec<_>>(), src_vertices, "{what}: inner_vertices");
    // result columns injective
    assert!(!sorted_dedup(&rec.inner.iter().map(|&(r, _)| r).collect::<Vec<_>>()).1, "{what}: inner result dup");
    assert!(!sorted_dedup(&rec.inner_edges.iter().map(|&(r, _)| r).collect::<Vec<_>>()).1, "{what}: inner_edges result dup");
    assert!(!sorted_dedup(&rec.inner_vertices.iter().map(|&(r, _)| r).collect::<Vec<_>>()).1, "{what}: inner_vertices result dup");
    // a twin never aliases a survivor key
    for &(r, _) in &rec.inner {
        assert!(source.get_face(r).is_none(), "{what}: twin face {r:?} aliases an operand key");
    }
    for &(r, _) in &rec.inner_edges {
        assert!(source.get_edge(r).is_none(), "{what}: twin edge {r:?} aliases an operand key");
    }
    for &(r, _) in &rec.inner_vertices {
        assert!(source.get_vertex(r).is_none(), "{what}: twin vertex {r:?} aliases an operand key");
    }
    ...
    assert_eq!(named_s, live, "{what}: face channels do not partition the live faces");
    ...
    for (e, _) in body.edges() {
        assert!(twin_e.contains(&e) != source.get_edge(e).is_some(), "{what}: edge {e:?} partition");
    }
    let twin_v: Vec<VertexKey> = rec.inner_vertices.iter().map(|&(r, _)| r).collect();
    for (v, _) in body.vertices() {
        assert!(twin_v.contains(&v) != source.get_vertex(v).is_some(), "{what}: vertex {v:?} partition");
    }
    ...
    let mentioned_f: Vec<FaceKey> = src_faces
        ...
        .chain(rec.rims.iter().flat_map(|r| r.holes.iter().map(|&(f, _)| f)))
        .collect();
    let (expect_dead_f, _) = sorted_dedup(&mentioned_f.iter().copied().filter(|&f| body.get_face(f).is_none()).collect::<Vec<_>>());
    let (dead_f, dup_f) = sorted_dedup(&rec.dead.faces);
    assert!(!dup_f, "{what}: dead.faces has duplicates: {:?}", rec.dead.faces);
    assert_eq!(dead_f, expect_dead_f, "{what}: dead.faces is not exactly the retired mentioned faces");
    let mentioned_e: Vec<EdgeKey> = src_edges.iter().copied().chain(twin_e.iter().copied()).collect();
    let (expect_dead_e, _) = sorted_dedup(&mentioned_e.iter().copied().filter(|&e| body.get_edge(e).is_none()).collect::<Vec<_>>());
    let (dead_e, dup_e) = sorted_dedup(&rec.dead.edges);
    assert!(!dup_e, "{what}: dead.edges has duplicates");
    assert_eq!(dead_e, expect_dead_e, "{what}: dead.edges is not exactly the retired mentioned edges");
    let mentioned_v: Vec<VertexKey> = src_vertices.iter().copied().chain(twin_v.iter().copied()).collect();
    let (expect_dead_v, _) = sorted_dedup(&mentioned_v.iter().copied().filter(|&v| body.get_vertex(v).is_none()).collect::<Vec<_>>());
    let (dead_v, dup_v) = sorted_dedup(&rec.dead.vertices);
    assert!(!dup_v, "{what}: dead.vertices has duplicates");
    assert_eq!(dead_v, expect_dead_v, "{what}: dead.vertices is not exactly the retired mentioned vertices");
    ...
    for (i, rim) in rec.rims.iter().enumerate() {
        ...
        for s in &rim.sources {
            assert!(chart.contains(s), "{what}: rim {i} source {s:?} was not designated");
            assert!(!seen_sources.contains(s), "{what}: {s:?} in two rims");
            seen_sources.push(*s);
        }
        assert!(rim.sources.contains(&rim.rim), "{what}: rim {i} face is not the chart's survivor");
        let data = body.get_face(rim.rim).unwrap_or_else(|| panic!("{what}: rim {i} face does not resolve"));
        assert!(data.rings.contains(&rim.ring), "{what}: rim {i}'s ring is not a ring of its face");
        // ring rows in cycle order
        let on_ring = loop_edges(body, rim.ring);
        assert_eq!(rim.ring_edges.iter().map(|&(r, _)| r).collect::<Vec<_>>(), on_ring, "{what}: ring_edges result column is not the ring cycle");
        let on_ring_v = loop_vertices(body, rim.ring);
        assert_eq!(rim.ring_vertices.iter().map(|&(r, _)| r).collect::<Vec<_>>(), on_ring_v, "{what}: ring_vertices result column is not the ring cycle");
        ...
        for &(r, s) in &rim.ring_edges {
            assert!(bounding_e.contains(&s), "{what}: ring edge source {s:?} does not bound the chart in the operand");
            assert!(rec.inner_edges.contains(&(r, s)), "{what}: ring row {r:?}<-{s:?} absent from inner_edges verbatim");
        }
        for &(r, s) in &rim.ring_vertices {
            assert!(bounding_v.contains(&s), "{what}: ring vertex source {s:?} does not bound the chart in the operand");
            assert!(rec.inner_vertices.contains(&(r, s)), "{what}: ring vertex row absent from inner_vertices verbatim");
        }
        // holes
        for &(promoted, src_ring) in &rim.holes {
            let d = body.get_face(promoted).unwrap_or_else(|| panic!("{what}: promoted {promoted:?} does not resolve"));
            assert_eq!(d.rings, vec![src_ring], "{what}: promoted face's ring is the row's");
            // The docs call `src_ring` a SOURCE key. Measured below (r1_hole_rows_...):
            // on the slit cap it is a loop `kemr` minted in the RESULT.
            let in_operand = rim.sources.iter().any(|&f| source.get_face(f).expect("src face").rings.contains(&src_ring));
            println!("[{what}] hole row ring {src_ring:?}: resolves in the operand as a designated face's ring = {in_operand}");
        }
    }
    // every designated chart got a rim row
    for f in chart {
        assert!(seen_sources.contains(f), "{what}: designated {f:?} appears in no rim row");
    }
    // an undesignated operand face never dies
    for f in &expect_outer {
        assert!(body.get_face(*f).is_some(), "{what}: undesignated {f:?} died");
    }
}
...
fn audit_twice(what: &str, source: &Body<f64>, chart: &[FaceKey], t: f64) -> Shelled<f64> {
    ...
    assert_eq!(format!("{:?}", a.naming), format!("{:?}", b.naming), "{what}: D9");
...
fn r1_box_sealed_cup_and_tube() {
    let body = extruded(vec![polygon(&[(0.0, 0.0), (2.0, 0.0), (2.0, 3.0), (0.0, 3.0)])], 4.0);
...
fn r1_annular_tube_cup() {
    ...
    match topo::shell_open(&body, 0.05, &both, FIT_TOL, Tol::witness()) {
        Err(ShellError::OpenFacesDisconnect { components, .. }) => println!("[tube both] disconnect gate: {components}"),
...
fn r1_p6_one_holed_slab() {
    ...
    let hole = polygon(&[(cx - s, cy - s), (cx + s, cy - s), (cx + s, cy + s), (cx - s, cy + s)]);
...
fn r1_p2_counterbore_is_still_the_disconnect_refusal() {
    ...
    let body = revolved_full(&[(0.0, 0.0), (ro, 0.0), (ro, h), (rb, h), (rb, h - depth), (0.0, h - depth)]);
...
fn r1_chart_worn_by_several_faces() {
    ...
    match topo::shell_open(&body, 0.05, &top, FIT_TOL, Tol::witness()) {
        ...
        Ok(s) => {
            ...
            println!("[several] BUILT; rims={} dead faces={}", s.naming.rims.len(), s.naming.dead.faces.len());
        }
    }
}
...
fn r1_bellied_pot() {
    ...
    let body = revolved_loop(bulged(&[(0.0, 0.0, 0.0), (4.0 / 64.0, 0.0, 0.0), (4.0 / 64.0, 1.0 / 64.0, BULGE), (3.0 / 64.0, 8.0 / 64.0, 0.0), (0.0, 8.0 / 64.0, 0.0)]));
    let top = plane_chart_at_y(&body, 8.0 / 64.0);
    println!("[belly] faces {} mouth chart {}", body.faces().count(), top.len());
    audit_twice("belly sealed", &body, &[], 1.0 / 128.0);
    audit_twice("belly cup", &body, &top, 0.03);
}
...
fn r1_hole_rows_source_key_claim_is_false_on_the_slit_cap() {
    ...
    let top = plane_chart_at_y(&body, 0.40);
    assert_eq!(top.len(), 1);
    assert!(body.get_face(top[0]).unwrap().rings.is_empty(), "the slit cap carries no ring in the operand");
    ...
    let (_, ring) = s.naming.rims[0].holes[0];
    assert!(slab.get_face(top[0]).unwrap().rings.contains(&ring), "on the slab it IS the operand's ring");
}
