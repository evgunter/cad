...
/// polygon through its two vertices — a zero-area segment. If the
/// crossing is missed, the union is the same silent wrong body the PR
/// fixed for full discs. This probe MEASURES which it is.
#[test]
fn r2_a_box_through_a_half_disc_cap_measures_the_mixed_loop_remainder() {
    ...
    match topo::union(&a, &b, tol) {
        Err(e) => println!("half-disc cap union refuses typed: {e:?}"),
        Ok(topo::BooleanResult::Body(out)) => {
            let v = topo::mass_properties(&out.body, tol).unwrap().volume;
