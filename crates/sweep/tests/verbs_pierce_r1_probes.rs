...
/// segment of zero area, exactly the cap's defect. The PR names the
/// half-disc as the stated blind spot. Both bulge senses are run
/// against the SAME box: one of the two genuinely contains it, so if
/// both answer "disjoint" the wrong answer is demonstrated without
/// having to know which sense is which.
#[test]
fn r1_a_box_through_a_half_disc_cap() {
    let b = boxx(-0.15, 0.15, -0.6, -0.3, 1.0, 3.0);
    let hd = PI * 0.5 * 2.0;
    println!(
        "R1[half-disc-cap] disjoint-answer={} buried-truth={}",
        hd + 0.3 * 0.3 * 2.0,
        hd + 0.3 * 0.3 * 1.0
    );
    for bulge in [1.0, -1.0] {
        ...
        report(&format!("half-disc-cap bulge={bulge}"), &a, &b);
    }
}
