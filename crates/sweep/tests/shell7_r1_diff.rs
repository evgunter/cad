...
fn dump(label: &str, body: &Body<f64>) {
    ...
    for (k, f) in body.faces() {
        println!(
            "[r1diff] {label}: face {k:?} sense={} surface={:?}",
            f.sense,
    ...
    for (k, e) in body.edges() {
        let c = body
            ...
            .unwrap();
        println!(
            "[r1diff] {label}: edge {k:?} carrier={:?} params={:?} description={:?}",
            c.carrier(),
            c.params(),
            c.description()
        );
...
fn shell7_r1_diff_corpus() {
    ...
    for (r0, r1) in [
        (4.0 / 64.0, 2.0 / 64.0),
        (2.0 / 64.0, 4.0 / 64.0),
        (3.0 / 64.0, 3.0 / 64.0 + 1.0 / 256.0),
    ] {
        shelled(
            &format!("frustum {r0} {r1}"),
            &polyline(
    ...
    shelled(
        "pot",
        &polyline(
            &[(0.0, 0.0), (0.8, 0.0), (1.2, 1.0), (1.0, 2.0), (0.0, 2.0)],
            Revolution::Full,
        ),
        0.15,
    );
    if let Some(hollow) = shelled(
        "vessel",
        &polyline(
            &[(0.0, 0.0), (1.0, 0.0), (1.0, 2.0), (0.0, 2.0)],
            Revolution::Full,
    ...
    match tube_along_arc_hollow::<f64>(
        ...
        Ok(b) => {
            dump("elbow hollow operand", &b.body);
            shelled("elbow hollow", &b.body, 0.05);
        }
