...
pub(crate) fn revolved(lp: ProfileLoop<f64>, turn: Revolution<f64>) -> Body<f64> {
...
pub(crate) fn wedge(r: f64, h: f64, turn: f64) -> Body<f64> {
    polyline(
...
pub(crate) fn axial(p: Point3<f64>) -> (f64, f64) {
    ((p.x * p.x + p.z * p.z).sqrt(), p.y)
}

pub(crate) fn point(body: &Body<f64>, v: VertexKey) -> Point3<f64> {
    *body
        .get_point(body.get_vertex(v).expect("vertex").point)
        .expect("point")
}

pub(crate) fn carrier(body: &Body<f64>, e: EdgeKey) -> (Curve3<f64>, (f64, f64)) {
    let c = body
...
pub(crate) fn line_edge(
    body: &Body<f64>,
    pick: impl Fn(Point3<f64>, Vec3<f64>, bool) -> bool,
) -> EdgeKey {
...
pub(crate) fn split_mid(body: &mut Body<f64>, edge: EdgeKey) -> VertexKey {
...
pub(crate) fn corner_refusal(e: &ShellError<f64>) -> Option<(VertexKey, usize, &'static str)> {
...
pub(crate) fn edge_refusal(e: &ShellError<f64>) -> Option<(EdgeKey, &'static str)> {
    let ShellError::Face { error, .. } = e else {
        return None;
    };
    match **error {
