#!/usr/bin/env python3
"""TESS-CAP-DIAG scratch generator (adapted from ../halfcap/gen_halfcap.py)."""
import math
import sys
...
def build(kind):
    """TESS-CAP-DIAG scratch fixtures: the cap of an R = 10 mm sphere
    above latitude B, closed by its base disc.
      rimonly2   : rim as two half arcs, 2 V / 2 E / 2 F (no meridian)
      rimonly1   : rim as ONE closed circle edge, 1 V / 1 E / 2 F
      seamed     : OCC style: closed rim + a seam meridian A -> pole
                   used twice by the sphere face, 2 V / 2 E / 2 F
    """
    h = R * math.sin(B)
    rc = R * math.cos(B)
    pos = {"A": (rc, 0.0, h)}
    ...
    seam = []
    if kind == "seamed":
        mc = s.add("CIRCLE('',#%d,%s)" % (s.ax2((0, 0, 0), (0, -1, 0), (1, 0, 0)), num(R)))
        E["m"] = s.add("EDGE_CURVE('',#%d,#%d,#%d,.T.)" % (V["A"], V["N"], mc))
        seam = [("m", "T"), ("m", "F")]
    ...
    # rim +u (interior toward the north pole), then the seam up and back.
    face(sph, [*rim_fwd, *seam])
    ...
    assert not bad or kind == "seamed", ("mis-paired edges", bad)
...
d = sys.argv[1]
for k in ("rimonly2", "rimonly1", "seamed"):
