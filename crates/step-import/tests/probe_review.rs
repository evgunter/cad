...
mod common;
use step_import::{import_step, ImportOptions, StepImport};
...
fn a3_cone_trunc_imports_then_fails_tier3_at_1e7() {
    ...
    let StepImport::Solid { body, .. } = imp else { panic!("wireframe") };
...
fn a3_sweep_all_tiers() {
    let eps = geom_core::Tolerance::get().eps;
    let names = [
        "box", "cylinder", "cone_trunc", "cone_apex", "sphere", "torus",
        "box_hole", "fuse_boxes", "box_fillet_edge", "box_fillet_corner",
        "compound_two", "box_importexport", "twobody_importexport",
    ];
    for name in names {
...
fn a1_inside_out_torus_cannot_slip_through() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        Ok(StepImport::Solid { body, .. }) => {
            ...
            assert!(g.is_err(), "an inside-out torus certified positive: laundered!");
        }
...
fn a1_reversed_bound_torus() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        Ok(StepImport::Solid { body, .. }) => {
            ...
            assert!(g.is_err(), "reversed-bound torus certified positive: laundered!");
        }
        Ok(_) => panic!("wireframe"),
...
fn a1_flipped_torus_volume() {
    for (label, from, to) in [
        ("sense", "#17 = ADVANCED_FACE('',(#18),#38,.T.);", "#17 = ADVANCED_FACE('',(#18),#38,.F.);"),
        ("bound", "#18 = FACE_BOUND('',#19,.T.);", "#18 = FACE_BOUND('',#19,.F.);"),
    ] {
        let text = fixture("torus").replace(from, to);
        let Ok(StepImport::Solid { body, normalizations, .. }) =
            import_step(&text, &ImportOptions::default()) else {
            println!("a1vol {label}: refused/other");
            continue;
        };
        let props = topo::mass_properties(&body);
        println!("a1vol {label}: norms={} volume={:?}",
            normalizations.len(), props.map(|p| p.volume));
    }
}
...
fn a1_double_flipped_torus() {
    let text = fixture("torus")
        .replace("#17 = ADVANCED_FACE('',(#18),#38,.T.);",
                 "#17 = ADVANCED_FACE('',(#18),#38,.F.);")
        .replace("#18 = FACE_BOUND('',#19,.T.);",
                 "#18 = FACE_BOUND('',#19,.F.);");
    match import_step(&text, &ImportOptions::default()) {
        Ok(StepImport::Solid { body, normalizations, .. }) => {
            let props = topo::mass_properties(&body);
            println!("a1 double-flip: norms={} vol={:?} t3={:?}",
                normalizations.len(), props.map(|p| p.volume),
                topo::validate_geometric(&body));
        }
        Ok(_) => panic!("wireframe"),
...
fn a1_flipped_torus_body_sense() {
    for (label, text) in [
        ("orig", fixture("torus")),
        ("sense-flip", fixture("torus").replace(
            "#17 = ADVANCED_FACE('',(#18),#38,.T.);",
            "#17 = ADVANCED_FACE('',(#18),#38,.F.);")),
    ] {
        let Ok(StepImport::Solid { body, .. }) =
            import_step(&text, &ImportOptions::default()) else { panic!() };
...
fn a1_torus_halfedge_diff() {
    let mk = |text: &str| {
        ...
        for (_, lp) in body.loops() {
            let topo::LoopBoundary::Cycle { first } = lp.boundary else { continue };
            let cycle = body.loop_cycle(first).unwrap();
            let dirs: Vec<String> = cycle.iter()
                .map(|&he| {
...
fn a1_control_box_bound_flip() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        Ok(StepImport::Solid { body, .. }) => {
            println!("a1bb imported; t2={:?} t3={:?}",
                topo::validate_closed(&body), topo::validate_geometric(&body));
        }
        Ok(_) => panic!("wireframe"),
        Err(e) => { let s=e.to_string(); println!("a1bb refused: {}", &s[..s.len().min(120)]); }
    }
}
    ...
    for (label, from, to) in [
        ...
        match import_step(&text, &ImportOptions::default()) {
            Ok(StepImport::Solid { body, .. }) => {
                ...
                println!("a1ca {label}: imported t3={:?} vol={:?}",
                    topo::validate_geometric(&body), v);
            }
            Ok(_) => panic!("wireframe"),
            Err(e) => { let s=e.to_string(); println!("a1ca {label} refused: {}", &s[..s.len().min(110)]); }
        }
    }
}
...
fn a1_control_cylinder_sense_flip() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        Ok(StepImport::Solid { body, .. }) => {
            let v = topo::mass_properties(&body).map(|p| p.volume);
            let senses: Vec<bool> = body.faces().map(|(_, f)| f.sense).collect();
            println!("a1cyl imported t3={:?} vol={:?} senses={senses:?}",
                topo::validate_geometric(&body), v);
        }
        Ok(_) => panic!("wireframe"),
        Err(e) => { let s=e.to_string(); println!("a1cyl refused: {}", &s[..s.len().min(110)]); }
    }
}
...
fn a5_off_locus_arc_between_coincident_planes() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        Ok(StepImport::Solid { body, .. }) => {
            let v = topo::mass_properties(&body).map(|p| p.volume);
            println!("a5a LAUNDERED: t3={:?} vol={:?}", topo::validate_geometric(&body), v);
        }
        Ok(_) => panic!("wireframe"),
        Err(e) => { let s=e.to_string(); println!("a5a refused: {}", &s[..s.len().min(160)]); }
    }
}
...
fn a5_nearly_coincident_planes_refuse() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        ...
        Ok(_) => panic!("wireframe"),
        Err(e) => { let s=e.to_string(); println!("a5b refused: {}", &s[..s.len().min(160)]); }
    }
}
...
fn a4_wrong_outer_designation_refuses() {
    let text = own_fixture("die_pips")
        .replace("#39 = FACE_OUTER_BOUND('', #38, .T.);",
                 "#39 = FACE_BOUND('', #38, .T.);")
        .replace("#59 = FACE_BOUND('', #58, .T.);",
                 "#59 = FACE_OUTER_BOUND('', #58, .T.);");
    match import_step(&text, &ImportOptions::default()) {
        Ok(_) => println!("a4wo LAUNDERED: imported with wrong outer designation"),
        Err(e) => { let s=e.to_string(); println!("a4wo refused: {}", &s[..s.len().min(180)]); }
    }
}
...
fn a4_inference_on_demoted_bounds() {
    ...
    match import_step(&text, &ImportOptions::default()) {
        ...
        Ok(_) => panic!("wireframe"),
        Err(e) => { let s=e.to_string(); println!("a4inf refused: {}", &s[..s.len().min(150)]); }
    }
}
...
fn a6_unit_edges() {
    ...
    match import_step(&t, &ImportOptions::default()) {
        Err(e) => { let s=e.to_string(); println!("a6 angle refused: {}", &s[..s.len().min(110)]); }
    ...
    match import_step(&t, &ImportOptions::default()) {
        ...
        Ok(_) => panic!(), Err(e) => println!("a6 micro refused: {e}"),
    }
}
