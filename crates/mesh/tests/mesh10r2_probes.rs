...
/// **`set_edge_curve` on a split child** (disclosed by the PR as an
/// unmeasured limit). The second child of the split seam is
/// re-certified on the SAME minor circle with `u_ref` turned by φ, so
/// its stored interval shifts by −φ while its lineage stands. Then the
/// fold reads `[first.t0, last.t1]` across two parametrisations. This
/// prints what every consumer answers for φ = π/2 (a visibly wrong
/// span: every consumer refuses `props_rim_level`, typed) and for
/// φ = 1e-12 (a sub-band shift: every consumer ANSWERS, the volume
/// hundreds of ulps from the donut's, where the unsplit edge shifted
/// the same way answers bitwise).
#[test]
