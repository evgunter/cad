            ...
            for (pk, p) in m.patches.iter().enumerate() {
                for t in &p.triangles {
                    for k in 0..3 {
                        ...
                        // LINT-ONLY touch on adoption (collapsible_if);
                        // the census itself is the reviewer's, and its
                        // edge-key spelling is deliberately its own —
                        // an independent re-derivation must not share
                        // the crate's helper.
                        if a < mark
                            && bb < mark
                            && let Some(v) = seg.get_mut(&(a.min(bb), a.max(bb)))
                        {
                            v.push(pk);
                        }
                    }
