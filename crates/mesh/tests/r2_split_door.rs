//! R2 review probe: the plane-SPLIT door, which MESH-3's no-route
//! verdict does not enumerate. Cut a unit ball just below its north
//! pole; the BELOW piece is a sphere face whose boundary excludes the
//! pole with a top rim of radius `rho` — issue 896's motivating shape.
...
fn r2_split_door_near_pole() {
    ...
    for rho in [0.9 * eps, 5.0 * eps, 1e3 * eps, 1e-6, 1e-3, 0.01, 0.1] {
        ...
        match r {
            Err(e) => lines.push(format!("rho={rho:.3e}: split refused {e:?}")),
            Ok(sr) => {
