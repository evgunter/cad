...
fn a_sqrt_zero_tangent_forfeits_every_parameter_and_a_max_kink_forfeits_none() {
    ...
    assert_eq!(report.nominal.expect("this fixture measures a closed form, which has an f64 nominal"), 0.0);
...
fn where_the_linearization_says_zero_the_hull_still_encloses_the_range() {
    ...
    assert_eq!(report.nominal.expect("this fixture measures a closed form, which has an f64 nominal"), -1.0);
    ...
    assert!(
        wc.hi > report.nominal.expect("this fixture measures a closed form, which has an f64 nominal") + row.contribution.clone().unwrap(),
...
fn the_rss_sigma_of_every_distribution_form_derived_independently() {
    ...
    let report = stackup(&doc, m, &analyzed, &verdict, None, false, Tol::witness())
        .unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(report.nominal.expect("this fixture measures a closed form, which has an f64 nominal"), 4.0);
...
fn the_bore_pin_fit_as_a_consumer_reads_it() {
    ...
    assert!((report.nominal.expect("this fixture measures a closed form, which has an f64 nominal") - 0.2).abs() < 1e-12);
    let row = &report.per_param[0];
    ...
    assert_eq!(row.contribution, Ok(half));
    let wc = report.worst_case;
    assert!(wc.lo <= report.nominal.expect("this fixture measures a closed form, which has an f64 nominal") && report.nominal.expect("this fixture measures a closed form, which has an f64 nominal") <= wc.hi);
