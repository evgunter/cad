...
use editor_core::drive::{
    BudgetKind, DriveConfig, Flip, MeasureAccounting, ReasonClass, RefusalReason, drive,
};
use editor_core::{
...
fn a_flip_on_the_far_wall_names_the_second_extrudes_predicate() {
    ...
    for leaf in v.refused() {
        ...
        for f in flipped {
            if let Flip::Predicate {
                predicate,
                witness,
                leaf: l,
                ..
            } = f
                && *predicate == "extrusion_normal_component"
                && *witness == geom_core::Sign::Positive
