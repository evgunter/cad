...
//! two steps drawing one segment — with the crease's name rebound by
//! the door to the vertex the crease now sits at.
//!
...
//! document no persisted log carried a program replaced whole, no
//! replay rewrote a name in place, and no name digest was minted over
//! a selection the door had moved. Here the registry battery — the
//! persistence round trip, the replay identity, the interval lane, the
//! latency table's structural row, the name-digest goldens — runs over
//! exactly that, so the rewrite is a persisted and replayed fact rather
//! than a suite's.
//!
...
//! rebound strut stands where the old one did.
//!
//! D2 bump: the extrude's `Distance` (mid-DAG — its cone is the extrude
//! and the fillet; the frame and the profile are reused).

use editor_core::{
    DocEdit, LoopProgram, LoopProvenance, Node, ProfileProgram, ProfileVertexRef, ProgramArcData,
...
/// The crease the fillet is on, as a profile vertex of the PLAIN
/// program: the arc's end, where the rod's section meets the block's
/// top on the left.
pub const CREASE: u32 = 4;

/// The same crease after the reshaping: one vertex along, the bump's
/// one segment having been inserted before it.
pub const CREASE_RESHAPED: u32 = 5;
...
pub fn bump_provenance() -> Vec<LoopProvenance> {
    vec![LoopProvenance {
        from: Some(0),
        steps: vec![
            Some(0),
...
/// The strut edge at profile vertex `vertex` of the rod.
pub fn lateral_edge(rod: RecipeNodeId, vertex: u32) -> StableName {
    ename(
        rod,
...
pub fn document() -> CorpusDoc {
    ...
    // the reshaping below is what moves its name: the recipe records
    // the selection as it was made, and the log records the door's
    // rewrite of it.
    let fillet = r.insert(Node::fillet(
        rod,
        len(ROD_FILLET),
        vec![lateral_edge(rod, CREASE)],
    ));
