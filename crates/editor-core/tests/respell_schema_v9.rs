...
fn schema_version_is_current() {
    ...
    // `UpdateReference` edit arm, M9-1 PR-2's v11 declaration-class
    // break, then ASM-R2a's v12 `Node::Mate` arm) — the convention is
    // that a bump updates every pin it invalidates, so the number
    // stays exact here. This file keeps pinning the v8 fixture below,
