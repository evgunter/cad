...
fn real_historical_documents_refuse_typed_and_name_what_they_lack() {
    ...
    for path in &entries {
        ...
        match load(&text, Tol::witness()) {
            ...
            // The oldest shapes predate the `id:` line. Recorded
            // rather than asserted-away: this arm carries NO recourse,
            // so the very oldest documents refuse without being told
            // to regenerate.
            Err(err @ PersistError::HeaderId { .. }) => {
                let msg = err.to_string();
                assert!(msg.contains("id:"), "{name}: {msg}");
                assert_eq!(msg.matches(REGENERATE_RECOURSE).count(), 0, "{name}: {msg}");
            }
...
/// Every `Unreadable` message ends on the recourse EXACTLY once, and
/// no other arm carries it — including the arms that a valid-JSON
/// rejection can now reach through the crate's own rebuild doors.
#[test]
fn only_unreadable_carries_the_recourse_and_carries_it_once() {
    let (header, v) = split(&small());
    ...
    for text in cases {
        if let Err(err) = load(&text, Tol::witness()) {
            ...
            match err {
                PersistError::Unreadable { .. } => assert_eq!(count, 1, "{msg}"),
