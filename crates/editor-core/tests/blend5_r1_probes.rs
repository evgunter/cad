...
// ---- Probe 6: the "other direction" of the break, executed ----

/// **Probe 6 — neither direction of the break reaches serde. Both
/// die at the VERSION DOOR.**
///
/// The v18 ledger entry (`persist/mod.rs`) justifies taking a whole
/// schema number on a two-way serde break: "a v17 file whose
/// selection spells `\"Plane\"` meets a `deny_unknown_fields` role enum
/// with no such variant and dies inside serde, and a v18 file's
/// `\"Host\"` dies the same way in a v17 reader". Measured, both halves
/// name the wrong mechanism:
///
/// * A real v17 file dies `SchemaTooOld` at the door — which the
///   unit's OWN row `blend5_schema_v18::v17_refuses_too_old` asserts,
///   naming the door and the missing step. It never reaches serde.
/// * A too-new file dies `UnknownSchema { found, newest }` at the same
///   door, which this row executes by stamping a future version on a
///   real save (the path a v17 build takes on a v18 file).
///
/// What DOES reach serde is only the suite's hand-built hybrid — a
/// v18 header over a v17 spelling — which is not a file any build ever
/// wrote. The break and the number are still justified; the version
/// door is precisely what justifies them. It is the ledger's stated
/// MECHANISM that is wrong, and the ledger is the permanent record of
/// why v18 exists.
///
/// (`deny_unknown_fields` is also inert on these enums: removing it
/// from `RimSupport` leaves `the_retired_spelling_dies_inside_serde`
/// green, because an externally-tagged enum rejects unknown variants
/// unconditionally.)
#[test]
fn a_newer_file_dies_at_the_version_door_not_on_the_role_variant() {
    let doc = ProfileDoc::empty_derived("blend5-r1-too-new", Tol::witness());
    let (doc, profile) = insert(
        doc,
        Node::Profile(desc(
            [0.0; 3],
    ...
    let text = editor_core::save(&doc, &[], Tol::witness()).expect("the document saves");
    let next = u64::from(editor_core::SCHEMA_VERSION) + 1;
    let too_new = text.replacen(
        &format!("schema: {}", editor_core::SCHEMA_VERSION),
        &format!("schema: {next}"),
        1,
    );
    assert_ne!(too_new, text, "the header substitution must land");
    match editor_core::load(&too_new, Tol::witness()) {
        Err(editor_core::PersistError::UnknownSchema { found, newest }) => {
            assert_eq!((found, newest), (next, editor_core::SCHEMA_VERSION));
        }
        other => panic!("a too-new file refuses at the version door, got {other:?}"),
    }
}
