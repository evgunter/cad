...
>(
    ...
    for step in 0..rest.len() {
        ...
        // (`look_through_merges`) before the door runs, and one the fold
...
/// [`look_through_merges`] has already refused it, naming the split or
...
/// ([`look_through_merges`]). So a step that refuses `UndeclaredContact` or
