...
fn refinement_tightens_the_bound() {
    ...
    for degree in 2..=4usize {
        for _ in 0..fuzz::scaled(50) {
            ...
            for index in kv.first_span()..=kv.last_span() {
                if let Some(win) = pair.span(index) {
                    tightest = tightest.min(win.hull().width());
                }
            }
            assert!(
...
fn bounds_are_bit_identical_across_repeats_and_coefficient_types() {
    ...
    for degree in 1..=5usize {
        for _ in 0..fuzz::scaled(63) {
            ...
            let da = pf.derivative_domain_hull();
            let dc = pr.derivative_domain_hull();
            assert_eq!(da.lo().to_bits(), dc.lo().to_bits());
            assert_eq!(da.hi().to_bits(), dc.hi().to_bits());
            for index in kv.first_span()..=kv.last_span() {
                let (Some(w1), Some(w2)) = (pf.span(index), pr.span(index)) else {
                    continue;
                };
                let s1 = w1.hull();
                let s2 = w2.hull();
...
fn structural_errors_poison_rather_than_panic() {
    ...
    assert!(kv.coeffs(&coeffs[..n - 1]).is_none());
    assert!(kv.coeffs(&coeffs).is_some());
    ...
    // so is a span of another vector beside these coefficients (the
    // `compile_fail` rows on `SplineCoeffs`).
    ...
    // valid weight vector mints, and a pair minted WITHOUT weights
    // licenses no rational claim.
    let ones = vec![1.0; n];
