...
fn r2_content_squares_and_nesting() {
    ...
    row(
        "sqrt(8) - 2 sqrt(2)",
        how(s, || lit(8.0).sqrt() - lit(2.0) * lit(2.0).sqrt()),
    );
    row(
        "sqrt(12x) - 2 sqrt(3) sqrt(x), x in [1,2]",
        how(s, || {
            let x = over("x", 1.0, 2.0);
    ...
    row(
        "sqrt(12x) - sqrt(3) sqrt(4x), x in [1,2]",
        how(s, || {
            let x = over("x", 1.0, 2.0);
    ...
    row(
        "sqrt(-2x) - sqrt(2) sqrt(-x), x in [-2,-1]",
        how(s, || {
    ...
    let l = row(
        "sqrt(x²) - x, x in [-1,1] (must NOT be a theorem)",
        how(s, || {
            let x = over("x", -1.0, 1.0);
            x.powi(2).sqrt() - x
        }),
    ...
    assert_ne!(l, "theorem");
    let l = row(
        "sqrt(x²) - x, x in [1,2] (not manifest: not a theorem)",
        how(s, || {
            let x = over("x", 1.0, 2.0);
            x.powi(2).sqrt() - x
        }),
    ...
    row(
        "sqrt(sqrt(x)) - sqrt(sqrt(x)) [same node], x in [1,2]",
        how(s, || {
            let x = over("x", 1.0, 2.0);
            let r = x.sqrt().sqrt();
            r - r
    ...
    row(
        "sqrt(x²y) - |x| sqrt(y), x in [-1,1], y in [1,2]",
        how(s, || {
            let x = over("x", -1.0, 1.0);
    ...
    row(
        "sqrt(x/y) sqrt(y/x) - 1, x,y in [-2,-1]",
        how(s, || {
            let x = over("x", -2.0, -1.0);
    ...
    row(
        "sqrt(x/(1+y²)) - sqrt(x)/sqrt(1+y²), x in [1,2], y in [-1,1]",
        how(s, || {
            let x = over("x", 1.0, 2.0);
...
fn r2_source_3_is_order_dependent() {
    let s = SymRules::shipped();
    let a = row(
        "sqrt(P²/x) * sqrt(x) - |P|, P = 1+2x, x in [1,2]",
        how(s, || {
            let x = over("x", 1.0, 2.0);
    ...
    let b = row(
        "sqrt(x) * sqrt(P²/x) - |P|, P = 1+2x, x in [1,2]",
        how(s, || {
            let x = over("x", 1.0, 2.0);
            let p = lit(1.0) + lit(2.0) * x;
    ...
    row(
        "[G off] sqrt(P²/x) * sqrt(x) - |P|",
        how(SymRules::without_canonical_root(), || {
            let x = over("x", 1.0, 2.0);
            let p = lit(1.0) + lit(2.0) * x;
            (p.powi(2) / x).sqrt() * x.sqrt() - p.abs()
        }),
...
fn r2_source_3_adversary_dead_arm() {
    ...
    let control = row(
        "control: copysign(Y,1) - Y, x in [-1.5,-1.1]",
...
fn r2_source_3_adversary_earlier_decision_in_the_session() {
    ...
    let ((first, second, v), _counts) = with_session_rules(budget(), s, || {
        ...
        let first =
            geom_core::k_stats::decide("decide3_r2_first", Margin::of(x.sqrt() - x.sqrt()), band());
        let r = adversary(x) * (x + lit(1.3));
        let second = geom_core::k_stats::decide("decide3_r2_second", Margin::of(r), band());
        (
            format!("{first:?}"),
            format!("{second:?}"),
            r.value.enclosure_probe(),
        )
    });
...
fn r2_the_reads_label_and_tie() {
    let s = SymRules::shipped();
    let l = row(
        "select(x-2, y, y+1) - y, x in [1,2] (tie at 2: the value door decides, the read?)",
        how(s, || {
            let x = over("x", 1.0, 2.0);
    ...
    let l = row(
        "select(2-x, y, y+1) - (y+1), x in [1,1.5]",
        how(s, || {
            let x = over("x", 1.0, 1.5);
            let y = over("y", 3.0, 4.0);
    ...
    let l = row(
        "select(x-1.5, a, a) - a, x in [1,2] (straddles; equal arms)",
        how(s, || {
            let x = over("x", 1.0, 2.0);
            let a = over("a", 3.0, 4.0);
