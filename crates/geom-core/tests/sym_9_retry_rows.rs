//! **The retry ladder at the scalar door** (`geom_core::SymRetry`) —
//! decisions the FIRST attempt refuses, driven through
//! `sym::with_session_retry` at each shape through the public API, with
//! the receipt read.
//!
...
fn through(retry: SymRetry, build: impl FnOnce() -> Sym<f64>) -> (String, SymCounts) {
    let (out, counts) = with_session_retry(budget(), SymRules::shipped(), retry, || {
        geom_core::k_stats::decide("sym_9_retry", Margin::of(build()), band())
    });
...
#[test]
fn a_product_past_256_that_fits_at_512_is_closed_by_the_ring_retry() {
...
#[test]
fn a_product_past_512_stays_numeric_under_a_512_bit_ladder() {
    let r512 = SymRetry {
        bits: Some(512),
        ..SymRetry::kept_atom()
    };
...
/// instead of 2.
#[test]
fn a_retrys_wider_ring_does_not_leak_into_the_next_decision() {
    let retry = SymRetry {
        bits: Some(512),
        ..SymRetry::none()
    };
    let (_, counts) = with_session_retry(budget(), SymRules::shipped(), retry, || {
        let a = geom_core::k_stats::decide("sym_9_retry", Margin::of(two_spellings(5, 2)), band());
        let b = geom_core::k_stats::decide("sym_9_retry", Margin::of(two_spellings(6, 2)), band());
        (a, b)
    });
...
/// again.
#[test]
fn a_retrys_wider_ring_does_not_leak_into_the_next_session() {
    let r1024 = SymRetry {
        bits: Some(1024),
        ..SymRetry::none()
    };
...
#[test]
fn a_retrys_form_never_serves_the_first_attempt() {
    let retry = SymRetry {
        bits: Some(512),
        ..SymRetry::none()
    };
    let (_, counts) = with_session_retry(budget(), SymRules::shipped(), retry, || {
        let a = geom_core::k_stats::decide("sym_9_retry", Margin::of(two_spellings(5, 2)), band());
        let b = geom_core::k_stats::decide("sym_9_retry", Margin::of(two_spellings(5, 2)), band());
        (a, b)
    });
...
#[test]
fn a_fewer_rules_retry_closes_the_rule_e_loss() {
...
#[test]
fn a_plain_theorem_is_the_same_receipt_at_every_shape() {
    let margin = || {
        let x = p("w", 0.37);
        x + Sym::from_f64(2.0) * x - Sym::from_f64(3.0) * x
    };
    let (_, base) = with_session_rules(budget(), SymRules::shipped(), || {
        geom_core::k_stats::decide("sym_9_retry", Margin::of(margin()), band())
    });
    for (name, retry) in [
        ("none", SymRetry::none()),
...
fn the_measured_ladders_masks_are_the_shipped_set_minus_one_rule() {
    ...
    assert_eq!(
        ...
        "the rule-G attempt is the shipped set with rule G shut, as one value"
    );
    assert_eq!(
        SymRules {
            sqrt_square: true,
    ...
    assert!(!a.sqrt_square, "the rule-A attempt shuts rule A");
}
