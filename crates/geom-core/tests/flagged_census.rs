...
/// (line comments ARE stripped). Two near-misses are deliberately NOT in
/// that list, because each **fails loudly** instead of being skipped: a
/// site whose `ledger_row` is not a string literal (a `const`, a field, a
/// table entry), and a turbofish whose angle brackets do not balance. The
/// asymmetry is the rule this file is built on — a census that skips a
/// site reports a smaller tree than the one that ships, so anything it
/// cannot read it refuses to pass over.
///
