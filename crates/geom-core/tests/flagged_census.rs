...
/// generated by a macro, and a site inside a `/* … */` block comment. A
/// line comment is not in that list: a `//` line is recognised and its
/// mention skipped, before any other reading of the site.
///
...
/// anything it cannot read it refuses to pass over. The converse matters
/// as much and is why the comment check runs first: a mention the scan
/// cannot read is still only a mention, and must not stop anything.
///
