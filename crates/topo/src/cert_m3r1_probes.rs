//! CERT-M3 reviewer R1 probes — in-crate because the corruption route
//! needs `Body::surfaces` (pub(crate)): replace the NURBS wall UNDER its
//! key after the M7-8 edges were attached, so the stored certificates no
//! longer hold, then drive the body through every at-rest door.
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
...
/// CLAIM 3 — a corrupt plane × NURBS edge at f64: which at-rest door
/// still catches it.
#[test]
...
#[test]
fn m3r1_dual64_body_through_the_lane_keeping_doors() {
...
/// A digon pillow whose two faces SHARE one cylinder key with opposite
/// senses — the shape `sweep_conformal_patches` examines. Generic so it
/// can be built at `Dual64`.
fn shared_key_pillow<T: Decide>(tol: Tol) -> Body<T> {
    let c = |x: f64, y: f64, z: f64| Point3::new(T::from_f64(x), T::from_f64(y), T::from_f64(z));
    let mut body = Body::<T>::new();
    ...
    let split = body
        .mef(
            ...
            EdgeCurveSpec::line_between(c(0.0, 0.0, 0.0), c(0.0, 1.0, 0.0)),
            FaceSurface::New(cylinder),
            tol,
        )
        .unwrap();
    let chart = body.get_face(split.face).unwrap().surface;
    body.set_face_surface(seed.face, FaceSurface::Shared(chart))
        .unwrap();
    for e in [seg.edge, split.edge] {
        let spec = EdgeCurveSpec::line_between(c(0.0, 0.0, 0.0), c(0.0, 1.0, 0.0))
            .at_rest_in_chart(chart, false);
        body.set_edge_curve(e, spec, tol).unwrap();
    }
    body.flipped_face_sense_for_tests(split.face).unwrap()
}

/// CLAIM 5 — is the census's `None` arm reachable in-repo at `Dual64`
/// through the PUBLIC `validate_pseudomanifold` door (not through
/// `AtRestPolicy`)?
#[test]
fn m3r1_census_lane_absence_reached_at_dual64_through_the_public_door() {
    ...
    let lane_absent = matches!(&d, Err(v) if v.iter().any(|e| matches!(e, ValidationError::CensusLaneUnsupported { .. })));
    eprintln!("[m3r1] CensusLaneUnsupported reached at Dual64 via the public door: {lane_absent}");
    // Tier 3 (LaminaWedge) stops the public door before the census on
