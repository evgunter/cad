...
fn body_with_cycle_ring() -> (Body<f64>, LoopKey) {
    ...
    body.mev(MevSite::Lone { r#loop: kill.ring }, p(3.0))
        .unwrap();
    assert_eq!(validate(&body), Ok(()));
    (body, kill.ring)
}
...
fn some_single_op_reaches(body: &Body<f64>, target: &str, coords: &[Point3<f64>]) -> bool {
    ...
    for &he1 in &halves {
        ...
        for &he2 in &orbit {
            for &c in coords {
                let mut probe = body.clone();
                if probe.mev(MevSite::Fan { he1, he2 }, c).is_ok() {
    ...
    for &l in &empties {
        for &c in coords {
            let mut probe = body.clone();
            if probe.mev(MevSite::Lone { r#loop: l }, c).is_ok() {
                candidates.push(probe);
            }
        }
        let mut probe = body.clone();
        if probe.mef(MefSite::Lone { r#loop: l }).is_ok() {
            candidates.push(probe);
        }
    }
    ...
    for (_, loop_data) in body.loops() {
        ...
        for &he1 in &cycle {
            for &he2 in &cycle {
                let mut probe = body.clone();
                if probe.mef(MefSite::Chords { he1, he2 }).is_ok() {
                    candidates.push(probe);
                }
            }
        }
...
fn kef_mate_alone_has_a_single_op_remake_when_survivor_is_bare_outer() {
    ...
    let remake = body.mef(MefSite::Chords { he1: b, he2: b }).unwrap();
    assert_eq!(validate(&body), Ok(()));
    assert_eq!(
        canonical_form(&body),
        before,
...
fn hexagon_pillow_with_fully_degenerate_coordinates_is_stable() {
    ...
    let same: Vec<Point3<f64>> = (0..6).map(|_| pt(7.0, 7.0, 7.0)).collect();
    let a = ngon_pillow(&same);
    let b = ngon_pillow(&same);
...
fn oracle_distinguishes_ring_attachment_even_at_shared_coordinates() {
    ...
    let build = |split: bool| {
        ...
        let plant = |body: &mut Body<f64>, at: HalfEdgeKey| {
            let strut = body
                .mev(MevSite::Fan { he1: at, he2: at }, pt(9.0, 9.0, 9.0))
                .unwrap();
            body.kemr(strut.he_plus, strut.he_minus).unwrap()
        };
...
fn failing_kill_calls_consume_no_keys_between_kills() {
    let run = |with_failures: bool| {
        ...
        if with_failures {
            ...
            assert!(body.mfkrh(LoopKey::default()).is_err()); // stale
        }
        let strut = body
            .mev(
                MevSite::Fan {
        ...
        body.kev(strut.he_plus).unwrap();
        if with_failures {
            assert!(body.mfkrh(seed.r#loop).is_err()); // outer, RingIsOuter
...
fn kvfs_slot_recycling_is_generation_safe() {
    ...
    assert_eq!(
        body.mfkrh(first.r#loop).unwrap_err(),
        EulerOpError::StaleKey {
...
fn kill_ops_survive_torn_bodies_without_panicking() {
    ...
    let foreign = {
        ...
        let sg = other
            .mev(MevSite::Lone { r#loop: s.r#loop }, p(1.0))
            .unwrap();
        sg.he_plus
    };
...
fn same_face_self_loop_bridge_has_no_direct_killer_but_mfkrh_frees_it() {
    ...
    let promoted = body.mfkrh(circ.r#loop).unwrap();
    let _ = promoted;
    body.kef(circ.he_minus).unwrap();
    assert_eq!(validate(&body), Ok(()));
    assert_eq!(
