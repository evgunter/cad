...
//!
//! **[`crate::Body::revert`] re-states the map with the frames** — a
//! fourth position rather than one of the three, and outside the
//! guard's walk because the door takes `&self` and returns a new body.
...
//! [`geom_brep::PcurveCache::mirrored_v`], whose docs carry why the
//! certificate is still the certificate. What the reversal does NOT
//! re-state is the loop walk's branch choice: where a forward walk
//! parked a periodic chart's one-period wrap at a loop's closure, the
//! wrap sits mid-chain once the loop runs the other way, and tier 3
//! of the reverted body reports it as a `LoopDiscontinuity` —
//! `sweep`'s SHELL-9 probe rows measure that on the two-arc sphere's
//! reverted cavity, and measure the drum's reverted cavity reporting
//! nothing but `NegativeVolume`. The wrap is the producer's closing
//! mint's to re-derive, as it does today.
//!
    ...
    pub(crate) const DECLARED: &[(&str, Posture, &str)] = {
        ...
        &[
            ...
            (
             ...
             module docs' fourth position) and grafts through `boolean::combine`, \
