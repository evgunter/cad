...
//!
//! `probe_g` is the one that matters: a container with a cavity and a
//! post threaded through it share 300 m^3 of material, every contact
//! between them is a true declared vertex-on-face rest, and the
//! certifying door answers `Ok(())`.
...
/// the L-bracket's own declaration shape — leaves the census with
/// nothing to say and the door certifies. At the merge base the same
/// body refused `CensusUndecidable` "one instance's extent box inside
/// another's".
#[test]
fn probe_g_a_post_through_the_container_certifies() {
    ...
    // Undeclared: sixteen touches and NO placement finding — the arm
    // cleared a pair that shares 300 m^3 of material.
    let undeclared =
        validate_pseudomanifold(&body, &ContactRecords::default(), Tol::witness()).unwrap_err();
    assert!(placement_findings(&undeclared).is_empty(), "{undeclared:?}");
    assert!(
        undeclared.iter().all(|e| matches!(
            e,
        ...
        "only touches: {undeclared:?}"
    );

    // Declared, every record true: the interference fit CERTIFIES.
    assert_eq!(
        validate_pseudomanifold(&body, &records, Tol::witness()),
        Ok(()),
        "this should be an InstanceInterference"
    );
}
...
/// strictly OUT of `m`, so the `m`-as-container ordering would clear.
/// Only the other ordering decides, and it does. Nothing in the unit's
/// own suite goes red if that ordering is deleted; this row does.
#[test]
fn probe_a_only_the_reverse_ordering_decides() {
    ...
    let errors =
        validate_pseudomanifold(&body, &ContactRecords::default(), Tol::witness()).unwrap_err();
    let [ValidationError::InstanceInterference { outer, inner, .. }] = &errors[..] else {
        panic!("the reverse ordering's verdict: {errors:?}");
    };
    assert_eq!((*outer, *inner), (us, ms), "{errors:?}");
}
...
/// The verdict therefore depends on `u`'s vertex ARENA ORDER, not on
/// the geometry: the outer shell happens to come first, so the arm
/// reports the interference. `probe_g` is the same shape with the
