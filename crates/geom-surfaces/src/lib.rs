...
//! machinery (iso-curve extraction, which would create the dependency,
//! belongs to the EdgeGeometry layer — M2 PR 3).
//!
//! # Surface conventions (normative, stated once)
//!
//! All the curve-side conventions carry over (units in meters/radians,
//! D6; entities are **complete loci** — the infinite plane, the full
//! cylinder, both cone nappes; evaluators never range-reduce — the
//! periodicity and bit-identity policy is word-for-word `geom-curves`'s
//! crate-doc statement, not repeated here). The surface-specific rules:
//!
...
//!   All frame fields are conventional data per D2 — `u_ref` carries
//!   the **seam** (where u = 0 lives) — and are unit/orthogonal **by
//!   convention, unchecked** (same posture and same rationale as
//!   `geom-curves`: no hidden renormalization, tier-3 certification
//!   owns the invariant at rest).
