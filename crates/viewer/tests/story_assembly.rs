...
fn the_windmill_story() {
    ...
    let hub_bottom = common::face_at(&session, &index, &asm::up_at(HUB_PARK[0], HUB_PARK[1]));
    assert_eq!(hub_bottom.node, hub_i, "the parked hub is picked");
    let tower_top = common::face_at(&session, &index, &asm::down_at(0.0, 0.0));
    ...
    let front_wall = common::face_at(&session, &index, &common::along_y(1.0, 0.0, wall_z));
    assert_eq!(front_wall.node, hub_i, "the seated hub's front wall");
    let back_wall = common::face_at(&session, &index, &common::along_y(-1.0, 0.0, wall_z));
