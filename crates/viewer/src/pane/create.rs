...
use crate::forms::{
    ...
    PatternKindChoice, SHAPE_LOCKED, ShapeEdits, ShapeKind, UNIT_DRAG_SPEED, boolean_op_label,
};
use crate::frame;
...
impl ViewerBehavior<'_> {
    ...
    pub(crate) fn add_profile_ui(&mut self, ui: &mut egui::Ui) {
        *self.profile_form_drawn = true;
        ...
        let refused = preview_verdict(
            ui,
            self.theme,
            self.profile_preview.as_ref().filter(|_| !at_rest),
        );
        if ui
            .add_enabled(
    ...
    /// drives): the caller shows the slot rows instead, which can.
    pub(crate) fn edit_profile_ui(&mut self, ui: &mut egui::Ui, node: RecipeNodeId) -> bool {
        ...
        *self.profile_edit_drawn = true;
        ...
        let refused = preview_verdict(ui, self.theme, self.edit_preview.as_ref());
        ...
        ui.horizontal(|ui| {
                ...
                .clicked()
            {
                match edit.programs(notation) {
                    Ok(loops) => self.ops.push(SessionOp::EditProfile { node, loops }),
                    Err(error) => self
                        .notices
