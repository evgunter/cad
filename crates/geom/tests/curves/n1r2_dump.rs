...
fn n1r2_dump_curve_evaluators() {
    let Ok(path) = std::env::var("N1R2_DUMP") else {
        return;
    };
    ...
    for (name, c) in curves3() {
        ...
        for t in params(kv) {
            ...
            let mut row = vec![
                p.x, p.y, p.z, d1.x, d1.y, d1.z, d2.x, d2.y, d2.z, pd.x.value, pd.y.value,
                pd.z.value, pd.x.deriv, pd.y.deriv, pd.z.deriv, dd.x.value, dd.y.value, dd.z.value,
