...
fn n1r2_curve_interval_lift_encloses_source_on_adversarial_nets() {
    ...
    for (name, c) in curves3() {
        ...
        assert!(matches!(&ei, Curve3::Nurbs(n) if !n.is_placeholder()), "{name}");
        for t in params(c.knots()) {
            ...
            for (label, li, s) in [
                ...
                for (l, r) in [(li.x, s.x), (li.y, s.y), (li.z, s.z)] {
                    assert!(contains(l, r), "{name} t={t} {label}: [{}, {}] must contain {r}", l.lo(), l.hi());
                    let w = (l.hi() - l.lo()) / (1.0 + r.abs());
                    worst = worst.max(w);
                    assert!(width_ok(l, r, 1e-6), "{name} t={t} {label}: width {w:e} too wide");
                }
            }
        }
    }
...
fn n1r2_curve2_interval_lift_encloses_source() {
    for (name, c) in curves2() {
        ...
        for t in params(c.knots()) {
            ...
            assert!(width_ok(p.x, q.x, 1e-9) && width_ok(p.y, q.y, 1e-9), "{name} t={t}");
        }
    }
}
...
fn n1r2_surface_interval_lift_encloses_source_on_adversarial_nets() {
    for (name, s) in surfaces() {
        ...
        assert!(matches!(&ei, Surface::Nurbs(n) if !n.is_placeholder()), "{name}");
        for u in params(s.knots_u()) {
            for v in params(s.knots_v()) {
                ...
                for (l, r) in [(p.x, q.x), (p.y, q.y), (p.z, q.z)] {
                    ...
                    assert!(width_ok(l, r, 1e-6), "{name} ({u},{v}) eval width {:e}", l.hi() - l.lo());
                }
                let du = ei.deriv_u(ui, vi);
                let dv = ei.deriv_v(ui, vi);
                let fu = e.deriv_u(u, v);
                let fv = e.deriv_v(u, v);
                for (l, r) in [(du.x, fu.x), (du.y, fu.y), (du.z, fu.z), (dv.x, fv.x), (dv.y, fv.y), (dv.z, fv.z)] {
                    assert!(contains(l, r), "{name} ({u},{v}) deriv containment [{}, {}] vs {r}", l.lo(), l.hi());
                    assert!(width_ok(l, r, 1e-6), "{name} ({u},{v}) deriv width {:e}", l.hi() - l.lo());
                }
                let n = e.normal(u, v);
                let ni = ei.normal(ui, vi);
                for (l, r) in [(ni.x, n.x), (ni.y, n.y), (ni.z, n.z)] {
                    // The normal normalises (a sqrt), so only containment.
                    assert!(contains(l, r) || (r.is_nan() && l.is_poison()), "{name} ({u},{v}) normal");
                }
            }
        }
    }
...
fn n1r2_placeholder_and_poison_at_interval_and_dual_interval() {
    ...
    assert!(cdi.eval(Dual::variable(Interval::from_f64(0.5))).x.value.is_poison());
    ...
    for (name, c) in curves3() {
        ...
        for t in params(c.knots()) {
            ...
            for (l, r) in [(a.x, b.x), (a.y, b.y), (a.z, b.z)] {
                assert_eq!(l.value.lo().to_bits(), r.value.lo().to_bits(), "{name} t={t}");
                assert_eq!(l.value.hi().to_bits(), r.value.hi().to_bits(), "{name} t={t}");
                assert_eq!(l.deriv.lo().to_bits(), r.deriv.lo().to_bits(), "{name} t={t}");
                assert_eq!(l.deriv.hi().to_bits(), r.deriv.hi().to_bits(), "{name} t={t}");
            }
        }
        // And the enum composition equals the direct payload composition.
        let e = Curve3::Nurbs(Arc::new(c.clone()));
        let ec: Curve3<Dual<Interval>> = e.map_scalar(Interval::from_f64).map_scalar(Dual::constant);
