...
// Adoption annotation (CERT-N2 fix pass), not the reviewer's: the
// shape table below is a `[(&str, fn(f64) -> Point3<f64>); 6]`, which
// `type_complexity` denies inside the aggregated test binary. The row
// is kept as written.
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
