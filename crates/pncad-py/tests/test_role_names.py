...
def axis_of(doc, frame):
    ...
    return doc.insert(Node.datum_axis_in_plane(frame, (0 * m, 0 * m), (0.0, 1.0)))
...
def ring(doc):
    ...
    return doc.insert(Node.revolve(profile, axis_of(doc, frame), 2 * math.pi * rad))
...
class TestTheDoorAnswersTheKernelsOwnText(unittest.TestCase):
    ...
    def test_carried_is_the_name_a_survivor_of_the_next_op_wears(self):
        ...
        hollow = doc.insert(Node.shell(node, T * m, [band(node, 0, 2)]))
        ev = evaluate(doc)
        survivors = ev.select(
            hollow,
            Selector.of(
...
class TestASelectionAuthoredBeforeAnyEvaluation(unittest.TestCase):
    ...
    def test_a_fillet_on_rims_named_before_the_revolve_ran(self):
        ...
        node = ring(doc)
        rolled = doc.insert(
            Node.fillet(node, ROLL * m, [band_rim(node, 0, 2), band_rim(node, 0, 3)])
        )
        ev = evaluate(doc)
