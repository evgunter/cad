...
## Re-cut 2026-09-04 (CHROME) — what is still Ev's, and what stopped being

**This section is the live statement of the item.** The checklist above
is kept because three of its sections are still open verbatim, but two
of its premises have moved and one of its sections is closed.

**Closed by CHROME's slate.** §4's "nothing in `viewer::gpu` has ever
executed" is no longer true of CONSTRUCTION. PR 1755 landed
`gpu::tests::every_pass_builds_on_a_real_device`, which creates a
device on a software adapter and constructs every pass in the module on
every toolkit-true CI run, asking `egui_wgpu` for the same device
descriptor the running app requests. So pipeline creation, shader-module
compilation and the whole family of "the pipeline refuses to build"
failures are gated now, and the incident that opened this item could not
ship again unseen. What still escapes is everything downstream of a
FRAME — buffer and texture allocation, render-pass encoding, and the id
pass's readback — because each needs a frame rather than a constructor.

**Closed by the hardening PR the 2026-08-28 comment promised.** `run()`
builds an explicit `ViewportBuilder` rather than the bare
`NativeOptions::default()` that comment names, so the resize finding is
addressed at the door it entered by.

**§2, the culling flip, has grown a pipeline since it was written.** The
item says TWO pipelines must change together (shaded and id). There are
now **three** — `cull_mode: None` at `crates/viewer/src/gpu.rs:468`,
`:738` and `:844` — because the edge pass landed after this item was
filed, and it draws triangles now rather than a `LineList`. Turning
culling on in two of three would make the id buffer and the picture
disagree about which faces exist, which is exactly the hazard the
original §2 names, so the instruction stands and its count does not.

**One question §2 does not ask and now should.** An edge mark is a
camera-facing quad rather than part of a closed solid, so back-face
culling may be wrong for that pass on its own terms — the flip is one
decision for the two solid passes and a separate judgement for the edge
pass. Whoever runs it should decide those separately rather than
setting three flags together.

**Still Ev's, unchanged and only settleable on hardware:**

1. **The culling flip** (§2) — set `Some(Face::Back)` with
   `FrontFace::Ccw`, run, and if the plate vanishes the answer is `Cw`.
   Record which it was in the module comment at
   `crates/viewer/src/gpu.rs:60-68`, which names this issue and is
   waiting for the answer.
2. **`R32Uint` clear semantics** (§4, failure 2) — the id pass clears
   with `LoadOp::Clear(Color::TRANSPARENT)` expecting zero
   (`gpu.rs:623`). lavapipe clearing a uint target to zero says nothing
   about another driver. The symptom is the disagreement message over
   EMPTY space.
3. **The blocking readback's cost** (§4, failure 3) — a software
   adapter's frame rate is not the signal. The remedy §4 named is in
   place: the query is gated on cursor movement (`frame::IdQueryLog`),
   so what is left is whether the cost bites on a real driver.
