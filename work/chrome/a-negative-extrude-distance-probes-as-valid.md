...
Evaluating the document at four thicknesses, failing nodes each time:

| thickness | failing nodes |
| --- | --- |
| -8 mm | `{}` |
| -1 mm | `{}` |
| 0 | `{RecipeNodeId(2)}` (the extrude) |
| +8 mm | `{}` |

**So a length field's floor is the single point `0`.** A negative
distance builds — presumably extruding the other way — and the probe,
whose oracle is "no failure the baseline did not have", reports it
valid. Forcing the probe's seed to one metre gives
`low: Open { probed: -2047.992 }`: the search walks two kilometres
into negative thickness and reports the whole span as nothing-new-
fails.
...
2. **A bracket around it is an arithmetic accident.** A direction
   brackets a single-point failure only when a doubling lands exactly
   on it. Measured on this document: a 1 mm seed brackets (the floor
   is 8 seeds out, and 8 is a power of two); an 0.8 mm seed reports
   `low: Open { probed: -1.6304 }`; a 1 m seed reports
   `Open { probed: -2047.992 }`. Any row asserting a bracket on a
   length field rests on that coincidence, which is what
   `work/chrome/probe-rows-assert-in-one-direction-only.md`'s
   discharge note now says out loud.
