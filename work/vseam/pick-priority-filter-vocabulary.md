...
`PickKinds` is two answers to one question, not a vocabulary. `crates/viewer/GUI-DESIGN.md` GQ7 still owns:
...
slate. `crates/viewer/GUI-DESIGN.md` GQ7 ratifies the deferral in as many
...
exist and is not scheduled, and `crates/viewer/GUI-DESIGN.md` GQ7 ratifies
...
Viewer/GUI ground: GAUTH's closing entry names this issue as its residue and `crates/viewer/GUI-DESIGN.md` GQ7 owns the open question, but both GAUTH and GUI are closed programs, so it lands in `work/issues/`.
...
The ratification this row cites is `crates/viewer/GUI-DESIGN.md` GQ7, which
