...
Moved from `work/issues/` to `work/eval/` in the tracker-wide cut of 2026-09-06 (Ev's direction, in-chat), which read every open `work/issues/` file and every open code-quality row against every live program's `paths` and opened four programs for the ground none covered. Id, body and header are unchanged except as noted; the directory is the claim (`work/README.md`). Status set to `deferred`: items (2)/(3) were ratified as not-now by VERB-SEAT-DESIGN §5 (now `crates/verbs/README.md` §5, "Out of scope, recorded"), deferred as one future conversation whose trigger is a real replay consumer. Item (1) and the cheap fix landed with SEAT (DOC-LEDGER sweep 8). EVAL holds the row because `crates/verbs` and the eval seat are where that consumer would arrive.
...
Its class at the cut was **H** — items (2)/(3) are an unratified design
round on kernel identity; waits for a replay consumer. The class is a
