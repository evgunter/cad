...
table), outside that unit's fence, which claims no editor-core paths.
`crates/editor-core/src/checks.rs` is no program's declared path;
`H5` §RATIFIED ruling 3 puts editor-core's lane-shaped traits on
WIRE's and PROPS' slates, and this one is a lane-shaped trait over the
evaluation service's checks, so it sits here.
