# EDIT — the document model's residue (plan)

**STATUS: OPEN (2026-09-13).** Opened at DOCM's exit sweep as its
successor (`docs/DOC-LEDGER.md` sweep 14; DOCM's done-state of record
is its exit walk at the sweep SHA). Live state is `work/edit/log.md`'s
tail and the item files beside this plan, never this file.

Branch prefix: **`edit/`** — unit branches `edit/<unit>-<slug>`,
orchestrator branch `edit/orchestrator`. Away-channel tag
`(EDIT orchestrator)`. A/B ordinal band **EDIT = 4800–4899**.

## Charter

Own what DOCM owned — the persisted recipe vocabulary, the `DocEdit`
...
Everything DOCM's keep_out named as another program's, restated in
`program.md`: WIRE's evaluation seat, MSOLVE's mates, PROPS's analysis
lane, LIB's façade, CHROME's and VIEW's viewer, S-BOOL's booleans.

## Exit shape

Every row above merged or re-homed with a reason, the three design
questions ruled and their rulings recorded beside the code; the walk
convention applies.
