...
| `persist/check.rs` — `validate_document` | `SnapshotError::*`, `NonFiniteSite`, `DistributionFault`, `DisplayUnitRefusal` | The original sweep's ground; its three hits are `three-door-predicates-are-hand-copied-not-shared`, built on this branch. |
