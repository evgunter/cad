...
(`S176`'s finding, and already the convention `docs/CODE-QUALITY-CONVENTIONS.md`
...
| `d321-row-number-reissued` | **M** | Needs a retired-id rule plus citation disambiguation; docs only, no code. | `docs/CODE-QUALITY-CONVENTIONS.md` (numbering rules); the `SMELL-*-LOG.md` half is archive-only since sweep 11 (below); possibly `scripts/work.py` lint |
...
| `S176` | **H** | Register-wide citation sweep plus a standing convention; blank Ev verdict | `work/**/*.md` and `docs/SMELL-*.md` citations (cite-by-name sweep), convention text in `docs/CODE-QUALITY-CONVENTIONS.md` / `docs/prompts/*` |
...
Then `d321-row-number-reissued`, whose shape **changed at sweep 11**
(`docs/DOC-LEDGER.md`, 2026-09-11): its live half is the retired-id rule,
which lands in `docs/CODE-QUALITY-CONVENTIONS.md` — the relocated
numbering rules — plus a lint rule filed on META. Its other half asked
for citation disambiguation inside `work/code-quality/logs/SMELL-T-LOG.md`
and `SMELL-KPW-LOG.md`, and those files left the tracker with the
directory; they are recoverable at the SHA the sweep names but there is
nothing live to edit, so that half closes as overtaken rather than done.
`S176` is in the same position for the `SMELL-G-LOG.md` census it cites.
