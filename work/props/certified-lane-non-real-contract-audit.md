...
(2026-09-05, PROPS rotation-floor.) Member 5 is decided — no respell — and the decomposition above has one home now, the width-floor paragraph at `Mat3::rotation_about` (`crates/geom-core/src/linalg/mat.rs`); the ruling is `work/props/rotation-about-diagonal-width-floor.md`, and the composition rider is its own file, `work/issues/mapped-curve-restrict-composes-placements-per-split.md`.

---
_Generated by [Claude Code](https://claude.ai/code)_

## Disposition of the CERT-3 lost-correlation batch

The 2026-08-30 comment's four members, tracked here rather than by
editing the comment transcript above:

- **Member 1, `frame::mirror_across_plane`** — CLOSED by PROPS-1. The
  translation mentions the anchor once (`n̂·(2·(n̂·q))`).
- **Member 2, `Mat3::rotation_about`'s `1 − cos` diagonal floor** — its
  own decision, sized against ~a sixth of the residue per the S-CERT
  correction beneath the batch comment, and carrying the
  `MappedCurve::restrict` composition-growth rider as one caseload
  entry. Not PROPS-1's.
- **Member 3, `Vec3::reject_from`** — CLOSED by PROPS-1. The rejection
  is the triple product `(onto × self) × onto / |onto|²`, which mentions
  `self` once. `onto` is still mentioned three times: that half is
  unrecovered and stated in the method's doc.
- **Member 4, `Point2::lerp` / `Point3::lerp`** — DECIDED AND LEFT. The
  one-difference form stays; the `Interval` cost of the chosen form is
  recorded at the site, in the paragraph beginning *"Its cost at an
  enclosure scalar"* on each of `Point2::lerp` and `Point3::lerp`
  (`crates/geom-core/src/linalg/point.rs`).
