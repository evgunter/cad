...
## Closed as a duplicate (2026-09-24)

The same question as `work/port/min-clearance-refusal-stringly-twin.md`,
answered there by PR #3188: `MinClearanceRefusal` is deleted, and the door
(`MinClearanceLane::min_separation`) and `NodeErrorKind::MeasureClearanceRefused`
carry `clearance::ClearanceRefusal` itself, so a caller matches the enum.
