---
id: carve
kind: program
title: CARVE — what a sweep verb builds and how it describes it
status: open
opened: 2026-09-17
area: kernel
prefix: carve/
tag: (CARVE orchestrator)
ab_band: 5600-5699
paths: [crates/sweep/src/blend/*, crates/sweep/src/revolve/*, crates/sweep/src/extrude.rs, crates/sweep/src/fillet.rs, crates/sweep/src/chamfer.rs, crates/sweep/src/skin.rs, crates/sweep/src/swept.rs, crates/sweep/src/loft.rs, crates/sweep/src/test_support.rs, crates/sweep/src/lib.rs, crates/sweep/README.md]
keep_out: [OPENED 2026-09-17 in BLEND's cut as its one successor on BLEND's ground - BLEND stays open until its exit walk with its three unit rows and its paths still cover every path above so every path here is BLEND's too by declaration and BLEND's keep_out names this program in return, this program does not dispatch a unit that touches a file one of BLEND's three open units (13 S90-impl, 14 blend/14-contact-edge-must-carry, 15 blend/15-recourse-roster) is landing on until that unit merges, crates/sweep/tests/* and crates/test-utils/* are S-TCOST's and S-TINT's by declaration - units add rows there as ordinary tests and a test-mechanism change is announced to both, crates/profile/* is PATHS' - the profile fillet door's rows moved there in the same cut and a fix on that side is filed on PATHS and never diffed from here except by announced seam, geom-brep certify.rs and dihedral.rs and geom-core predicate.rs are PROPS-adjacent - the must-carry wrapper and the TangentParallel margin and the gap-sentence newtype are announced to PROPS, topo/src/query.rs is unowned and TOPO edits it as its own ground - rim_of's row moved there, editor-core verbs/blend.rs and names/emit_*.rs are EVAL's ground and a name-emitter row found here is filed there, the S14 ruling holds the typed-error announcement sites in blend/ - they stay typed errors while it is open]
---

BLEND's one successor, opened by BLEND's cut of 2026-09-17 on the
part of BLEND's slate whose subject is **what a sweep verb builds and
how it describes what it built**: the blend surgery's phases and their
records, the verbs' description rule at every site that mints a stored
description, the loft's parameterisation and its section checks, and
the fixtures the crate's own suites build those bodies from.
Twenty-one rows arrived from `work/blend/`, each by `git mv` with its
body, its id and its history unchanged.

Charter and order: `work/carve/plan.md`; narrative in
`work/carve/log.md`. Review posture is BLEND's, inherited: the full v6
dual on a kernel unit, drawing from 5600 on; E-class rows take a single
style review and no A/B row.
