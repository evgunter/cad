...
--selftest` runs at `ci.yml:1459` and `local-scripts/ci-local.sh:464`, and
nothing runs it inside `nightly.yml`'s `opt-level` job, whose `record` step
(`nightly.yml:1277`) appends a sample to `docs/perf-data/opt-level/`.
...
2. **Delete the criterion job's copy instead**, and say that a guard proved by
   ...
   would re-prove a tree this row already proved"* (`ci.yml:1450-1451`) — and it is
