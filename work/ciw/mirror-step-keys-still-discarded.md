...
(`scripts/check-ci-mirror-parity.py:856`) leaves two keys whose value decides
...
1. **`continue-on-error:` on a STEP.** A hosted step that may fail without
   ...
   step-level one is not. Four live sites, `.github/workflows/ci.yml:3238`,
   `:3274`, `:3725`, `:3751` — **none of them on a cited pair** (two are the
