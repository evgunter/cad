...
(`scripts/check-ci-mirror-parity.py:811`) leaves two keys whose value decides
...
1. **`continue-on-error:` on a STEP.** A hosted step that may fail without
   ...
   step-level one is not. Four live sites, `.github/workflows/ci.yml:3205`,
   `:3241`, `:3692`, `:3718` — **none of them on a cited pair** (two are the
