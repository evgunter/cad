...
S-CERT: the conversation's `compose::tensor` twin is the program's rational-flux ground (`crates/geom-brep/src/props/*`), and PCURVE — the program that executed the ruling — is closed. Closed on migration: the ruling this conversation asked for was answered as U2 and ratified into `docs/PCURVE-UNIFY-DESIGN.md`, whose execution is the closed PCURVE program's done-state of record (its exit walk, recorded in `docs/DOC-LEDGER.md`).
