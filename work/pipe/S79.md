...
## Two of three rows closed (2026-09-12, TOPO orchestrator)

`#758` closed with TOPO's census door (PR 2131,
`work/topo/no-public-census-or-genus-query`: `readback::euler_counts`
and `EulerCounts::genus`). `#759`'s item
(`work/lib/facade-polygon-door-demoted-without-replacement`) closed on
2026-09-08. `blocked_on` drops both; this row waits on `#757` alone.
