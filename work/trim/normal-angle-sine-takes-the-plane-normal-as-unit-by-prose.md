---
id: normal-angle-sine-takes-the-plane-normal-as-unit-by-prose
kind: issue
title: edge_nurbs.rs normal_angle_sine takes the plane's normal as unit by prose — carrier-derived, so no caller holds the witness yet
status: open
opened: 2026-09-15
---
...
`crates/geom-brep/src/edge_nurbs.rs`, `normal_angle_sine` (`:580`):
"`|n̂ × m̂|` — the sine of the angle between the plane's exact unit
normal and the wall's normal". `plane_normal` is `Surface::Plane`'s
stored `normal` (the carrier case); `wall_normal` is normalized inside
by its own norm, so only the first parameter is in the class. A
non-unit `plane_normal` scales the sine every comparison downstream
reads.
