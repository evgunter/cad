...
Moved from `work/issues/` to `work/props/` in the tracker-wide cut of 2026-09-06 (Ev's direction, in-chat), which read every open `work/issues/` file and every open code-quality row against every live program's `paths` and opened four programs for the ground none covered. Id, body and header are unchanged except as noted; the directory is the claim (`work/README.md`). The `TangentParallel` margin is `crates/geom-brep/src/certify.rs`'s, in no program's `paths`; it needs the K-REPORT runbook for a new margin's distribution, which is PROPS' instrument. PROPS draws the fence on `certify.rs` in the PR that takes it; BLEND (whose review found it) is announced.
