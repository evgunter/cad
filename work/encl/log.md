...
Plan: `work/encl/plan.md`. A/B band 5800-5899
...
**Your files:** `offset_meters.rs` (`MeterError`, 2 of 3 arms),
`patch_bound.rs` (`PatchBoundError`, 5 of 7), and `offset_fit.rs` (the
wrapper row, whose four delegating arms are now asserted TRANSITIVELY
because all four carriers hold enforcement rows of their own).

`Degree1Crease` and `Crease` were **left exactly as they were** — they
already say to split the face at the crease, and the new row asserts
them as they stand. Rewriting an arm that is already right is the loss
this class keeps warning about.

**A row filed on your slate while reading:**
`offset-fit-reuses-derivedknots-for-a-degree-elevation-failure` —
`Composite::build` reports a failed degree ELEVATION as
`PatchBoundError::DerivedKnots`, so the message describes a derivative
knot vector that was never built. Not actively wrong today (the new
recourse clause is true of both readings) but the variant names the
wrong operation.
