...
`session::Gesture` (`crates/viewer/src/session.rs:162`) and
...
`display.rs:629-630` ("the no-move rule the document gestures follow")
and `session.rs:1086-1088` ("Same rule as a no-move commit").
