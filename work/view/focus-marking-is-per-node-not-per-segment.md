...
So the unit is: establish the authored-step → canonical-segment map as a door with its own rows (it belongs beside the lowering, in `profile`/`editor-core`, not in the viewer), then narrow `marks::focus` through it. Two consequences worth deciding at the same time:
...
Not a defect of what shipped — the shipped behaviour is stated as a gap in the module docs of `marks::focus` and in the GUI log — but it is the natural next step and the request that produced the feature asked for it by name.
