...
So the unit is: establish the authored-step → canonical-segment map as a door with its own rows (it belongs beside the lowering, in `profile`/`editor-core`, not in the viewer), then narrow `pickindex::focus` through it. Two consequences worth deciding at the same time:
...
Not a defect of what shipped — the shipped behaviour is stated as a gap in the module docs of `pickindex::focus` and in the GUI log — but it is the natural next step and the request that produced the feature asked for it by name.
...
Viewer ground (`crates/viewer/src/pickindex.rs`, where `focus` is `:2076` since the seam split) with a lowering-side door in `profile`/`editor-core`; the GUI program is closed and may hold only closed items, so it lands under `work/issues/`.
