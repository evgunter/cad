...
**What runs.** Tier 3 check 1 calls `geom_brep::OffsetFitLane::recertify` on every `Surface::Approx` face, every validation call. That is `geom_brep::certify_offset`: both door meters (`meter_patch`) plus a two-limb `measure` over the span schedule — a `(u,v)` GRID, against the line schedule an edge carrier pays.
