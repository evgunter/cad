...
Plan: `work/suite/plan.md`.

## Opened (2026-09-11)

Opened in the tracker cut of 2026-09-11 (Ev's direction, in-chat;
`docs/WORK-TRACKS-2026-09.md` addendum 3). Eight rows moved in by
`git mv`, each with a `## Re-homed` record: one from `work/issues/`,
seven from `work/code-quality/`.

The honest alternative was to file all eight onto `work/tint/` and open
nothing, since S-TINT owns every path they touch. The cut kept them as a
track because the eight share a fix shape rather than a file, and
S-TINT's board is the test-integrity charter rather than a
duplication-removal one; the cost is that this program can never claim a
path, which `plan.md` says in as many words.
