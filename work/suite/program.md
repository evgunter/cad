...
copied fixtures over six crates, a part-resolver stub copied across
editor-core's mate and assembly suites, the generic `cube<T: Real>` two
rows have been describing from opposite sides, and the differential
probe `D114` wants. Every row is on S-TINT's
ground and every unit is announced there — this program is a slate, not
a second claim on `crates/*/tests/*`. Class M throughout with one H.
Charter and unit order: `work/suite/plan.md`.
