# STACK — the log

## 2026-09-20 — opened

Cut out of ENCL, which was carrying 40.5 budget points, when Ev
ratified the priority and track-size conventions in chat the same day.
The cut followed ENCL's PRIORITY seam per `work/README.md` "Track
size", into several tracks at once so they can run in PARALLEL — Ev, in
chat: *"for these high priority tracks it's ideal to have several
components that can be worked on in parallel."*

4 rows arrived by `git mv` with their ids, bodies and history
unchanged. ENCL keeps its band 5800-5899; band 9100-9199 is claimed
for this program in the same commit (`docs/MODEL-AB-LOG.md`). Nothing
dispatched.
