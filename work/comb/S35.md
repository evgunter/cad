...
| ~~`transform_rigid` refuses every **described** NURBS surface under an error named `NurbsPlaceholder`, though tier 3 stopped conflating those two states at M6-3 and the same file *was* updated for the description match. Blocks placement of any lofted/swept/imported body with a real NURBS wall~~ **CLOSED 2026-09-04** by FIX PR 1742 (`transform-rigid-refuses-described-nurbs`): both arms gate on `is_placeholder()` and a described net maps by its control points, so every loft/sweep/skinned body is movable. The row's line numbers had already drifted | `transform.rs:248`, `:110` (as filed) | sure |
...
- **The `frame::path_start_frame` row's first half has moved** (SCALAR
  `S393`, 2026-09-15). *"no kernel caller; `sweep` still builds its own
  frames"* was true when the row was written and is not now: the sweep
  test corpus (`crates/sweep/tests/common/mod.rs`'s
  `normal_start_place`, and the `cert5` copy that had gone undeclared)
  and the demo tour's three sweep/loft scenes
  (`demos/tour/src/skinned.rs`) all call the door, and the sweep and
  loft door docs name it as where a caller gets the frame. What the
  row's SECOND half says is untouched and is now the sharper question:
  `path_start_frame` and `Vec3::orthonormal_basis` still answer "an
  axis off a tangent" with two policies and no written rule for which
  a caller uses — filed as its own row on PROPS's slate
  (`work/props/two-doors-answer-an-axis-off-a-tangent-with-no-rule-for-which.md`).
...
Its class at the cut was **H** — sixty-two independent findings spanning
every track; partitioned out row by row. The class is a dispatch
