...
- **~1115 lines of public rustdoc carrying spec codes, in `publish = false`
  ...
  anything does.** Scheduled in the *Last, deliberately* group — this
  program's whole slate — because it collides with every track. (It was
  labelled `C2`/`H17` while code-quality held it; those were a process
  observation and a track row id, and neither survives the sweep of
  2026-09-11. The population and the disposition above are the content.)
...
Its class at the cut was **H** — cross-cutting rustdoc/comment sweep
over four kernel crates; collides with every track. The class is a
