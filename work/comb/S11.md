...
| `nurbs_curve_aabb` | `geom/src/curves/boxes.rs` | Zero production consumers: the conic doors beside it are read by `topo::boolean::boxes::edge_box`, and the surface twin `nurbs_surface_aabb` by `face_box`'s `ControlNet` arm, but the NURBS EDGE arm of `EdgeBoxRule` stays `NoSoundBox` until the rung-3 operand gate admits the kind |
...
| `nurbs_curve_aabb` | **PLANNED** — the NURBS edge arm's box, gated on the rung-3 operand gate admitting `Nurbs` edges (`EdgeBoxRule`'s NURBS bullet states the obligation). |
...
- **`CornerBall::surface`** is a struct **field** with **one** production
  caller (`corner_plan`, `crates/sweep/src/blend/open/planar.rs`), which discards it and rebuilds
...
Its class at the cut was **H** — roll-up over 8 crates; several rows
still await an unasked delete-or-keep call. The class is a dispatch
