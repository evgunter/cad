...
Moved from `work/issues/` to `work/blend/` in the tracker-wide cut of 2026-09-06 (Ev's direction, in-chat), which read every open `work/issues/` file and every open code-quality row against every live program's `paths` and opened four programs for the ground none covered. Id, body and header are unchanged except as noted; the directory is the claim (`work/README.md`). Closed at BLEND's opening as a record, not work: its own `## The ask` says nothing is owed unless a consumer arrives, and the statement it keeps true lives at `blend::surgery::HostSide`'s doc. Reopen it there if a consumer does.
