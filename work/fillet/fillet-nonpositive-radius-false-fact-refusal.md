...
The issue names S-BLEND territory (`crates/sweep/src/blend/build.rs`), but S-BLEND is closed (`docs/DOC-LEDGER.md`; its tracker directory deleted) and no open program's territory covers the blend module — so it lands in `work/issues/` until a program claims the blend doors.
