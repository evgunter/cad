...
Both halves of the Q1 ruling landed: the lattice half with BOOL-8,
BOOL-11 and BOOL-12 (the declared point-target continuation, the seam
declarations, `Start.arrives_tangent()`), the raw-door half with BOOL-9
(PR 2134, merged 2026-09-08): `RawLoop` demoted to a test-support door,
authoring through the lattice only. The original "disagreement" is
written up at `validate.rs`'s header and PATHS §4 as two questions —
the lattice checks AUTHORING, `validate` checks the MATERIALIZED table
— not one rule with two answers.
