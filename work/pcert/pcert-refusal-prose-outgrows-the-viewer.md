---
id: pcert-refusal-prose-outgrows-the-viewer
kind: issue
title: geom-brep: PcurveCertifyError::FittedCertificate is over 50 words (Ev's concision request)
status: open
opened: 2026-09-22
...
| words | site | arm |
|---|---|---|
| 55 | `geom-brep/src/pcurve_cache.rs` `PcurveCertifyError::FittedCertificate` | |
...
A `topo::ValidationError` that carries a `PcurveCertifyError` (through `PcurveMintError::Certify`) used to render it whole
...
itself, as the callers that hold a `PcurveCertifyError` (through `PcurveMintError::Certify`) directly still read it.
