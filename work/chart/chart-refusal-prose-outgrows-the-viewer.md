---
id: chart-refusal-prose-outgrows-the-viewer
kind: issue
title: topo: the chart-region refusals are too long for the viewer (Ev's concision request)
status: open
opened: 2026-09-22
...
| words | site | arm |
|---|---|---|
| 72 | `topo/src/chart_region.rs` `ChartRegionError::CarrierTilt` | |
| 70 | `ChartRegionError::NonPlanarTrim` | |
| 59 | `ChartRegionError::ChartDivergence` | |
| 53 | `ChartRegionError::PeriodFold` | |
| 51 | `ChartRegionError::RayExhausted` | |
| 51 | `ChartRegionError::ArmUnbounded` | |
...
A `topo::ValidationError` that carries a `ChartRegionError` (through `CensusUnsupportedCause::ChartRegion`) used to render it whole
behind the at-rest and product badges, composed counts up to 99
...
itself, as the callers that hold a `ChartRegionError` (through `CensusUnsupportedCause::ChartRegion`) directly still read it.
