...
| ~~`viewer-pathverb-all-hand-written-seventeen`~~ | **M** | **Closed** (#2830), by removing the mirror rather than adding a census: `PathVerb`/`PathStep` are deleted and the path form edits `profile::Step<f64>` and walks `Verb::ALL`, so there is no viewer vocabulary left to count. | `crates/viewer/src/sketch.rs` (`fresh_step`), `crates/viewer/src/widgets.rs` (`path_step_fields`) |
