...
Not the full orchestrator protocol. Per Ev, 2026-08-20, and identical
to Track F's:
...
**Do not resolve an Ev-only decision.** §D's *Decisions only Ev can make*
...
**Wave 2 — opened 2026-08-20, five lanes, after Ev restored the CI budget.**
...
on Track F's F1 (#791, still open)** — Ev's own S87/S88 sequencing ruling, and
...
visible one. That is Ev's criterion (`SMELL-C-LOG` C-R12) applied, not row
size.
...
per **G-R7** — Ev's own S87/S88 sequencing ruling, and the one constraint on
...
| **G4** | — | gated on Track F's **F1** ← Track E's **#753**. Ev's own S87/S88 ruling; not this orchestrator's to lift |
...
- **The parenthetical is issue #827.** The unreachable r > R class was called
  *"a design question"* with no number and no unit — Q6's shape. Filed with
  what would close it either way; the decision stays Ev's.
...
- **`S135` + `D113`.** Every intra-doc link under `tests/` is inert, because
  ...
  **At least nine** are already broken (nine is a floor: the check resolves a
  ...
  dispositioned. **`D113` holds the decision**, Ev's, after the review
...
- **S114(c)** — **the census exists and the schema does not.** The lane
  ...
  **The design-conversation PR asking Ev is the orchestrator's to open**, and
...
the file is under `crates/`, and #833 was green and holding Ev's
