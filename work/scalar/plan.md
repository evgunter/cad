...
| LANE-4P | the four door values' wiring pins in one helper shape each; the certified-enclosure census enumerates door values (constructors) as well as scalars, each roster entry certifying-set or `f64`-only | E | unowned `props.rs`, CHART (`chart_region.rs` tests), unowned `offset_fit_lane.rs`, TCOST/TINT (the census) | no |
| LANE-4 | `PcurveFittedLane` → a `FittedLane<T>` door value (geom-brep) answered by `AtRestPolicy::fitted_lane()` (topo), per the survey (2026-09-24, `6709bc3229`): 66 production bound sites in five crates, ~38 re-spelled; `lane_name` → a per-scalar name source passed as an argument; `certify_at_dual` becomes a `compile_fail` doctest plus a `None` row; `FittedLane<T>` lands with a wiring helper pinning its three fields and a `ROSTERS` entry (the census reds otherwise); after RING-3, RING-4 and TOPO's #3160 | **H** | PCERT, REACH, CARVE/BAND, ATREST, OFFSET/SHELL, TANG/ZIP, TOPO, TQUERY, CHART, ISO, CURVED, STRUT, WIRE, PROPS, GUARD, TCOST/TINT; unowned `pcurves.rs`/`props.rs` | no |
