...
| --- | --- | --- | --- | --- |
| LANE-0 | the offset-fit hook: `recertify_approx`/`approx_offset_surface`/`remap_certificate` become one `Option<OffsetFitLane>` argument on `tier3_local_checks_marked`, `mint_offset`, `map_approx`; `Some` only from the `f64` seam arms | E | TOPO (`validate.rs`), SHELL (`replace_face.rs`, `transform.rs`), unowned `topo/src/props.rs` | no |
