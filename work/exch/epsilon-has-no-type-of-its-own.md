...
## Closed (2026-09-04) — ruled: no ε type; the checks stay local

Ev, in-chat, 2026-09-04. The measured resolution of the placement
questions: there is no ε-alone type, and the three hand validations
stay where they are (each ~3 lines; unify only if a shared helper
falls out naturally). The provenance facts the ruling rests on,
recorded so nobody re-files the S4 shape:

- **Export default** is literally the run's ε — `uncertainty_m: None`
  reads `Tolerance::get().eps` at write time (D4 ¶1, the one ε the
  body was built under). No validation needed on that path.
- **`Some(ε)`** is an explicit interchange declaration with live
  consumers — every committed export fixture passes `Some(1e-9)` so
  fixture bytes are deterministic regardless of the generating run's
  ambient tolerance (`step-export/tests/kernel_sidecars.rs:33`), and
  `pncad-py` exposes it as the bound `uncertainty` option
  (`pncad-py/src/py/value.rs:1262`). It stays. Considered and
  declined: validating the override against the build ε (a
  finer-than-built declaration would be a false claim) — no consumer
  asks, and the finite-and-positive check is the only invariant the
  format itself owes.
- **Import's `eps_in`** is the FILE's declared uncertainty — a
  foreign exporter's ε (`step-import/src/entities.rs:235-245`) — so
  a conversion from our `Tolerance` is wrong there by construction,
  and the door validates because a file can declare 0 or NaN.

So the three sites validate differently-sourced values; the
"one vocabulary, N copies" reading dissolves once the sources are
named. `mesh::sizing::Eps` stays mesh-local per its own collapse-seam
