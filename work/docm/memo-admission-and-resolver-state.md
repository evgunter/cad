...
Ruled for option 2: `docs/DOCM-IDENTITY-DESIGN.md` DI2 (the memo is a
pure function of the document; A4's refusal sentence narrows to the
seam; store freshness is the session's, and `Reevaluate` re-mounts the
store) and DI3 (an evaluation carries its document's identity, the
pairing class this item's comment named). The kernel build is `DOCM-4`;
the session half rides `document-seam-no-in-session-change-detection`
(CHROME). One premise here was stale: the session hands no `prior`
(`request_eval`); the memo lives in `viewer::evalseam::PriorRun`.
