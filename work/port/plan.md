...
| `load-path-stringifies-structured-refusals` | **H** (confirmed by the lane, PORT-DIMS-1) | Arrived 2026-09-13 at DOCM's exit sweep, after the table above was first written. Structured kernel refusals stringified at the load door; carries a two-sweep class obligation. **The one row this program gives a full review.** | `crates/editor-core/src/persist/wire.rs` (`Error::custom`), `crates/pncad-py/src/tags.rs`, `crates/pncad-py/src/py/value.rs`, `py/flush.rs` |
| `python-dimensionerror-names-the-quantity-check-not-the-dimension-check` | **M** (rides PORT-DIMS-1's **H**) | **Ruled a defect** (Ev, 2026-09-15) — `S107`'s successor, filed the day the ruling landed. The rename is small; the re-documentation it deletes is the bulk. | `crates/pncad-py/src/errors.rs`, `src/py/mod.rs`, the door docstrings in `src/py/*`, `pncad.pyi`, `src/tests.rs` + the py suite |
