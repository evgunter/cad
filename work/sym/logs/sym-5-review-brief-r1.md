# SYM-5 PR-2 review brief — r1 (rule E, the quotient's common factor)

You are reviewer **r1** of the v6 dual for unit SYM-5 of program
...
- Worktree `/home/user/lanes/sym-5-r1` at the frozen head, branch
  `sym/5-review-r1` for probe rows. Every command:
  `cd /home/user/lanes/sym-5-r1 && CARGO_TARGET_DIR=/home/user/sym-5-r1-target CARGO_INCREMENTAL=0 …`
  on ONE line. Wait until `/home/user/sym-5-r1-target/SEEDED`
  exists before any cargo command (foreground `ls` poll). Private
  scratch `/home/user/sym-5-r1-tmp/`.
