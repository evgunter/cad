# SYM-4 review brief — r1 (the cost of a form)

You are reviewer **r1** of the v6 dual for unit SYM-4 of program
...
- Worktree `/home/user/lanes/sym-4-r1`, checked out at the frozen
  head on branch `sym/4-review-r1` for any probe rows. Every
  command: `cd /home/user/lanes/sym-4-r1 && …` with
  **`CARGO_TARGET_DIR=/home/user/sym-4-r1-target` on the same
  line**. Wait until `/home/user/sym-4-r1-target/SEEDED` exists
  before any cargo command (foreground `ls` poll). Private scratch
  `/home/user/sym-4-r1-tmp/`.
