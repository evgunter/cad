...
is unchanged, which the pins and a new rendered-form digest row hold),
with the degree cached and the monomial's small case inline if the
numbers say so; `Rat::from_parts` skips the gcd on the dyadic shape.
No count moves, no dial, no rule. The drive-scoped memo and the
