...
three in `no-ambient-env.sh`, one in `witness-not-ambient.sh`. An
alternation became the alternation of BUILT anchors: the homes are one
array per gate and a six-line builder joins `gate_record_anchor` over
it, so the exemption and the fixture read the same list and a home
cannot be exempt in one and absent from the other. The two single-home
gates spell it inline, as `signed-zero-one-home.sh` does.
...
`<home>.rs:x.rs` — the one reachable over-match for a `*.rs` scan, and
the whole of the reachable set this row argued — must fire, and the home
must pass. Restoring the hand-spelled literal reds exactly that case in
each of the six and nothing else: with the case removed as well, every
other case is green.
