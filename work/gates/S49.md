...
both ways — a live discard no entry names, and an entry no discard
matches. Re-derived at this branch's merge base: **80 sites in 39
files** (73 let-else, 7 match-arm; 32 `continue`, 46 `return`, 2 empty
arms), of which **2 are audited** and 78 are not. The two audited are
`census.rs`'s `face_points` and `line_bounded` deferrals inside
`sweep_cross_solid_backstop`; `splitting/rules.rs` holds none, because
both of its `Empty` arms bind the lone vertex and use it — one refuses
by name with `CorruptOperand`, the other measures the vertex — so
neither is a discard any more.

The AUDIT of the 78 is not this row's and not GATES': each site belongs
to the program that owns its file (CURVED, S-BOOL, TOPO, and EVAL for
`editor-core/src/names/emit.rs`), and the register is what those audits
close against. The site list with its dispositions is in the PR body.
