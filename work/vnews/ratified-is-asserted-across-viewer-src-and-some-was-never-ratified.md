...
## A fourth worked example: TRUE of the behaviour, FALSE of the wording (from #2960's review)

Added by `vnews/app-controls-read-their-refusals`, whose review
found it. It needs a verdict column the table above does not have yet,
because the claim ESCALATES from a ratified clause rather than inventing
one.

| site | claim | verdict |
|---|---|---|
| `crates/viewer/src/session/refuse.rs`, `Refusal::affordance`'s doc | *"a ratified micro-decision whose WORDING is part of the decision"* | **SPLIT** — the behaviour is G4's (`crates/viewer/GUI-DESIGN.md`, Micro-decisions: *"Dragging an expression-driven dimension refuses, with an affordance offering to edit the expression"*), and G4 states **no sentence**. *"driven by an expression over X (currently 0.01) — edit the expression?"* is this crate's own composition. |
| `crates/viewer/src/session/refuse.rs`, `impl Display for Refusal`'s doc | *"the affordance arm's wording is a RATIFIED decision of this layer's"* | **FALSE** — the same escalation, and it names no clause at all |

The same clause backs `frame.rs:565` above as TRUE, which is the point
of the split: a census that sorts by clause will pass these two as
members of a TRUE family unless it asks what the clause DECIDES. The
one-home rule these docs draw from the claim is right on its own merits
— two copies of any sentence drift — so the repair is to give that
reason instead, not to move the sentence.

It is live rather than academic: a lane asked to reword the affordance
today would read these two comments, conclude the change needs Ev, and
stall or open an `[ev]` PR for a decision Ev never made.
